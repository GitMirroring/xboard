...
void WidgetEcho(Option * opt, int state) {}
...
void SetComboChoice(Option * opt, int n) { SetWidgetText(opt, opt->choice[n], MasterDlg); }
...
int ReadScroll(Option * opt, float * top, float * bottom) {
    ...
    j++;
    XtSetArg(args[j], XtNtopOfThumb, top);
    j++;
...
void HighlightListBoxItem(Option * opt, int nr) { XawListHighlight(opt->handle, nr); }
...
void HighlightWithScroll(Option * opt, int sel, int max) {
    ...
    f = g = top;
    top *= max;
    if (sel > (top + 3 * bottom) / 4) {
        f = (sel - 0.75f * (bottom - top)) / max;
    } else if (sel < (3 * top + bottom) / 4) {
        f = (sel - 0.25f * (bottom - top)) / max;
    }
    if (f < 0.f) {
        f = 0.;
    }
...
void Show(Option * opt, int hide) {
#if 0
    /* TODO: Determine if this code should be removed or resurrected.  It clearly used to do something at one point, but determining
       when and why it broke would require investigation. */
    static Dimension h;
    Arg args[16];
    Dimension v;
    int j = 0;
    /* FIXME: it would be nice if the Chat window did have an ICS pane we could hide behind */
    return;
    /*printf("Show(%d) %x\n", hide, opt->handle);*/
    if (!opt->handle) {
        return;
    ...
    if (hide) {
        /* make sure original size is saved */
        XtSetArg(args[j], XtNheight, &v);
        j++;
    ...
    printf("h = %d\n", h);
    j = 0;
    XtSetArg(args[j], XtNheight, hide ? 1 : h);
    j++;
...
void HighlightText(Option * opt, int start, int end, Boolean on) {
    if (on) {
        /* for lack of a better method, use selection for highlighting */
...
static void CheckCallback(Widget ww, XtPointer client_data, XEvent * event, Boolean * b) {
    int s, data = (intptr_t)client_data;
...
static void SpinCallback(Widget w, XtPointer client_data, XtPointer call_data) {
    String name, val;
    Arg args[16];
    char buf[MSG_SIZ], *p;
    int j = 0;
...
char moveTypeInTranslations[] = "<Key>Return: TypeInProc(1) \n"
...
/* Beware! Order must correspond to DialogClass enum */
WindowPlacement * wp[NrOfDialogs] = {
...
int PopDown(DialogClass n) {
    ...
    if (n && wp[n]) {
        ...
        j++;
        XtSetArg(args[j], XtNy, &windowY);
        j++;
        XtSetArg(args[j], XtNheight, &windowH);
        j++;
        XtSetArg(args[j], XtNwidth, &windowW);
        j++;
...
void SetColor(char * colorName, Option * box) {
    ...
    if (!appData.monoMode) {
        ...
        buttonColor = timerBackgroundPixel;
    }
    ...
    case Expose:
        ...
        if (w < graph->max || w > graph->max + 1 /* use width fudge of 1 pixel */ || h != graph->value) {
            ...
            if (((XExposeEvent *)event)->count >= 0) {
                ...
                graph->max = w;
                graph->value = h;
                /* note: old values are kept if we we don't exceed width fudge */
            }
        } else {
            w = graph->max;
        }
        ...
        if (((XExposeEvent *)event)->x + w > graph->max) {
            /* cut off fudge pixel */
            w--;
        }
        if (w) {
            ExposeRedraw(graph, ((XExposeEvent *)event)->x, ((XExposeEvent *)event)->y, w, ((XExposeEvent *)event)->height);
        }
        return;
    case MotionNotify:
        f = 0;
...
static void GenericCallback(Widget w, XtPointer client_data, XtPointer call_data) {
    ...
    if (data == 30000) {
        ...
        /* save buttons imply OK */
        if (GenericReadout(currentOption, -1)) {
            /* calls OK-proc after full readout, but no popdown if it returns false */
            PopDown(dlg);
        }
    } else
    ...
    for (opt = currentOption; opt->type != EndMark; opt++) {
        if (opt->handle == w) {
            while (++opt) {
                ...
                if (opt->type == TextBox || opt->type == Spin || opt->type == Fractional || opt->type == FileName ||
                 opt->type == PathName) {
...
static char * oneLiner = "<Key>Return: redraw-display() \n \
    <Key>Tab: TabProc() \n ";
static char scrollTranslations[] = "<Btn1Up>(2): WheelProc(0 0 A) \n \
...
int SetPositionAndSize(Arg * args, Widget leftNeigbor, Widget topNeigbor, int b, int w, int h, int chaining) {
    ...
    if (chaining & 1) {
        /* same row: position w.r.t. last (on current row) and lastrow */
        XtSetArg(args[j], XtNfromVert, topNeigbor);
        j++;
        XtSetArg(args[j], XtNfromHoriz, leftNeigbor);
        j++;
    } else {
        /* otherwise, it goes at left margin (which is default), below the previous element */
...
int GenericPopUp(Option * option, char * title, DialogClass dlgNr, DialogClass parent, int modal, int top) {
    Arg args[24];
    Widget popup, layout, dialog = NULL, edit = NULL, form, last, b_ok, b_cancel, previousPane = NULL, textField = NULL, oldForm,
                          oldLastRow, oldForeLast;
    Window root, child;
    ...
    if (dlgNr && dlgNr < PromoDlg && shells[dlgNr]) {
        /* reusable, and used before (but popped down) */
        XtPopup(shells[dlgNr], XtGrabNone);
        shellUp[dlgNr] = TRUE;
        return 0;
    }
    if (dlgNr == TransientDlg && parent == BoardWindow && shellUp[MasterDlg]) {
        /* MasterDlg can always take role of main window */
        parent = MasterDlg;
    }

    /* make available to callback */
    dialogOptions[dlgNr] = option;
    ...
    if (engineDlg) {
        /* Settings popup for engine: format through heuristic */
        ...
        /* Delimit list by callback-less end mark. */
    ...
    if (dlgNr == BoardWindow) {
        ...
        popup = shells[dlgNr] = XtCreatePopupShell(
         title, !top || !appData.topLevel ? transientShellWidgetClass : topLevelShellWidgetClass, shells[parent], args, i);
    }

    layout = XtCreateManagedWidget(layoutName, formWidgetClass, popup, layoutArgs, XtNumber(layoutArgs));
    if (!appData.monoMode && appData.dialogColor[0]) {
        XtSetArg(args[0], XtNbackground, dialogColor);
    }
    XtSetValues(layout, args, 1);

    for (c = 0; c < width; c++) {
        pane[4] = 'A' + c;
        form = XtCreateManagedWidget(pane, formWidgetClass, layout, formArgs, XtNumber(formArgs));
        j = 0;
        XtSetArg(args[j], stack ? XtNfromVert : XtNfromHoriz, previousPane);
        j++;
        if (!appData.monoMode && appData.dialogColor[0]) {
            XtSetArg(args[j], XtNbackground, dialogColor), j++;
        }
...
tBox:
                if (option[i].name[0]) {
                    /* prefixed by label with option name */
                    j = SetPositionAndSize(
                     args, last, lastrow, 0 /* border */, 0 /* w */, textHeight /* h */, 0xc0 /* chain to left edge */);
                    XtSetArg(args[j], XtNjustify, XtJustifyLeft);
                    j++;
                    XtSetArg(args[j], XtNlabel, _(option[i].name));
                    j++;
                    texts[h] = dialog = XtCreateManagedWidget(option[i].name, labelWidgetClass, form, args, j);
                } else {
                    /* kludge to position from left margin */
                    dialog = NULL;
                    texts[h] = NULL;
                }
                ...
                j = SetPositionAndSize(args, dialog, last, 1 /* border */, w /* w */,
                 option[i].type == TextBox ? option[i].value : 0 /* h */, 0x91 /* chain full width */);
                if (option[i].type == TextBox) {
                    /* decorations for multi-line text-edits */
                    if (option[i].min & T_VSCRL) {
                        XtSetArg(args[j], XtNscrollVertical, XawtextScrollAlways);
                        j++;
                    }
                    if (option[i].min & T_HSCRL) {
                        XtSetArg(args[j], XtNscrollHorizontal, XawtextScrollAlways);
                        j++;
                    }
                    ...
                    if (option[i].min & T_WRAP) {
                        XtSetArg(args[j], XtNwrap, XawtextWrapWord);
                        j++;
                    }
                    if (option[i].min & T_TOP) {
                        ...
                        j++;
                        if (!option[i].value) {
                            XtSetArg(args[j], XtNbottom, XtChainTop);
                            j++;
                ...
                j = SetPositionAndSize(args, last, edit, 3 /* border */, w /* w */, j /* h */, 0x31 /* chain to right edge */);
                ...
                j = SetPositionAndSize(
                 args, last, edit, 3 /* border */, 20 /* w */, textHeight / 2 /* h */, 0x31 /* chain to right edge */);
                XtSetArg(args[j], XtNvertDistance, -1);
                j++;
            ...
            case CheckBox:
                if (!engineDlg) {
                    /* where checkbox callback uses it */
                    option[i].value = *(Boolean *)option[i].target;
                }
                j = SetPositionAndSize(args, last, lastrow, 1 /* border */, textHeight / 2 /* w */, textHeight / 2 /* h */,
                 0xc0 /* chain both to left edge */);
                XtSetArg(args[j], XtNvertDistance, (textHeight + 2) / 4 + 3);
                j++;
                XtSetArg(args[j], XtNstate, option[i].value);
                j++;
                lastrow = last;
                option[i].handle = (void *)(last = XtCreateManagedWidget(" ", toggleWidgetClass, form, args, j));
                j = SetPositionAndSize(
                 args, last, lastrow, 0 /* border */, option[i].max /* w */, textHeight /* h */, 0xc1 /* chain */);
                XtSetArg(args[j], XtNjustify, XtJustifyLeft);
                j++;
                XtSetArg(args[j], XtNlabel, _(option[i].name));
                j++;
                ...
                /* following buttons must get text height */
                shrink = TRUE;
                break;
            case Icon:
            case Label:
                ...
                j = SetPositionAndSize(args, last, lastrow, (chain & 2) != 0 /* border */, option[i].max /* w */,
                 shrink ? textHeight : 0 /* h */, chain | 2 /* chain */);
#if ENABLE_NLS
                if (option[i].choice) {
                    XtSetArg(args[j], XtNfontSet, *(XFontSet *)option[i].choice), j++;
                }
#else
                if (option[i].choice) {
                ...
                XtSetArg(args[j], XtNjustify, XtJustifyLeft);
                j++;
                XtSetArg(args[j], XtNlabel, _(msg));
                j++;
            ...
            case Button:
                if (option[i].min & SAME_ROW) {
                    /* 0011.0001 = both left and right side to right edge */
                    chain = 0x31;
                    forelast = lastrow;
                } else {
                    chain = 0, shrink = FALSE;
                }
                j = SetPositionAndSize(args, last, lastrow, 3 /* border */, option[i].max /* w */, shrink ? textHeight : 0 /* h */,
                 option[i].min & 0xe | chain /* chain */);
                XtSetArg(args[j], XtNlabel, _(option[i].name));
                j++;
                if (option[i].textValue && *option[i].textValue == '#') {
                    /* special for buttons of New Variant dialog */
                    char *p = NULL, *v, n = option[i].value;
                    if (n >= 0) {
                        v = VariantName(n), p = strstr(first.variants, v);
                    }
                    XtSetArg(args[j], XtNsensitive,
                     option[i].value >= 0 &&
                      (appData.noChessProgram || p && (!*v || strlen(p) == strlen(v) || p[strlen(v)] == ',')));
                    j++;
                    XtSetArg(args[j], XtNborderWidth, (gameInfo.variant == option[i].value) + 1);
                    j++;
                }
                option[i].handle =
                 (void *)(dialog = last = XtCreateManagedWidget(option[i].name, commandWidgetClass, form, args, j));
                if (option[i].choice && ((char *)option[i].choice)[0] == '#' && !engineDlg) {
                    /* for the color picker default-reset */
                ...
                XtAddCallback(last, XtNcallback, GenericCallback, (XtPointer)(intptr_t)i + (dlgNr << 16));
                if (option[i].textValue && *option[i].textValue == '#') {
                    /* for new-variant buttons */
                    SetColor(option[i].textValue, &option[i]);
                }
                break;
            case ComboBox:
                j = SetPositionAndSize(
                 args, last, lastrow, 0 /* border */, 0 /* w */, textHeight /* h */, 0xc0 /* chain both sides to left edge */);
                XtSetArg(args[j], XtNjustify, XtJustifyLeft);
                j++;
                XtSetArg(args[j], XtNlabel, _(option[i].name));
                j++;
                ...
                if (option[i].min & COMBO_CALLBACK) {
                    msg = _(option[i].name);
                } else {
                    if (!engineDlg) {
                        SetCurrentComboSelection(option + i);
                    }
                    msg = _(((char **)option[i].choice)[option[i].value]);
                }

                /* same row as its label! */
                j = SetPositionAndSize(args, dialog, last, (option[i].min & 2) == 0 /* border */,
                 option[i].max && !engineDlg ? option[i].max : 100 /* w */, textHeight /* h */, 0x91 /* chain */);
                XtSetArg(args[j], XtNmenuName, XtNewString(option[i].name));
                j++;
                XtSetArg(args[j], XtNlabel, msg);
                j++;
                shrink = TRUE;
                option[i].handle = (void *)(last = XtCreateManagedWidget(" ", menuButtonWidgetClass, form, args, j));
            ...
            case ListBox:
                ...
                /* for mouse-wheel */
                XtOverrideTranslations(edit, XtParseTranslationTable(scrollTranslations));
                break;
            case Graph:
                j = SetPositionAndSize(
                ...
                /* mandatory user-supplied expose handler */
                XtAddEventHandler(last, ExposureMask | ButtonPressMask | ButtonReleaseMask | PointerMotionMask, FALSE,
                 (XtEventHandler)GraphEventProc, &option[i]);
                if (option[i].min & SAME_ROW) {
                    last = forelast, forelast = lastrow;
                }
                /* image buffer */
                option[i].choice = (char **)cairo_image_surface_create(CAIRO_FORMAT_ARGB32, option[i].max, option[i].value);
                break;
            case PopUp:
                /* note: used only after Graph, so 'last' refers to the Graph widget */
            ...
            case BoxBegin:
                ...
                j = SetPositionAndSize(args, last, lastrow, 0 /* border */, 0 /* w */, 0 /* h */, option[i].min /* chain */);
                XtSetArg(args[j], XtNorientation, XtorientHorizontal);
                j++;
                XtSetArg(args[j], XtNvSpace, 0);
                j++;
                option[box = i].handle = (void *)(last = XtCreateWidget("box", boxWidgetClass, form, args, j));
                oldForm = form;
                form = last;
                oldLastRow = lastrow;
                oldForeLast = forelast;
                lastrow = NULL;
                last = NULL;
                break;
            case DropDown:
                j =
                 SetPositionAndSize(args, last, lastrow, 0 /* border */, 0 /* w */, 0 /* h */, 1 /* chain (always on same row) */);
                forelast = lastrow;
                /* write name on the menu button */
                msg = _(option[i].name);
                if (msg) {
                    if (*msg == '_') {
                        msg++;
                    } else if (msg[1] == '_') {
                        /* kludge to remove GTK shortkut-key indicators */
                        static char buf[MSG_SIZ];
                        strncpy(buf, msg, MSG_SIZ);
                        msg = buf + 1;
                        *msg = *buf;
                    }
                }
                ...
                XtSetArg(args[j], XtNlabel, msg);
                j++;
                option[i].handle = (void *)(last = XtCreateManagedWidget(option[i].name, menuButtonWidgetClass, form, args, j));
            ...
            case BoxEnd:
                ...
                if (option[i].target) {
                    ...
                    ((ButtonCallback *)option[i].target)(box);
                }
                last = form;
                lastrow = oldLastRow;
                form = oldForm;
                forelast = oldForeLast;
                break;
            case Break:
                if (c) {
                    break;
        ...
        if (browse != NULL) {
            j = 0;
            XtSetArg(args[j], XtNwidth, &bWidth);
            j++;
        ...
        for (h = 0; h < height || c == width - 1; h++) {
            ...
            if (option[i].type == Spin || option[i].type == TextBox || option[i].type == ComboBox || option[i].type == PathName ||
             option[i].type == FileName) {
                Dimension w;
                if (!texts[h]) {
                    continue;
                }
        ...
        for (h = 0; h < height || c == width - 1; h++) {
            ...
            if (!texts[h]) {
                /* Note: texts[h] can be undefined (giving errors in valgrind), but then both if's below will be false. */
                continue;
            }
            j = 0;
            if (option[i].type == Spin) {
                XtSetArg(args[j], XtNwidth, maxWidth);
                j++;
                XtSetValues(texts[h], args, j);
            } else if (option[i].type == TextBox || option[i].type == ComboBox || option[i].type == PathName ||
             option[i].type == FileName) {
                XtSetArg(args[j], XtNwidth, maxTextWidth);
                j++;
                XtSetValues(texts[h], args, j);
                if (bWidth != 50 && (option[i].type == FileName || option[i].type == PathName)) {
                    int tWidth = (option[i].max ? option[i].max : 205) - 5 - bWidth;
                    j = 0;
                    XtSetArg(args[j], XtNwidth, tWidth);
                    j++;
    ...
    /* even when OK suppressed, this EndMark bit can request chaining of last row to bottom */
    if (option[i].min & SAME_ROW) {
        ...
        if ((option[j].type == TextBox || option[j].type == ListBox) && option[j].name[0] == NULLCHAR) {
            ...
            if (option[j].type == ListBox) {
                /* for listbox we must chain viewport */
                w = XtParent(w);
            }
        ...
        shrink = FALSE, lastrow = last, last = widest ? widest : dialog;
    }
    j = SetPositionAndSize(args, last, anchor ? anchor : lastrow, 3 /* border */, 0 /* w */, shrink ? textHeight : 0 /* h */,
     0x37 /* chain: right, bottom and use both neighbors */);
    ...
    if (!(option[i].min & NO_OK)) {
        ...
        if (!(option[i].min & NO_CANCEL)) {
            /* overwrites! */
            XtSetArg(args[1], XtNfromHoriz, b_ok);
    ...
    if (dlgNr != BoardWindow) {
        /* assign close button, and position w.r.t. pointer, if not main window */
        ...
        XQueryPointer(xDisplay, xBoardWindow, &root, &child, &x, &y, &win_x, &win_y, &mask);
    ...
    if (dlgNr && wp[dlgNr]) {
        ...
        if (wp[dlgNr]->width > 0 && wp[dlgNr]->height > 0) {
            XtSetArg(args[j], XtNheight, (Dimension)(wp[dlgNr]->height));
            j++;
            XtSetArg(args[j], XtNwidth, (Dimension)(wp[dlgNr]->width));
            j++;
        }
        if (wp[dlgNr]->x > 0 && wp[dlgNr]->y > 0) {
            XtSetArg(args[j], XtNx, (Position)(wp[dlgNr]->x));
            j++;
            XtSetArg(args[j], XtNy, (Position)(wp[dlgNr]->y));
            j++;
        }
        if (j) {
    ...
    RaiseWindow(dlgNr);
    /* Tells the caller it must do initialization (e.g., add specific event handlers). */
    return 1;
}
...
void SetInsertPos(Option * opt, int pos) {
    Arg args[16];
    if (pos == 999999) {
        /* this kludge to indicate end in GTK is fatal in Xaw */
        char * s;
        GetWidgetText(opt, &s);
        pos = strlen(s) - 1;
    }
...
void TypeInProc(Widget w, XEvent * event, String * prms, Cardinal * nprms) {
    int n = prms[0][0] - '0';
    /* popup shell */
    Widget sh = XtParent(XtParent(XtParent(w)));
    extern int hidden;
    hidden = 0;

    /* Enter or Esc typed from primed text widget: treat as if dialog OK or cancel button hit. */
    if (n < 2) {
        /* figure out what the dialog number is by comparing shells (because we must pass it :( ) */
        int dlgNr;
        for (dlgNr = 0; dlgNr < NrOfDialogs; dlgNr++) {
            if (shellUp[dlgNr] && shells[dlgNr] == sh) {
