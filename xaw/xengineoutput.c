...
static unsigned char WHITE_14[] = {0xe0, 0x01, 0x18, 0x06, 0x04, 0x08, 0x02, 0x10, 0x02, 0x10, 0x01, 0x20, 0x01, 0x20, 0x01, 0x20,
 0x01, 0x20, 0x02, 0x10, 0x02, 0x10, 0x04, 0x08, 0x18, 0x06, 0xe0, 0x01};

static unsigned char BLACK_14[] = {0xe0, 0x01, 0xf8, 0x07, 0xfc, 0x0f, 0xfe, 0x1f, 0xfe, 0x1f, 0xff, 0x3f, 0xff, 0x3f, 0xff, 0x3f,
 0xff, 0x3f, 0xfe, 0x1f, 0xfe, 0x1f, 0xfc, 0x0f, 0xf8, 0x07, 0xe0, 0x01};

static unsigned char ANALYZE_14[] = {0x80, 0x03, 0x60, 0x0c, 0x10, 0x10, 0x90, 0x10, 0xc8, 0x20, 0x08, 0x20, 0x08, 0x20, 0x10, 0x10,
 0x10, 0x10, 0x68, 0x0c, 0x94, 0x03, 0x0a, 0x00, 0x07, 0x00, 0x00, 0x00};

static unsigned char THINK_14[] = {0xe0, 0x00, 0x18, 0x03, 0x44, 0x04, 0x42, 0x08, 0x42, 0x08, 0x41, 0x10, 0xe1, 0x13, 0x41, 0x10,
 0x02, 0x08, 0x02, 0x08, 0x04, 0x04, 0x18, 0x03, 0xe0, 0x00, 0x00, 0x00};

static unsigned char PONDER_14[] = {0x30, 0x03, 0x8c, 0x0c, 0x02, 0x10, 0x01, 0x08, 0x01, 0x10, 0x06, 0x20, 0x04, 0x20, 0x02, 0x10,
 0x04, 0x0c, 0xc8, 0x04, 0x34, 0x03, 0x0e, 0x00, 0x01, 0x00, 0x00, 0x00};

static unsigned char UNKNOWN_14[] = {0xe0, 0x01, 0x58, 0x07, 0xac, 0x0a, 0x56, 0x15, 0xaa, 0x1a, 0x55, 0x35, 0xab, 0x2a, 0x55, 0x35,
...
static void ReadIcon(unsigned char pixData[], int iconNr, Widget w) {
...
void CopyMemoProc(Widget w, XEvent * event, String * prms, Cardinal * nprms) {
    if (appData.pasteSelection) {
        return;
    }
...
void ResizeWindowControls(int mode) {  // another hideous kludge: to have only a single pane, we resize the
    ...
    j = 0;
    XtSetArg(args[j], XtNheight, (XtArgVal)&ew_height);
    j++;
    ...
    if (mode == 0) {
        j = 0;
        XtSetArg(args[j], XtNheight, (XtArgVal)5);
        j++;
        XtSetValues(form2, args, j);
        j = 0;
        XtSetArg(args[j], XtNheight, (XtArgVal)(ew_height - 5));
        j++;
        XtSetValues(form1, args, j);
    } else {
        j = 0;
        XtSetArg(args[j], XtNheight, (XtArgVal)(ew_height / 2));
        j++;
        XtSetValues(form1, args, j);
        j = 0;
        XtSetArg(args[j], XtNheight, (XtArgVal)(ew_height / 2));
