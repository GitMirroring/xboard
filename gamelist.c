...
static void GameListDeleteGame(ListGame * listGame) {
    if (listGame) {
        if (listGame->gameInfo.event) {
            free(listGame->gameInfo.event);
        }
        if (listGame->gameInfo.site) {
            free(listGame->gameInfo.site);
        }
        if (listGame->gameInfo.date) {
            free(listGame->gameInfo.date);
        }
        if (listGame->gameInfo.round) {
            free(listGame->gameInfo.round);
        }
        if (listGame->gameInfo.white) {
            free(listGame->gameInfo.white);
        }
        if (listGame->gameInfo.black) {
            free(listGame->gameInfo.black);
        }
        if (listGame->gameInfo.fen) {
            free(listGame->gameInfo.fen);
        }
        if (listGame->gameInfo.resultDetails) {
            free(listGame->gameInfo.resultDetails);
        }
        if (listGame->gameInfo.timeControl) {
            free(listGame->gameInfo.timeControl);
        }
        if (listGame->gameInfo.extraTags) {
            free(listGame->gameInfo.extraTags);
        }
        if (listGame->gameInfo.outOfBook) {
            free(listGame->gameInfo.outOfBook);
        }
...
int GameListBuild(FILE * f) {
    ...
    do {
        ...
        case PGNTag:
            ...
            if (1) {
                int btm = 0;
                if (currentListGame->gameInfo.fen) {
                    ParseFEN(boards[scratch], &btm, currentListGame->gameInfo.fen, FALSE);
                } else {
                    CopyBoard(boards[scratch], initialPosition);
                }
                plyNr = (btm != 0);
        ...
        case BlackASideCastleFR:
            ...
            if (currentListGame && currentListGame->moves) {
    ...
    if (currentListGame) {
        if (!currentListGame->moves) {
            DisplayError("Game cache overflowed\nPosition-searching might not work properly", 0);
        }

        if (appData.debugMode) {
            for (currentListGame = (ListGame *)gameList.head; currentListGame->node.succ;
...
char * GameListLineOld(int number, GameInfo * gameInfo) {
    char * event = (gameInfo->event && strcmp(gameInfo->event, "?") != 0) ? gameInfo->event : gameInfo->site ? gameInfo->site : "?";
    char * white = gameInfo->white ? gameInfo->white : "?";
    char * black = gameInfo->black ? gameInfo->black : "?";
    char * date = gameInfo->date ? gameInfo->date : "?";
    int len = 10 + strlen(event) + 2 + strlen(white) + 1 + strlen(black) + 11 + strlen(date) + 1;
    char * ret = (char *)malloc(len);
    sprintf(ret, "%d. %s, %s-%s, %s, %s", number, event, white, black, PGNResult(gameInfo->result), date);
    return ret;
}
...
char * GameListLineFull(int number, GameInfo * gameInfo) {
    ...
    sprintf(ret, "%d, \"%s\", \"%s\", \"%s\", \"%s\", \"%s\", \"%s\", \"%s\", \"%s\", \"%s\"", number, event, site, round, white,
     black, PGNResult(gameInfo->result), reason, date, oob);
