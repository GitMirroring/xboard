...
static int GameListCreate(char * name) {
    int new;
    if (new = GenericPopUp(gamesOptions, name, GameListDlg, BoardWindow, NONMODAL, appData.topLevel)) {
        AddHandler(&gamesOptions[1], GameListDlg, 4), AddHandler(&gamesOptions[0], GameListDlg, 5);
    }
...
static int GameListPrepare(int byPos, int narrow) {  // [HGM] filter: put in separate routine, to make callable from call-back
    ...
    if (appData.debugMode) {
        GetTimeMark(&t2);
        printf("GameListPrepare %ld msec\n", SubtractTimeMarks(&t2, &t));
    }
    DisplayTitle("XBoard");
    *st = NULL;
    return listLength;
...
static void GameListReplace(int page) {
    ...
    for (i = 0; i < 1000; i++) {
        if (!(*st++ = glc->strings[page + i])) {
            st--;
            break;
        }
    }
    listEnd = st - list;
    if (page + 1000 <= listLength) {
        *st++ = _("next page");
    }
    *st = NULL;
    ...
    snprintf(buf, MSG_SIZ, _("%s - %d/%d games (%d-%d-%d)"), glc->filename, listLength, ((ListGame *)gameList.tailPred)->number,
     wins, losses, draws);
...
FILE * GameFile(void) { return glc ? glc->fp : NULL; }
