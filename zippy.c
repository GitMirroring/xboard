...
void Speak(char * how, char * whom) {
    ...
    if (strcmp(how, "shout") == 0) {
        ...
        if (now - lastShout < 1 * 60) {
            return;
        }
        lastShout = now;
        if (appData.zippyUseI) {
            how = swifties[(unsigned)random() % (sizeof(swifties) / sizeof(char *))];
        }
    }
...
int ZippyCalled(char * str) { return ics_handle[0] != NULLCHAR && StrCaseStr(str, ics_handle) != NULL; }
...
static void SetCurColor(ColorClass color) { curColor = color; }
...
int ZippyControl(char * buf, int * i) {
    ...
    if (appData.zippyPlay && appData.zippyNoplayCrafty && forwardMostMove < 4 &&
     looking_at(buf, i, "* kibitzes: Hello from Crafty")) {
        ...
        if ((gameMode == IcsPlayingWhite && StrCaseCmp(player, gameInfo.black) == 0) ||
         (gameMode == IcsPlayingBlack && StrCaseCmp(player, gameInfo.white) == 0)) {
            snprintf(reply, MSG_SIZ, "%ssay This computer does not play Crafty clones\n%sabort\n%s+noplay %s\n", ics_prefix,
             ics_prefix, ics_prefix, player);
            SendToICS(reply);
        }
        return TRUE;
    }
    ...
    /* the engine.  */
    if (appData.zippyPlay && looking_at(buf, i, "* is in the computer list")) {
        int i;
        for (i = 0; i < num_opps; i++) {
            if (!strcmp(opp_name[i], star_match[0])) {
                break;
            }
        }
        if (i >= num_opps) {
            safeStrCpy(opp_name[num_opps++], star_match[0], sizeof(opp_name[num_opps]) / sizeof(opp_name[num_opps][0]));
        }
    }
    if (appData.zippyPlay && looking_at(buf, i, "* * is a computer *")) {
        int i;
        for (i = 0; i < num_opps; i++) {
            if (!strcmp(opp_name[i], star_match[1])) {
                break;
            }
        }
        if (i >= num_opps) {
            safeStrCpy(opp_name[num_opps++], star_match[1], sizeof(opp_name[num_opps]) / sizeof(opp_name[num_opps][0]));
        }
    }
    ...
    if (appData.zippyPlay && appData.zippyBughouse && first.initDone && looking_at(buf, i, "* agrees to be your partner")) {
        player = StripHighlightAndTitle(star_match[0]);
    ...
    if (appData.zippyPlay && appData.zippyBughouse && first.initDone &&
     (looking_at(buf, i, "are no longer *'s partner") ||
      looking_at(buf, i, "* tells you: [automatic message] I'm no longer your"))) {
        player = StripHighlightAndTitle(star_match[0]);
        if (strcmp(zippyPartner, player) == 0) {
    ...
    if (appData.zippyPlay && appData.zippyBughouse && first.initDone &&
     (looking_at(buf, i, "no longer have a bughouse partner") || looking_at(buf, i, "partner has disconnected") ||
    ...
    if (appData.zippyPlay && appData.zippyBughouse && first.initDone && looking_at(buf, i, "* (your partner) tells you: *")) {
    ...
    if (looking_at(buf, i, "* tells you: *") || looking_at(buf, i, "* says: *")) {
        player = StripHighlightAndTitle(star_match[0]);
        if (appData.zippyPassword[0] != NULLCHAR &&
         strncmp(star_match[1], appData.zippyPassword, strlen(appData.zippyPassword)) == 0) {
         ...
         strncmp(star_match[1], appData.zippyPassword2, strlen(appData.zippyPassword2)) == 0) {
         ...
         strncmp(star_match[1], appData.zippyWrongPassword, strlen(appData.zippyWrongPassword)) == 0) {
            ...
            SendToICS(reply);
        } else if (appData.zippyBughouse && first.initDone && strcmp(player, zippyPartner) == 0) {
...
int ZippyConverse(char * buf, int * i) {
    ...
    /* Shouts and emotes */
    if (looking_at(buf, i, "--> * *") || looking_at(buf, i, "* shouts: *")) {
        if (appData.zippyTalk) {
            char * player = StripHighlightAndTitle(star_match[0]);
            if (strcmp(player, ics_handle) == 0) {
                return TRUE;
            } else if (appData.zippyPinhead[0] != NULLCHAR && StrCaseStr(star_match[1], appData.zippyPinhead) != NULL) {
    ...
    /* Messages */
    if ((looking_at(buf, i, ". * (*:*): *") && isdigit(star_match[1][0])) || looking_at(buf, i, ". * at *:*: *")) {
        if (appData.zippyTalk) {
            FILE * f;
            char * player = StripHighlightAndTitle(star_match[0]);
    ...
    if (looking_at(buf, i, "*(*: *")) {
        char * channel;
        if (star_match[0][0] == NULLCHAR || strchr(star_match[0], ' ') || strchr(star_match[1], ' ')) {
    ...
    if ((looking_at(buf, i, "You have * message") && atoi(star_match[0]) != 0) ||
     looking_at(buf, i, "* has left a message for you") || looking_at(buf, i, "* just sent you a message")) {
...
void ZippyGameEnd(ChessMove result, char * resultDetails) {
    if (appData.zippyAcceptOnly[0] == NULLCHAR && appData.zippyGameEnd[0] != NULLCHAR) {
...
void ZippyHandleChallenge(char * srated, char * swild, char * sbase, char * sincrement, char * opponent) {
    char buf[MSG_SIZ];
    int i = 0;
    ...
    if (variant == VariantLoadable) {
        snprintf(buf, MSG_SIZ, "%stell %s This computer can't play wild type %s\n%sdecline %s\n", ics_prefix, opponent, swild,
         ics_prefix, opponent);
        SendToICS(buf);
        return;
    }
    if (StrStr(appData.zippyVariants, varname) == NULL ||
     ((i = first.protocolVersion) != 1 && StrStr(first.variants, varname) == NULL) /* [HGM] zippyvar */
        ...
        snprintf(buf, MSG_SIZ, "%stell %s This computer can't play %s [%s], only %s\n%sdecline %s\n", ics_prefix, opponent, swild,
         varname, i ? first.variants : appData.zippyVariants, /* [HGM] zippyvar */
         ics_prefix, opponent);
        SendToICS(buf);
        return;
    }
    ...
    if (appData.zippyAcceptOnly[0] != NULLCHAR && StrCaseCmp(opponent, appData.zippyAcceptOnly)) {
    ...
    if (appData.zippyMaxGames && strcmp(opponent, zippyLastOpp) == 0 && zippyConsecGames >= appData.zippyMaxGames &&
     difftime(time(0), zippyLastGameEnd) < appData.zippyReplayTimeout) {
        snprintf(buf, MSG_SIZ,
         "%stell %s Sorry, you have just played %d consecutive games against %s.  To give others a chance, please wait %d seconds or until someone else has played.\n%sdecline %s\n",
         ics_prefix, opponent, zippyConsecGames, ics_handle, appData.zippyReplayTimeout, ics_prefix, opponent);
        SendToICS(buf);
        return;
    }

    /* [HGM] aborter: opponent is cheater that aborts games he doesn't like on first move. Make him wait */
    if (strcmp(opponent, zippyOffender) == 0 && difftime(time(0), zippyLastGameEnd) < appData.zippyReplayTimeout) {
        snprintf(buf, MSG_SIZ,
         "%stell %s Sorry, your previous game against %s was rather short. "
         " It will wait %d seconds to see if a tougher opponent comes along.\n%sdecline %s\n",
         ics_prefix, opponent, ics_handle, appData.zippyReplayTimeout, ics_prefix, opponent);
        SendToICS(buf);
        return;
    }
...
int ZippyMatch(char * buf, int * i) {
    if (looking_at(buf, i, "* * match * * requested with * (*)")) {
        ZippyHandleChallenge(star_match[0], star_match[1], star_match[2], star_match[3], StripHighlightAndTitle(star_match[4]));
        return TRUE;
    }

    /* Old FICS 0-increment form */
    if (looking_at(buf, i, "* * match * requested with * (*)")) {
        ZippyHandleChallenge(star_match[0], star_match[1], star_match[2], "0", StripHighlightAndTitle(star_match[3]));
        return TRUE;
    }

    if (looking_at(buf, i, "* has made an alternate proposal of * * match * *.")) {
        ZippyHandleChallenge(star_match[1], star_match[2], star_match[3], star_match[4], StripHighlightAndTitle(star_match[0]));
        return TRUE;
    }
    ...
    if (looking_at(buf, i, "Your opponent offers you a draw") || looking_at(buf, i, "* offers you a draw")) {
    ...
    if (looking_at(buf, i, "requests that the game be aborted") || looking_at(buf, i, "would like to abort")) {
        if (appData.zippyAbort || (gameMode == IcsPlayingWhite && whiteTimeRemaining < 0) ||
    ...
    if (looking_at(buf, i, "requests adjournment") || looking_at(buf, i, "would like to adjourn")) {
...
void ZippyFirstBoard(int moveNum, int basetime, int increment) {
    ...
    if ((startedFromSetupPosition && moveNum == 0) || (!appData.getMoveList && moveNum > 0)) {
        SendToProgram("force\n", &first);
...
void ZippyHoldings(char * white_holding, char * black_holding, char * new_piece) {
    char buf[MSG_SIZ];
