...
unsigned char kanjiTab[] = {
 ...
 0357,
 0274,
 0221,  // kanji notation for arabic digits
 '2',
 0357,
 0274,
 0222,
 ...
 0344,
 0270,
 0200,  // in reality these are numbers in Japanese a=1, b=2 etc.
 'b',
 0344,
 0272,
 0214,
...
char GetKanji(char ** p, int start) {
    ...
    if ((*q & 0x80) == 0) {
        return 0;  // plain ASCII, refuse to parse
    }
...
int KifuMove(char ** p) {
    ...
    if (k < 0) {
        (*p)++;
        return Nothing;
    }  // must try shift-JIS here
    if (k >= '1' && k <= '9') {
        buf[0] = k;
        buf[1] = GetKanji(p, YCO);  // to-square coords
    } else if (k == 'x') {
        if (GetKanji(p, YCO) != ' ') {
            (*p) -= 3;  // skip spacer kanji after recapture
        }
    ...
    if (buf[3] == '+') {
        buf[2] = '+', buf[3] = GetKanji(p, PIECE);  // +N, +L, +S
    }
    k = GetKanji(p, MISC);
    if (k == '@') {  // drop move
    ...
    if (**p == '(' && (*p)[3] == ')') {  // kif disambiguation
        ...
        buf[9] = NULLCHAR;
        (*p) += 4;
        ptr++;  // strip off piece name if we know full from-square
        if (appData.debugMode) {
        ...
        if (kifu * ~1) {  // disambiguation was given, and thus is probably needed
            if (buf[3] != 'B' && buf[3] != 'R') {  // stepper, so distance must be <= 1 (N or L never need vertical disambiguation!)
                if (kifu & 0x10) {
                    *q++ = buf[0] - (wom ? -1 : 1);  // translate left/right/straight to PSN file disambiguators
                }
                if (kifu & 0x20) {
                    *q++ = buf[0] + (wom ? -1 : 1);
                }
                if (kifu & 0x40) {
                    *q++ = buf[0], kifu |= 2;  // kludge: 'straight' only needs disambiguation if forward!
                }
                if (kifu & 2) {
                    *q++ = buf[1] + (wom ? -1 : 1);  // translate forward/backward/sideway to PSN rank disambiguators
                }
                if (kifu & 4) {
                    *q++ = buf[1] - (wom ? -1 : 1);
                }
            ...
            }  // for B, R, +B and +R it gets ugly, as we cannot deduce the distance, and the Disambiguate callback has to directly
               // look at 'kifu'
        }
        *q++ = buf[0];
        *q++ = buf[1];
...
int ReadLine(void) {  // Read one line from the input file, and append to the buffer
    int c;
    char * start = inPtr;
    if (fromString) {
        return 0;  // parsing string, so the end is a hard end
    }
    if (!inputFile) {
        return 0;
    }
    while ((c = fgetc(inputFile)) != EOF) {
        *inPtr++ = c;
        ...
        if (inPtr - inputBuf > PARSEBUFSIZE - 2) {
            inPtr--;  // prevent crash on overflow
        }
    }
    if (inPtr == start) {
        return 0;
...
int Scan(char c, char ** p) {  // line-spanning skip to mentioned character or EOF
    do {
        while (**p) {
            if (*(*p)++ == c) {
                return 0;
...
int SkipWhite(char ** p) {  // skip spaces tabs and newlines; return 1 if anything was skipped
    char * start = *p;
    do {
        while (**p == ' ' || **p == '\t' || **p == '\n' || **p == '\r') {
            (*p)++;
...
int Verb(char * pattern, char ** p) {
    ...
    if (res && !Match("s", p)) {
        Match("ed", p);  // eat conjugation suffix, if any
    }
    return res;
}
...
int RdTime(char c, char ** p) {
    ...
    if (Match(":", p) && Number(p) != BADNUMBER && *p - sec == 3) {  // well formed
        ...
        if (Match(".", p) && Number(p) != BADNUMBER && *(*p)++ == c) {
            return 1;  // well-formed fraction
        }
        *p = sec;
        if (*(*p)++ == c) {
            return 1;  // matching bracket without fraction
        }
    }
    *p = start;  // failure
    return 0;
...
char PromoSuffix(char ** p) {
    char * start = *p;
    if (**p == ' ') {
        return NULLCHAR;  // common case, test explicitly for speed
    }
    ...
    if (**p == '=' || (gameInfo.variant == VariantSChess) && **p == '/') {
        (*p)++;  // optional = (or / for Seirawan gating)
    }
    ...
    if (isalpha(**p) && **p != 'x') {
        return ToLower(*(*p)++);  // reserve 'x' for multi-leg captures?
    }
    if (*p != start) {
        return **p == '+' ? *(*p)++ : '=';  // must be the optional = (or =+)
    }
    return NULLCHAR;  // no suffix detected
}

int NextUnit(char ** p) {  // Main parser routine
    ...
    if (**p == NULLCHAR) {  // make sure there is something to parse
        if (fromString) {
            return 0;  // we are parsing string, so the end is really the end
        }
    ...
    if (**p & 0x80) {
        return KifuMove(p);  // non-ascii. Could be some kanj notation for Shogi or Xiangqi
    }
    if (**p == '+') {
        (*p)++, promoted++;
    }
    ...
    if (n <=
     2) {  // could be from-square or disambiguator, when -:xX follow, or drop with @ directly after piece, but also to-square
        if (**p == '-' || **p == ':' || **p == 'x' || **p == 'X' ||  // these cannot be move suffix, so to-square must follow
         (**p == '@' || **p == '*') && n == 0 && !promoted && piece) {  // P@ must also be followed by to-square
            separator = *(*p)++;
            if (n == 1) {
                coord[1] = coord[0];  // must be disambiguator, but we do not know which yet
            }
            n = 2;
        ...
        for (i = 3; i > 0; i--) {
            coord[i] = coord[i - 1], type[i] = type[i - 1];  // move to-square to where it belongs
        }
    ...
    if (n == 3) {  // incomplete to-square. Could be Xiangqi traditional, or stuff like fxg
        ...
        if (!piece && type[1] == NOTHING && type[0] == ALPHABETIC && type[2] == ALPHABETIC &&
         (coord[0] != 14 || coord[2] != 14) /* reserve oo for castling! */) {
            piece = 'P';
            n = 4;  // kludge alert: fake full to-square
        }
    ...
    if (n == 4 && type[2] != type[3] &&  // we have a valid to-square (kludge: type[3] can be NOTHING on fxg type move)
     ...
     (type[2] == ALPHABETIC || IS_SHOGI(gameInfo.variant))) {  // in Shogi also allow alphabetic rank
        ...
        if (slash && (!piece || type[1] == NOTHING)) {
            goto badMove;  // slash after piece only in ICS long format
        }
        if (yyskipmoves) {
            return (int)AmbiguousMove; /* not disambiguated */
        }
        ...
        if (type[3] == NOTHING) {
            cl.rtIn = -1;  // for fxg type moves ask for toY disambiguation
        } else if (toY >= BOARD_HEIGHT || toY < 0) {
            return ImpossibleMove;  // vert off-board to-square
        }
        ...
        if (piece) {
            ...
            if (cl.pieceIn == EmptySquare) {
                return ImpossibleMove;  // non-existent piece
            }
            if (promoted) {
        ...
        if (type[1] == NOTHING && type[0] != NOTHING) {  // there is a disambiguator
            if (type[0] != type[2]) {
                coord[0] = -1, type[1] = type[0], type[0] = NOTHING;  // it was a rank-disambiguator
            }
        }
         ...
         (type[0] == NOTHING || type[0] == type[2])) {  // well formed
            ...
            if (**p == 'x' && !cl.promoCharIn) {  // other leg follows
                ...
                if (**p == '-' || **p == 'x') {  // 3-leg move!
                    ...
                    *p = q;  // 2-leg move, rewind to leave reading of 2nd leg to code below
                }
            }
            ...
            if (type[0] != NOTHING && type[1] != NOTHING && type[3] != NOTHING) {  // fully specified.
                 ...
                 **p == '+') {  // could be a protocol move, where bare '+' suffix means shogi-style promotion
                    if (PieceToChar(CHUPROMOTED(realPiece)) == '+') {  // seems to be that
                        if (!autoProm[realPiece]) {
                            **p++;
                        } else {  // for now ignore promosuffix on auto-promoting pieces' protocol moves
                ...
                if (currentMoveString[4] == NULLCHAR) {  // suppy missing mandatory promotion character
                    if (result == WhitePromotion || result == BlackPromotion) {
                        ...
                        case VariantShogi:
                            ...
                            break;
                        case VariantSChess:
                            if (!gameInfo.holdingsSize) {  // missing auto-gating
...
badMove:  // we failed to find algebraic move
        ...
        if (castlingType) {  // code from old parser, collapsed for both castling types, and streamlined a bit
            ...
            if (gate) {  // gating disambiguator present
                if (gate != ff + AAA) {
                    int h = ft;
                    ft = ff;
                    ff = h;  // reverse for gating at Rook square
            ...
            return Nothing;  // prevent this from being mistaken for move number 1
        }
    }
    ...
    *p = oldp;
    ...
    if (**p == '{') {
        ...
        commentEnd = *p;
        if (i) {
            return Comment;  // return comment that runs to EOF immediately
        }
    }
    if (commentEnd) {
        SkipWhite(p);
    ...
    if (result) {
        if (Match(" (", p) && !Scan(')', p) || Match(" {", p) && !Scan('}', p)) {  // there is a comment after the PGN result!
            if (commentEnd) {
                *p = commentEnd;
                return Comment;
            }  // so comment before it is normal comment; return that first
        }
    ...
    if (Match("---", p)) {
        ...
        return Nothing;
    }  // prevent separators parsing as null move
    ...
    if (*p != oldp) {
        ...
        if (lastChar == '\n' && Match(": ", p)) {  // mail header, skip indented lines
            do {
                ...
                if (!ReadLine()) {
                    return Nothing;  // append next line if not EOF
                }
            } while (Match("\n ", p) || Match("\n\t", p));
        }
        return Nothing;
...
void yynewfile(FILE * f) {  // prepare parse buffer for reading file
    inputFile = f;
...
int yylex(void) {  // this replaces the flex-generated parser
    int result = NextUnit(&parsePtr);
    char *p = parseStart, *q = yytext;
    if (p == yytext) {
        return result;  // kludge to allow kanji expansion
    }
    while (p < parsePtr) {
        *q++ = *p++;  // copy the matched text to yytext[]
    }
    *q = NULLCHAR;
    lastChar = q[-1];
    return result;
...
int Myylex(void) {  // [HGM] wrapper for yylex, which treats nesting of parentheses
