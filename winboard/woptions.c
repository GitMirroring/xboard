...
extern COLORREF lightSquareColor, darkSquareColor, whitePieceColor, blackPieceColor, highlightSquareColor, premoveHighlightColor;
...
VOID PaintSampleSquare(HWND hwnd, int ctrlid, COLORREF squareColor, COLORREF pieceColor, COLORREF squareOutlineColor,
 COLORREF pieceDetailColor, BOOL isWhitePiece, BOOL isMono, HBITMAP pieces[3]);
...
char * InterpretFileName(char * buf,
 char * homeDir) {  // [HGM] file name relative to homeDir. (Taken out of SafeOptionsDialog, because it is generally useful)
    char * result = NULL;
    if ((isalpha(buf[0]) && buf[1] == ':') || (buf[0] == '\\' && buf[1] == '\\')) {
        return strdup(buf);
    } else {
...
LRESULT CALLBACK GeneralOptionsDialog(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam) {
...
#undef CHECK_BOX
        ...
        EnableWindow(GetDlgItem(hDlg, OPT_AutoFlag), appData.icsActive || !appData.noChessProgram);
        EnableWindow(GetDlgItem(hDlg, OPT_AutoFlipView), appData.icsActive || !appData.noChessProgram);
        EnableWindow(GetDlgItem(hDlg, OPT_PonderNextMove), !appData.noChessProgram);
        EnableWindow(GetDlgItem(hDlg, OPT_PeriodicUpdates), !appData.noChessProgram && !appData.icsActive);
        EnableWindow(GetDlgItem(hDlg, OPT_ShowThinking), !appData.noChessProgram);
        return TRUE;
...
#undef IS_CHECKED
            ...
            if (oldAutoLogo != appData.autoLogo) {  // [HGM] logo: remove any logos when we switch autologo off
                if (oldAutoLogo) {
                    first.programLogo = second.programLogo = NULL;
                }
                InitDrawingSizes(boardSize, 0);
            } else if (oldShowButtonBar != appData.showButtonBar) {
                InitDrawingSizes(boardSize, 0);
            } else if ((oldShowCoords != appData.showCoords) || (oldBlindfold != appData.blindfold)) {
...
VOID GeneralOptionsPopup(HWND hwnd) {
    ...
    DialogBox(hInst, MAKEINTRESOURCE(DLG_GeneralOptions), hwnd, (DLGPROC)lpProc);
...
VOID PaintSampleSquare(HWND hwnd, int ctrlid, COLORREF squareColor, COLORREF pieceColor, COLORREF squareOutlineColor,
 COLORREF pieceDetailColor, BOOL isWhitePiece, BOOL isMono, HBITMAP pieces[3]) {
    ...
    bufferBitmap = CreateCompatibleBitmap(hdcScreen, rect.right - rect.left + 1, rect.bottom - rect.top + 1);
    ...
    if (isMono) {
        ...
        BitBlt(hdcMem, x, y, SAMPLE_SQ_SIZE, SAMPLE_SQ_SIZE, hdcTemp, 0, 0, isWhitePiece ? SRCCOPY : NOTSRCCOPY);
        ...
        if (isWhitePiece) {
            ...
            BitBlt(hdcMem, x, y, SAMPLE_SQ_SIZE, SAMPLE_SQ_SIZE, hdcTemp, 0, 0, 0x00b8074a);
            /* Use black for outline of white pieces */
            SelectObject(hdcTemp, pieces[OUTLINE]);
            BitBlt(hdcMem, x, y, SAMPLE_SQ_SIZE, SAMPLE_SQ_SIZE, hdcTemp, 0, 0, SRCAND);
        } else {
    ...
    BitBlt(hdcScreen, rect.left, rect.top, rect.right - rect.left, rect.bottom - rect.top, hdcMem, rect.left, rect.top, SRCCOPY);
...
BoardSize BoardOptionsWhichRadio(HWND hDlg) {
    return (IsDlgButtonChecked(hDlg, OPT_SizeTiny)
      ? SizeTiny
      : (IsDlgButtonChecked(hDlg, OPT_SizeTeeny)
          ? SizeTeeny
          : (IsDlgButtonChecked(hDlg, OPT_SizeDinky)
              ? SizeDinky
              : (IsDlgButtonChecked(hDlg, OPT_SizePetite)
                  ? SizePetite
                  : (IsDlgButtonChecked(hDlg, OPT_SizeSlim)
                      ? SizeSlim
                      : (IsDlgButtonChecked(hDlg, OPT_SizeSmall)
                          ? SizeSmall
                          : (IsDlgButtonChecked(hDlg, OPT_SizeMediocre)
                              ? SizeMediocre
                              : (IsDlgButtonChecked(hDlg, OPT_SizeMiddling)
                                  ? SizeMiddling
                                  : (IsDlgButtonChecked(hDlg, OPT_SizeAverage)
                                      ? SizeAverage
                                      : (IsDlgButtonChecked(hDlg, OPT_SizeModerate)
                                          ? SizeModerate
                                          : (IsDlgButtonChecked(hDlg, OPT_SizeMedium)
                                              ? SizeMedium
                                              : (IsDlgButtonChecked(hDlg, OPT_SizeBulky)
                                                  ? SizeBulky
                                                  : (IsDlgButtonChecked(hDlg, OPT_SizeLarge)
                                                      ? SizeLarge
                                                      : (IsDlgButtonChecked(hDlg, OPT_SizeBig)
                                                          ? SizeBig
                                                          : (IsDlgButtonChecked(hDlg, OPT_SizeHuge)
                                                              ? SizeHuge
                                                              : (IsDlgButtonChecked(hDlg, OPT_SizeGiant)
                                                                  ? SizeGiant
                                                                  : (IsDlgButtonChecked(hDlg, OPT_SizeColossal)
                                                                      ? SizeColossal
                                                                      : SizeTitanic)))))))))))))))));
}

LRESULT CALLBACK BoardOptionsDialog(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam) {
    ...
    case WM_PAINT:
        ...
        PaintSampleSquare(hDlg, OPT_SampleLightSquare, lsc, wpc, hsc, bpc, TRUE, mono, pieces);
        PaintSampleSquare(hDlg, OPT_SampleDarkSquare, dsc, bpc, phc, wpc, FALSE, mono, pieces);
    ...
    case WM_COMMAND: /* message: received a command */
        ...
        case IDOK:
            ...
            if (bitmaps && !appData.useBitmaps) {
                InitTextures();
            }

            if ((mono != appData.monoMode) || (lsc != lightSquareColor) || (dsc != darkSquareColor) ||
             (wpc != fonts ? appData.fontBackColorWhite : whitePieceColor) ||
             (bpc != fonts ? appData.fontForeColorBlack : blackPieceColor) || (hsc != highlightSquareColor) ||
             (flip != appData.upsideDown) || (white != appData.allWhite) || (fonts != appData.useFont) ||
             (bitmaps != appData.useBitmaps) || (grid != appData.overrideLineGap >= 0) || (phc != premoveHighlightColor)) {
                ...
                if (grid != appData.overrideLineGap >= 0) {
                    appData.overrideLineGap = grid - 1;
                }
...
VOID BoardOptionsPopup(HWND hwnd) {
    ...
    DialogBox(hInst, MAKEINTRESOURCE(DLG_BoardOptions), hwnd, (DLGPROC)lpProc);
    FreeProcInstance(lpProc);
}

int radioButton[] = {
 // this is a remnant of the radiobuttons, and should be rewritten
 OPT_VariantNormal,
 -1,  // Loadable
 OPT_VariantWildcastle, OPT_VariantNocastle, OPT_VariantFRC, OPT_VariantBughouse, OPT_VariantCrazyhouse, OPT_VariantLosers,
 OPT_VariantSuicide, OPT_VariantGiveaway, OPT_VariantTwoKings,
 -1,  // Kriegspiel
 OPT_VariantAtomic, OPT_Variant3Check, OPT_VariantShatranj, -1, -1, -1, -1, -1, -1, -1, -1, OPT_VariantShogi,
 -1,  // Chu
 OPT_VariantCourier, OPT_VariantGothic, OPT_VariantCapablanca, OPT_VariantKnightmate, OPT_VariantFairy, OPT_VariantCylinder,
 OPT_VariantFalcon, OPT_VariantCRC, OPT_VariantBerolina, OPT_VariantJanus, OPT_VariantSuper, OPT_VariantGreat,
 -1,  // Twilight,
 OPT_VariantMakruk, OPT_VariantSChess, OPT_VariantGrand,
 OPT_VariantSpartan,  // Spartan
 OPT_VariantXiangqi, OPT_VariantASEAN, OPT_VariantLion, -1, OPT_VariantJanggi, 666,
 -2  // sentinel
};

VariantClass meaning[1000];

VariantClass VariantWhichRadio(HWND hDlg) {
    int i = 0, j;
    *engineVariant = NULLCHAR;
    HWND combo = GetDlgItem(hDlg, OPT_EngineVariant);
    j = SendMessage(combo, CB_GETCURSEL, (WPARAM)0, (LPARAM)0);
    if (j == CB_ERR) {
        return gameInfo.variant;  // nothing selected, keep old
    }
    i = meaning[j];
    if (i == VariantUnknown) {
...
void VariantShowRadio(HWND hDlg) {
...
LRESULT CALLBACK NewVariantDialog(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam) {
    ...
    case WM_COMMAND: /* message: received a command */
        ...
        case IDOK:
            ...
            if (!appData.noChessProgram) {
                ...
                } else if (second.initDone && second.protocolVersion > 1 && StrStr(second.variants, name) == NULL) {
...
VOID NewVariantPopup(HWND hwnd) {
    ...
    DialogBox(hInst, MAKEINTRESOURCE(DLG_NewVariant), hwnd, (DLGPROC)lpProc);
    FreeProcInstance(lpProc);
}
...
VOID UpdateSampleText(HWND hDlg, int id, MyColorizeAttribs * mca) {
    ...
    cf.dwMask = CFM_COLOR | CFM_CHARSET | CFM_BOLD | CFM_ITALIC | CFM_UNDERLINE | CFM_STRIKEOUT | CFM_FACE | CFM_SIZE;
...
LRESULT CALLBACK ColorizeTextDialog(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam) {
    ...
    case WM_INITDIALOG:
        ...
        SendMessage(GetWindow(hDlg, GW_OWNER), WM_COMMAND, (WPARAM)WM_USER_GetConsoleBackground, (LPARAM)&background);
    ...
    case WM_COMMAND: /* message: received a command */
        ...
        case IDOK:
            ...
            if (cc == ColorNormal) {
                ...
                cf.dwMask = CFM_COLOR;
                cf.crTextColor = mca.color;
                SendDlgItemMessage(hwndConsole, OPT_ConsoleInput, EM_SETCHARFORMAT, SCF_ALL, (LPARAM)&cf);
            }
            EndDialog(hDlg, TRUE);
            return TRUE;
        ...
        default:
            mca.effects = (IsDlgButtonChecked(hDlg, OPT_Bold) ? CFE_BOLD : 0) |
...
VOID ColorizeTextPopup(HWND hwnd, ColorClass cc) {
    ...
    DialogBoxParam(hInst, MAKEINTRESOURCE(DLG_Colorize), hwnd, (DLGPROC)lpProc, (LPARAM)cc);
    FreeProcInstance(lpProc);
}
...
LRESULT CALLBACK IcsOptionsDialog(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam) {
    char buf[MSG_SIZ];
    int number;
    int i;
    static COLORREF cbc;
    ...
    case WM_COMMAND: /* message: received a command */
        ...
        case IDOK:
            ...
            if (sscanf(buf, "%d", &number) != 1 || (number < 0)) {
                MessageBox(hDlg, _("Invalid ICS Alarm Time"), _("Option Error"), MB_OK | MB_ICONEXCLAMATION);
                return FALSE;
            }
...
#undef IS_CHECKED
            ...
            appData.chatBoxes = StrSave(buf);  // memory leak
            ...
            appData.colorize = (Boolean)!IsDlgButtonChecked(hDlg, OPT_DontColorize);
            ...
            if (!appData.colorize) {
                ...
                */
                ...
                SendDlgItemMessage(hwndConsole, OPT_ConsoleInput, EM_SETCHARFORMAT, SCF_ALL, (LPARAM)&cf);
                SendDlgItemMessage(hwndConsole, OPT_ConsoleText, EM_SETBKGNDCOLOR, FALSE, background);
                SendDlgItemMessage(hwndConsole, OPT_ConsoleInput, EM_SETBKGNDCOLOR, FALSE, background);
            }

            if (cbc != consoleBackgroundColor) {
                consoleBackgroundColor = cbc;
                if (appData.colorize) {
                    SendDlgItemMessage(hwndConsole, OPT_ConsoleText, EM_SETBKGNDCOLOR, FALSE, consoleBackgroundColor);
                    SendDlgItemMessage(hwndConsole, OPT_ConsoleInput, EM_SETBKGNDCOLOR, FALSE, consoleBackgroundColor);
                }
            }
        ...
        case OPT_ChooseChannel1Color:
            ...
            UpdateSampleText(hDlg, OPT_SampleChannel1, &colorizeAttribs[ColorChannel1]);
            break;
        ...
        case OPT_DefaultColors:
            for (i = 0; i < NColorClasses - 1; i++) {
                ParseAttribs(&mca[i].color, &mca[i].effects, defaultTextAttribs[i]);
            }
...
VOID IcsOptionsPopup(HWND hwnd) {
    ...
    DialogBox(hInst, MAKEINTRESOURCE(DLG_IcsOptions), hwnd, (DLGPROC)lpProc);
    FreeProcInstance(lpProc);
}
...
VOID SetSampleFontText(HWND hwnd, int id, const MyFont * mf) {
    ...
    len = snprintf(buf, MSG_SIZ, "%.0f pt. %s%s%s\n", mf->mfp.pointSize, mf->mfp.faceName, mf->mfp.bold ? " bold" : "",
...
LRESULT CALLBACK FontOptionsDialog(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam) {
    ...
    case WM_COMMAND: /* message: received a command */
        ...
        case IDOK:
            ...
            if (commentDialog) {
                SendDlgItemMessage(
                 commentDialog, OPT_CommentText, WM_SETFONT, (WPARAM)font[boardSize][COMMENT_FONT]->hf, MAKELPARAM(TRUE, 0));
            ...
            if (editTagsDialog) {
                SendDlgItemMessage(
                 editTagsDialog, OPT_TagsText, WM_SETFONT, (WPARAM)font[boardSize][EDITTAGS_FONT]->hf, MAKELPARAM(TRUE, 0));
            ...
            if (engineOutputDialog != NULL) {
                SendDlgItemMessage(engineOutputDialog, IDC_EngineMemo1, WM_SETFONT, (WPARAM)font[boardSize][MOVEHISTORY_FONT]->hf,
                 MAKELPARAM(TRUE, 0));
                SendDlgItemMessage(engineOutputDialog, IDC_EngineMemo2, WM_SETFONT, (WPARAM)font[boardSize][MOVEHISTORY_FONT]->hf,
                 MAKELPARAM(TRUE, 0));
            }
...
VOID FontsOptionsPopup(HWND hwnd) {
    ...
    DialogBox(hInst, MAKEINTRESOURCE(DLG_Fonts), hwnd, (DLGPROC)lpProc);
    FreeProcInstance(lpProc);
}
...
int SoundDialogWhichRadio(HWND hDlg) {
    if (IsDlgButtonChecked(hDlg, OPT_NoSound)) {
        return OPT_NoSound;
    }
    if (IsDlgButtonChecked(hDlg, OPT_DefaultBeep)) {
        return OPT_DefaultBeep;
    }
    if (IsDlgButtonChecked(hDlg, OPT_BuiltInSound)) {
        return OPT_BuiltInSound;
    }
    if (IsDlgButtonChecked(hDlg, OPT_WavFile)) {
        return OPT_WavFile;
    }
    return -1;
}
...
VOID SoundDialogSetEnables(HWND hDlg, int radio) {
    EnableWindow(GetDlgItem(hDlg, OPT_BuiltInSoundName), radio == OPT_BuiltInSound);
...
void DisplaySelectedSound(HWND hDlg, HWND hCombo, const char * name) {
    ...
    case '!':
        if (name[1] == NULLCHAR) {
            ...
            if (SendMessage(hCombo, CB_SELECTSTRING, (WPARAM)-1, (LPARAM)(name + 1)) == CB_ERR) {
                SendMessage(hCombo, CB_SETCURSEL, (WPARAM)-1, (LPARAM)0);
...
LRESULT CALLBACK SoundOptionsDialog(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam) {
    ...
    case WM_COMMAND: /* message: received a command */
        ...
        if (((HWND)lParam == hSoundCombo) && (HIWORD(wParam) == CBN_SELCHANGE)) {
            /*
        ...
        case IDOK:
            ...
            for (cc = (ColorClass)0; cc < NColorClasses - 2; cc++) {
                index = (int)cc + (int)NSoundClasses;
                if (strcmp(soundComboData[index].name, textAttribs[cc].sound.name) != 0) {
        ...
        case OPT_PlaySound:
            ...
            MyLoadSound(&tmp);
            oldMute = mute;
            mute = FALSE;  // [HGM] mute: always sound when user presses play, ignorig mute setting
            MyPlaySound(&tmp);
            mute = oldMute;
            if (tmp.data != NULL) {
        ...
        case OPT_BrowseSound:
            f = OpenFileDialog(hDlg, "rb", NULL, "wav", SOUND_FILT, _("Browse for Sound File"), NULL, NULL, buf);
            if (f != NULL) {
                fclose(f);
...
VOID ParseCommSettings(char * arg, DCB * dcb) {
    ...
    ComboData * cd;
    count = sscanf(arg, "%d%*[, ]%[^, ]%*[, ]%[^, ]%*[, ]%[^, ]%*[, ]%[^, ]", &dataRate, bits, parity, stopBits, flow);
    if (count != 5) {
        goto cant_parse;
    }
...
VOID PrintCommSettings(FILE * f, char * name, DCB * dcb) {
    ...
    fprintf(f, "/%s=%d,%d,%s,%s,%s\n", name, (int)dcb->BaudRate, dcb->ByteSize, parity, stopBits, flow);
}
...
LRESULT CALLBACK CommPortOptionsDialog(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam) {
    char buf[MSG_SIZ];
    HANDLE hwndCombo;
    char * p;
    LRESULT index;
    ...
    case WM_INITDIALOG: /* message: initialize dialog box */
        ...
        if (p++ == NULL) {
            p = appData.icsCommPort;
        }
        if ((*p == '\0') || (SendMessage(hwndCombo, CB_SELECTSTRING, (WPARAM)-1, (LPARAM)p) == CB_ERR)) {
    ...
    case WM_COMMAND: /* message: received a command */
        ...
        case IDOK:
            ...
            if (sscanf(buf, "%u", &value) != 1) {
                MessageBox(hDlg, _("Invalid data rate"), _("Option Error"), MB_OK | MB_ICONEXCLAMATION);
                return TRUE;
            }
            dcb.BaudRate = value;
            ...
            if (!SetCommState(hCommPort, (LPDCB)&dcb)) {
                err = GetLastError();
                switch (MessageBox(hDlg, "Failed to set comm port state;\r\ninvalid options?", _("Option Error"),
...
int LoadOptionsWhichRadio(HWND hDlg) {
    return (IsDlgButtonChecked(hDlg, OPT_Exact)
      ? 1
      : (IsDlgButtonChecked(hDlg, OPT_Subset)
          ? 2
          : (IsDlgButtonChecked(hDlg, OPT_Struct)
              ? 3
              : (IsDlgButtonChecked(hDlg, OPT_Material)
                  ? 4
                  : (IsDlgButtonChecked(hDlg, OPT_Range) ? 5 : (IsDlgButtonChecked(hDlg, OPT_Difference) ? 6 : -1))))));
}
...
LRESULT CALLBACK LoadOptions(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam) {
    char buf[MSG_SIZ];
    float fnumber;
    int ok;
        ...
        case IDOK:
            ...
            if (IsDlgButtonChecked(hDlg, OPT_Autostep)) {
                ...
                if (sscanf(buf, "%f", &fnumber) != 1) {
                    MessageBox(hDlg, _("Invalid load game step rate"), _("Option Error"), MB_OK | MB_ICONEXCLAMATION);
                    return FALSE;
                }
...
VOID SetSaveOptionEnables(HWND hDlg) {
    ...
    if (state && !IsDlgButtonChecked(hDlg, OPT_AVPrompt) && !IsDlgButtonChecked(hDlg, OPT_AVToFile)) {
...
LRESULT CALLBACK SaveOptionsDialog(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam) {
    char buf[MSG_SIZ];
    FILE * f;
    ...
    case WM_COMMAND: /* message: received a command */
        ...
        case OPT_AVBrowse:
            f = OpenFileDialog(
             hDlg, "a", NULL, appData.oldSaveStyle ? "gam" : "pgn", GAME_FILT, _("Browse for Auto Save File"), NULL, NULL, buf);
            if (f != NULL) {
                fclose(f);
...
VOID SetTimeControlEnables(HWND hDlg) {
    ...
    state = IsDlgButtonChecked(hDlg, OPT_TCUseMoves) + 2 * IsDlgButtonChecked(hDlg, OPT_TCUseFixed);
...
LRESULT CALLBACK TimeControl(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam) {
    char buf[MSG_SIZ], *tc;
    ...
    case WM_INITDIALOG: /* message: initialize dialog box */
        ...
        /* Initialize the dialog items */
        if (/*appData.clockMode &&*/ !appData
          .icsActive) {  // [HGM] even if !clockMode, we could want to set it in tournament dialog
            if (searchTime) {
                CheckRadioButton(hDlg, OPT_TCUseMoves, OPT_TCUseFixed, OPT_TCUseFixed);
                SetDlgItemInt(hDlg, OPT_TCFixed, searchTime, FALSE);
            } else if (appData.timeIncrement == -1) {
                CheckRadioButton(hDlg, OPT_TCUseMoves, OPT_TCUseFixed, OPT_TCUseMoves);
                SetDlgItemText(hDlg, OPT_TCTime, appData.timeControl);
                SetDlgItemInt(hDlg, OPT_TCMoves, appData.movesPerSession, FALSE);
                ...
                if (i == appData.timeIncrement) {
                    ...
                    snprintf(buf, MSG_SIZ, "%4.2f", appData.timeIncrement);
                }
                CheckRadioButton(hDlg, OPT_TCUseMoves, OPT_TCUseFixed, OPT_TCUseInc);
    ...
    case WM_COMMAND: /* message: received a command */
        ...
        case IDOK:
            ...
            if (IsDlgButtonChecked(hDlg, OPT_TCUseFixed)) {
                ...
                if (!ok || st <= 0) {
                    MessageBox(hDlg, _("Invalid max time per move"), _("Option Error"), MB_OK | MB_ICONEXCLAMATION);
                    return FALSE;
                }
            } else if (IsDlgButtonChecked(hDlg, OPT_TCUseMoves)) {
                increment = -1;
                mps = GetDlgItemInt(hDlg, OPT_TCMoves, &ok, FALSE);
                if (!ok || mps <= 0) {
                    MessageBox(hDlg, _("Invalid moves per time control"), _("Option Error"), MB_OK | MB_ICONEXCLAMATION);
                    return FALSE;
                }
                GetDlgItemText(hDlg, OPT_TCTime, buf, MSG_SIZ);
                if (!ParseTimeControl(buf, increment, mps)) {
                    MessageBox(hDlg, _("Invalid minutes per time control"), _("Option Error"), MB_OK | MB_ICONEXCLAMATION);
                    return FALSE;
                }
                tc = buf;
            } else {
                ...
                if (!ok || increment < 0) {
                    MessageBox(hDlg, _("Invalid increment"), _("Option Error"), MB_OK | MB_ICONEXCLAMATION);
                    return FALSE;
                }
                GetDlgItemText(hDlg, OPT_TCTime2, buf, MSG_SIZ);
                if (!ParseTimeControl(buf, increment, mps)) {
                    MessageBox(hDlg, _("Invalid initial time"), _("Option Error"), MB_OK | MB_ICONEXCLAMATION);
                    return FALSE;
                }
                tc = buf;
            }
            ...
            if (!ok || !ok2 || odds1 <= 0 || odds2 <= 0) {
                MessageBox(hDlg, _("Invalid time-odds factor"), _("Option Error"), MB_OK | MB_ICONEXCLAMATION);
                return FALSE;
            }
            searchTime = st;
...
LRESULT CALLBACK UciOptionsDialog(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam) {
    ...
    case WM_COMMAND: /* message: received a command */
        ...
        case IDC_BrowseForBook: {
            char filter[] = {'A', 'l', 'l', ' ', 'F', 'i', 'l', 'e', 's', 0, '*', '.', '*', 0, 'B', 'I', 'N', ' ', 'F', 'i', 'l',
             'e', 's', 0, '*', '.', 'b', 'i', 'n', 0, 0};
