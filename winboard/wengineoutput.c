...
// front end
static HICON LoadIconEx(int id) { return LoadImage(hInst, MAKEINTRESOURCE(id), IMAGE_ICON, ICON_SIZE, ICON_SIZE, 0); }
...
static void PositionControlSet(HWND hDlg, int x, int y, int clientWidth, int memoHeight, int idColor, int idEngineLabel, int idNPS,
 int idMemo, int idStateIcon, int idStateData) {
...
void ResizeWindowControls(int mode) {
    ...
    if (mode == 0) {
        /* One engine */
        PositionControlSet(hDlg, H_MARGIN, V_MARGIN, clientWidth,
         clientHeight - V_MARGIN - LABEL_V_DISTANCE - headerHeight - V_MARGIN, IDC_Color1, IDC_EngineLabel1, IDC_Engine1_NPS,
         IDC_EngineMemo1, IDC_StateIcon1, IDC_StateData1);
        ...
        PositionControlSet(hDlg, H_MARGIN, header1_y, clientWidth, memo_h, IDC_Color1, IDC_EngineLabel1, IDC_Engine1_NPS,
         IDC_EngineMemo1, IDC_StateIcon1, IDC_StateData1);

        PositionControlSet(hDlg, H_MARGIN, header2_y, clientWidth, memo_h, IDC_Color2, IDC_EngineLabel2, IDC_Engine2_NPS,
         IDC_EngineMemo2, IDC_StateIcon2, IDC_StateData2);
    }
...
VOID GetMemoLine(HWND hDlg, int x, int y) {  // [HGM] pv: translate click to PV line, and load it for display
    char buf[10000];
...
void DoSetWindowText(int which, int field, char * s_label) { SetWindowText(outputField[which][field], s_label); }

void SetEngineOutputTitle(char * title) { SetWindowText(engineOutputDialog, title); }
...
LRESULT CALLBACK EngineOutputProc(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam) {
    ...
    case WM_INITDIALOG:
        if (engineOutputDialog == NULL) {
            ...
            /* Set font */
            SendDlgItemMessage(
             engineOutputDialog, IDC_EngineMemo1, WM_SETFONT, (WPARAM)font[boardSize][MOVEHISTORY_FONT]->hf, MAKELPARAM(TRUE, 0));
            SendDlgItemMessage(
             engineOutputDialog, IDC_EngineMemo2, WM_SETFONT, (WPARAM)font[boardSize][MOVEHISTORY_FONT]->hf, MAKELPARAM(TRUE, 0));
...
void DoClearMemo(int which) { SendMessage(outputField[which][nMemo], WM_SETTEXT, 0, (LPARAM) ""); }

// front end (because only other front-end wants to know)
int EngineOutputIsUp() { return engineOutputDialogUp; }

// front end, to give back-end access to engineOutputDialog
int EngineOutputDialogExists() { return engineOutputDialog != NULL; }
