...
void PrintOpt(int i, int right, Option * optionList) {
    if (i < 0) {
        if (!right) {
            fprintf(debugFP, "%30s", "");
        }
...
void CreateOptionDialogTest(int * list, int nr, Option * optionList) {
    int line;
...
void LayoutOptions(int firstOption, int endOption, char * groupName, Option * optionList) {
    ...
    while (nextOption < endOption) {
        ...
        while (nextOption < endOption && !stop) {
            ...
            case CheckBox:
                checkList[checks++] = nextOption;
                lastType = CheckBox;
                break;
            case ComboBox:
                comboList[combos++] = nextOption;
                lastType = ComboBox;
                break;
            case ResetButton:
            case SaveButton:
            case Button:
                buttonList[buttons++] = nextOption;
                lastType = Button;
                break;
            case TextBox:
            case ListBox:
        ...
        if ((combos + checks || nextType == TextBox || nextType == ListBox || nextType == FileName || nextType == PathName ||
             nextType == Label) &&
         layout & 1) {
            layoutList[layout++] = -1;
        }
        ...
        if (nextType == Spin && lastType != Button) {
            if (lastType == CheckBox) {
                prefix = checkList[--checks];
            } else if (lastType == ComboBox) {
                prefix = comboList[--combos];
            }
        }
        ...
        } else if (nextType == TextBox || nextType == FileName || nextType == PathName || nextType == Label) {
...
void DesignOptionDialog(int nrOpt, Option * optionList) {
    int k = 0, n = 0;
    char buf[MSG_SIZ];
    ...
    while (k < nrOpt) {  // k steps through 'solitary' options
        ...
        if (optionList[k].type == GroupBox) {
            groupSize = optionList[k].max, groupNameLength = -strlen(optionList[k].name), groupNameList[groups + 1] = k;
        } else {
            while (k + groupSize < nrOpt && strstr(optionList[k + groupSize].name, buf) == optionList[k + groupSize].name) {
                int j;
                for (j = 0; j < groupNameLength; j++) {  // determine common initial part of option names
                    if (optionList[k].name[j] != optionList[k + groupSize].name[j]) {
                        break;
                    }
                }
                groupNameLength = j;
        ...
        if (groupSize > 3) {
            ...
            k += groupSize;  // small groups are grouped with the solitary options
        }
    }
    if (n != k) {
        LayoutOptions(n, k, "", optionList);  // flush remaining solitary options
    }
    ...
    if (appData.debugMode) {
        CreateOptionDialogTest(layoutList, layout, optionList);
    }
    // CreateOptionDialog(layoutList, layout, optionList);
    if (!activeCps) {
        okFunc = optionList[nrOpt].target;
    }
}
...
struct {
 ...
 0x0000, 0x0000, L"Engine #1 Settings ", 8,
 L"MS Sans Serif"
};
...
void SetOptionValues(HWND hDlg, ChessProgramState * cps, Option * optionList)
// Put all current option values in controls, and write option names next to them
{
    ...
    for (i = 0; i < layout + buttons; i++) {
        int j = layoutList[i];
        if (j == -2) {
            SetDlgItemText(hDlg, 2000 + 2 * i, ". . .");
        }
        if (j < 0) {
            continue;
        }
        name = cps ? optionList[j].name : _(optionList[j].name);
        if (strstr(name, "Polyglot ") == name) {
            name += 9;
        }
    ...
    for (i = 0; i < groups; i += 2) {
        ...
        for (p = 0; p < groupNameList[i]; p++) {
            buf[p] = optionList[layoutList[k]].name[p];
        }
        buf[p] = 0;
        if (groupNameList[i] < 0) {
            safeStrCpy(buf, optionList[groupNameList[i + 1]].name, MSG_SIZ);
        }
...
// read out all controls, and if value is altered, remember it and send it to the engine
{
    ...
    for (i = 0; i < layout; i++) {
        ...
        case Spin:
            ...
            if (new < optionList[j].min) {
                new = optionList[j].min;
            }
            if (new > optionList[j].max) {
                new = optionList[j].max;
            }
            if (!cps) {
                *(int *)optionList[j].target = new;
                break;
            }
        ...
        case PathName:
            if (cps) {
                len = MSG_SIZ - strlen(optionList[j].name) - 9, text = newText;
            } else {
                len = GetWindowTextLength(GetDlgItem(hDlg, 2001 + 2 * i)) + 1, text = (char *)malloc(len);
            }
        ...
        case CheckBox:
            ...
            if (!cps) {
                *(Boolean *)optionList[j].target = new;
                break;
            }
            changed = 2 * (optionList[j].value != new);
            optionList[j].value = new;
        ...
        case ComboBox:
            ...
            new = -1;
            for (k = 0; k < optionList[j].max; k++) {
                if (choices[k] && !strcmp(choices[k], newText)) {
                    new = k;
                }
            }
            if (!cps && new >= 0) {
                if (*(char **)optionList[j].target) {
                    free(*(char **)optionList[j].target);
                }
    ...
    if (!cps && okFunc) {
        return ((ButtonCallback *)okFunc)(0);
    }
    return 1;
}
...
LRESULT CALLBACK SettingsProc(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam) {
    ...
    case WM_COMMAND:
        ...
        case IDOK:
            if (!GetOptionValues(hDlg, activeCps, activeList)) {
                return FALSE;
            }
            EndDialog(hDlg, 0);
            comboCallback = NULL;
        ...
        default:
            ...
            if (i >= 2000 && i < 2000 + 2 * (layout + buttons)) {
                ...
                if (j == -2) {
                    char filter[] = "All files\0*.*\0Game files\0*.pgn;*.gam\0Position files\0*.fen;*.epd;*.pos\0"
                    ...
                    if (activeList[layoutList[(i - 2000) / 2 + 1]].max & 32 ? GetOpenFileName(&ofn) : GetSaveFileName(&ofn)) {
                ...
                if (comboCallback && activeList[j].type == ComboBox && HIWORD(wParam) == CBN_SELCHANGE) {
                    if (j > 5) {
                        break;  // Yegh! Must solve problem with more than one combobox in dialog
                    }
                    (*comboCallback)(hDlg);
                    break;
                } else if (activeList[j].type == ListBox && HIWORD(wParam) == /*LBN_SELCHANGE*/ LBN_DBLCLK) {
                    ((ButtonCallback *)activeList[j].target)(hDlg);
                    break;
                } else if (activeList[j].type == SaveButton) {
                    GetOptionValues(hDlg, activeCps, activeList);
                } else if (activeList[j].type != Button) {
                    break;
                } else if (!activeCps) {
                    (*(ButtonCallback *)activeList[j].target)(hDlg);
                    break;
...
void AddOption(int x, int y, Control type, int i) {
    ...
    case TextBox:
        ...
        AddControl(x + (extra ? 50 : 95), y + (extra ? 13 : 0), extra ? 105 : 200, 11 + (extra ? extra - 13 : 0), 0x0081,
         ES_AUTOHSCROLL | WS_BORDER | WS_VISIBLE | WS_CHILD | WS_TABSTOP | (extra ? ES_MULTILINE | ES_WANTRETURN | WS_VSCROLL : 0),
         i + 1);
        break;
    case ListBox:
        AddControl(x, y + 1, 95, 9, 0x0082, SS_ENDELLIPSIS | WS_VISIBLE | WS_CHILD, i);
        extra = 13 * activeList[layoutList[i / 2]].min;
        AddControl(x, y + 13, 105, 11 + extra - 13, 0x0083,
         LBS_NOINTEGRALHEIGHT | WS_VSCROLL | WS_HSCROLL | WS_BORDER | LBS_NOTIFY | WS_VISIBLE | WS_CHILD | WS_TABSTOP, i + 1);
        break;
    case Label:
...
void CreateDialogTemplate(int * layoutList, int nr, Option * optionList) {
    ...
    for (ii = 0; ii < nr; ii++) {
        i = ii ^ 1;
        if (i == nr) {
            i = ii;  // if two on one line, swap order of treatment, to get good (left to right) tabbing order.
        }
        if (k < groups && ii == boxList[k]) {
            int tlen = groupNameList[k];
            if (tlen < 0) {
                tlen = -tlen;
            }
            y += 10;
            AddControl(x + 2, y + 13 * (i >> 1) - 2, 301, 13 * (boxList[k + 1] - boxList[k] >> 1) + 8, 0x0082,
             WS_VISIBLE | WS_CHILD | SS_BLACKFRAME, 2400);
            AddControl(x + 60, y + 13 * (i >> 1) - 6, 10 * tlen / 3, 10, 0x0082, SS_ENDELLIPSIS | WS_VISIBLE | WS_CHILD,
             2 * (ii + MAX_OPTIONS));
        }
        j = layoutList[i];
        if (j >= 0) {
            ...
            if (optionList[j].type == ListBox) {
                x -= optionList[j].value, template.header.cx -= optionList[j].value;
            }
        }
        ...
        if (ii + 1 >= breakPoint && breaks[ii + 1 >> 1] >= 0) {
            x += 318;
            maxY = y + 13 * (ii + 1 >> 1) + 5;
            y = -13 * (ii + 1 >> 1);
            breakPoint = 1000;
    ...
    for (i = 0; i < buttons; i++) {
        AddControl(
         x + 70 * (i % 4) + 5, y + 18 * (i / 4), 65, 15, 0x0080, BS_PUSHBUTTON | WS_VISIBLE | WS_CHILD | WS_TABSTOP, 2 * (nr + i));
    ...
    AddControl(
     x + 260, y + 18 * (buttonRows - 1), 40, 15, 0x0080, BS_PUSHBUTTON | WS_VISIBLE | WS_CHILD | WS_TABSTOP, IDCANCEL - 2000);
    y += 18 * buttonRows;
    if (y < maxY) {
        y = maxY;
    }
...
void EngineOptionsPopup(HWND hwnd, ChessProgramState * cps) {
    FARPROC lpProc = MakeProcInstance((FARPROC)SettingsProc, hInst);

    activeCps = cps;
    activeList = cps->option;
...
void RefreshSettingsDialog(ChessProgramState * cps, int val) {
    ...
    if (val == 1) {
        if (activeCps == cps && isUp) {
            SetOptionValues(settingsDlg, cps, activeList);
        }
        return;
    }
    if (settingsDlg) {
        EndDialog(settingsDlg, 1);
    }
    comboCallback = NULL;
    activeCps = NULL;
    settingsDlg = NULL;
    if (val == 3 || isUp) {
        EngineOptionsPopup(hwndMain, cps);
    }
}
...
int InstallOK() {
    ...
    if (!strcmp(protocolChoice, protocols[0])) {
        isUCI = 3;
    } else if (!strcmp(protocolChoice, protocols[2])) {
        isUCI = 1;
    } else if (!strcmp(protocolChoice, protocols[3])) {
        isUCCI = 1;
    } else if (!strcmp(protocolChoice, protocols[4])) {
        v1 = 1;
    }
    if (isUCCI) {
        isUCI = 2;
...
void GenericPopup(HWND hwnd, Option * optionList) {
    FARPROC lpProc = MakeProcInstance((FARPROC)SettingsProc, hInst);
    int n = 0;

    while (optionList[n].type != EndMark) {
        n++;
    }
    activeCps = NULL;
...
void DeleteTheme(HWND hDlg) {
    ...
    themeOptions[0].max =
     NamesToList(appData.themeNames, engineList, engineMnemonic, "");  // replace list by only the group contents
    SendMessage(hwndCombo, LB_RESETCONTENT, 0, 0);
...
int MatchOK() {
    ...
    return matchMode ||
     !appData.participants[0];  // if we failed to create and are not in playing, forbid popdown if there are participants
}
...
char * GetParticipants(HWND hDlg) {
    ...
    if (len < 4) {
        return NULL;  // box is empty (enough)
    }
