...
int GameListToListBox(HWND hDlg, BOOL boReset, char * pszFilter, struct GameListStats * stats, BOOL byPos, BOOL narrow) {
    ...
    if (!hDlg) {
        hDlg = gameListDialog;  // [HGM] to allow calling from Game List Options dialog
    }
    if (!hDlg) {
        return 0;
    }
    ...
    for (nItem = 0; nItem < ((ListGame *)gameList.tailPred)->number; nItem++) {
        ...
        if (!narrow || lg->position >= 0) {
            ...
            if (!skip && byPos) {
                if ((pos = GameContainsPosition(gameFile, lg)) < 0) {
                    skip = TRUE;
                }
            }
...
LRESULT CALLBACK GameListDialog(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam) {
    ...
    case WM_INITDIALOG:
        ...
        if (!gameListDialog) {
            ...
            sizeX = rect.right;
            sizeY = rect.bottom;
            if (wpGameList.x != CW_USEDEFAULT && wpGameList.y != CW_USEDEFAULT && wpGameList.width != CW_USEDEFAULT &&
             wpGameList.height != CW_USEDEFAULT) {
                WINDOWPLACEMENT wp;
        ...
        GameListUpdateTitle(
         hDlg, _("Game List"), count, ((ListGame *)gameList.tailPred)->number, &stats);  // [HGM] always update title
    ...
    case WM_SIZE:
        ...
        ResizeEditPlusButtons(hDlg, GetDlgItem(hDlg, OPT_GameListText), sizeX, sizeY, newSizeX, newSizeY);
        sizeX = newSizeX;
        sizeY = newSizeY;
        break;
      ...
      break; /* load the game*/
        ...
        case IDC_GameListDoFilter: {
            ...
            if (GetDlgItemText(hDlg, IDC_GameListFilter, filter, sizeof(filter)) >= 0) {
                ...
                count = GameListToListBox(
                 hDlg, TRUE, filter, &stats, LOWORD(wParam) != IDC_GameListDoFilter, LOWORD(wParam) == OPT_GameListNarrow);
...
VOID GameListPopUp(FILE * fp, char * filename) {
    ...
    if (gameListDialog) {
        ...
        if (!gameListUp) {
            ShowWindow(gameListDialog, SW_SHOW);
        } else {
            SetFocus(gameListDialog);
        }
    } else {
        lpProc = MakeProcInstance((FARPROC)GameListDialog, hInst);
        CreateDialog(hInst, MAKEINTRESOURCE(DLG_GameList), hwndMain, (DLGPROC)lpProc);
        FreeProcInstance(lpProc);
    }
    gameListUp = TRUE;
}

FILE * GameFile() { return gameFile; }
...
VOID GameListHighlight(int index) {
    ...
    while (i - j > 1) {
        ...
        if (res == LB_ERR || index < (k = atoi(buf))) {
            i = n;
        } else {
            j = n;
            if (index == k) {
