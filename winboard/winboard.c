...
static HighlightInfo premoveHighlightInfo = {
 {{-1, -1}, {-1, -1}}
};
static HighlightInfo partnerHighlightInfo = {
 {{-1, -1}, {-1, -1}}
};
static HighlightInfo oldPartnerHighlight = {
 {{-1, -1}, {-1, -1}}
};
...
RECT boardRect;
COLORREF lightSquareColor, darkSquareColor, whitePieceColor, blackPieceColor, highlightSquareColor, premoveHighlightColor;
...
static HBRUSH lightSquareBrush, darkSquareBrush, blackSquareBrush, /* [HGM] for band between board and holdings */
...
static struct {
    ...
    int y;
    int mode;
} backTextureSquareInfo[BOARD_RANKS][BOARD_FILES];
...
int dialogItems[][42] = {
 ...
 {DLG_TimeControl, IDC_Babble, OPT_TCUseMoves, OPT_TCUseInc, OPT_TCUseFixed, OPT_TCtext1, OPT_TCtext2, OPT_TCitext1, OPT_TCitext2,
  OPT_TCftext, GPB_Factors, IDC_Factor1, IDC_Factor2, IDOK, IDCANCEL},
 {DLG_LoadOptions, OPT_Autostep, OPT_AStext1, OPT_Exact, OPT_Subset, OPT_Struct, OPT_Material, OPT_Range, OPT_Difference, OPT_elo1t,
  OPT_elo2t, OPT_datet, OPT_Stretch, OPT_Stretcht, OPT_Reversed, OPT_SearchMode, OPT_Mirror, OPT_thresholds, OPT_Ranget, IDOK,
  IDCANCEL},
 {DLG_SaveOptions, OPT_Autosave, OPT_AVPrompt, OPT_AVToFile, OPT_AVBrowse, 801, OPT_PGN, OPT_Old, OPT_OutOfBookInfo, IDOK,
  IDCANCEL},
 {1536, 1090, IDC_Directories, 1089, 1091, IDOK, IDCANCEL, 1038, IDC_IndexNr, 1037},
 {DLG_CommPort, IDOK, IDCANCEL, IDC_Port, IDC_Rate, IDC_Bits, IDC_Parity, IDC_Stop, IDC_Flow, OPT_SerialHelp},
 {DLG_EditComment, IDOK, OPT_CancelComment, OPT_ClearComment, OPT_EditComment},
 {DLG_PromotionKing, PB_Chancellor, PB_Archbishop, PB_Queen, PB_Rook, PB_Bishop, PB_Knight, PB_King, IDCANCEL, IDC_Yes, IDC_No,
  IDC_Centaur},
 {ABOUTBOX2, IDC_ChessBoard},
 {DLG_GameList, OPT_GameListLoad, OPT_GameListPrev, OPT_GameListNext, OPT_GameListClose, IDC_GameListDoFilter},
 {DLG_EditTags, IDOK, OPT_TagsCancel, OPT_EditTags},
 {DLG_Error, IDOK},
 {DLG_Colorize, IDOK, IDCANCEL, OPT_ChooseColor, OPT_Bold, OPT_Italic, OPT_Underline, OPT_Strikeout, OPT_Sample},
 {DLG_Question, IDOK, IDCANCEL, OPT_QuestionText},
 {DLG_Startup, IDC_Welcome, OPT_ChessEngine, OPT_ChessServer, OPT_View, IDC_SPECIFY_ENG_STATIC, IDC_SPECIFY_SERVER_STATIC,
  OPT_AnyAdditional, IDOK, IDCANCEL, IDM_HELPCONTENTS},
 {DLG_IndexNumber, IDC_Index},
 {DLG_TypeInMove, IDOK, IDCANCEL},
 {DLG_TypeInName, IDOK, IDCANCEL},
 {DLG_Sound, IDC_Event, OPT_NoSound, OPT_DefaultBeep, OPT_BuiltInSound, OPT_WavFile, OPT_BrowseSound, OPT_DefaultSounds, IDOK,
  IDCANCEL, OPT_PlaySound},
 {DLG_GeneralOptions, IDOK, IDCANCEL, OPT_AlwaysOnTop, OPT_HighlightLastMove, OPT_AlwaysQueen, OPT_PeriodicUpdates,
  OPT_AnimateDragging, OPT_PonderNextMove, OPT_AnimateMoving, OPT_PopupExitMessage, OPT_AutoFlag, OPT_PopupMoveErrors,
  OPT_AutoFlipView, OPT_ShowButtonBar, OPT_AutoRaiseBoard, OPT_ShowCoordinates, OPT_Blindfold, OPT_ShowThinking,
  OPT_HighlightDragging, OPT_TestLegality, OPT_SaveExtPGN, OPT_HideThinkFromHuman, OPT_ExtraInfoInMoveHistory,
  OPT_HighlightMoveArrow, OPT_AutoLogo, OPT_SmartMove, OPT_AutoTags, OPT_AutoComment, OPT_Headers, OPT_Variations, OPT_AutoExtend},
 {DLG_IcsOptions, IDOK, IDCANCEL, OPT_AutoComment, OPT_AutoKibitz, OPT_AutoObserve, OPT_GetMoveList, OPT_LocalLineEditing,
  OPT_QuietPlay, OPT_SeekGraph, OPT_AutoRefresh, OPT_BgObserve, OPT_DualBoard, OPT_Premove, OPT_PremoveWhite, OPT_PremoveBlack,
  OPT_SmartMove, OPT_IcsAlarm, IDC_Sec, OPT_ChooseShoutColor, OPT_ChooseSShoutColor, OPT_ChooseChannel1Color,
  OPT_ChooseChannelColor, OPT_ChooseKibitzColor, OPT_ChooseTellColor, OPT_ChooseChallengeColor, OPT_ChooseRequestColor,
  OPT_ChooseSeekColor, OPT_ChooseNormalColor, OPT_ChooseBackgroundColor, OPT_DefaultColors, OPT_DontColorize, IDC_Boxes, GPB_Colors,
  GPB_Premove, GPB_General, GPB_Alarm, OPT_AutoCreate},
 {DLG_BoardOptions, IDOK, IDCANCEL, OPT_SizeTiny, OPT_SizeTeeny, OPT_SizeDinky, OPT_SizePetite, OPT_SizeSlim, OPT_SizeSmall,
  OPT_SizeMediocre, OPT_SizeMiddling, OPT_SizeAverage, OPT_SizeModerate, OPT_SizeMedium, OPT_SizeBulky, OPT_SizeLarge, OPT_SizeBig,
  OPT_SizeHuge, OPT_SizeGiant, OPT_SizeColossal, OPT_SizeTitanic, OPT_ChooseLightSquareColor, OPT_ChooseDarkSquareColor,
  OPT_ChooseWhitePieceColor, OPT_ChooseBlackPieceColor, OPT_ChooseHighlightSquareColor, OPT_ChoosePremoveHighlightColor,
  OPT_Monochrome, OPT_AllWhite, OPT_UpsideDown, OPT_DefaultBoardColors, GPB_Colors, IDC_Light, IDC_Dark, IDC_White, IDC_Black,
  IDC_High, IDC_PreHigh, GPB_Size, OPT_Bitmaps, OPT_PieceFont, OPT_Grid},
 {DLG_NewVariant, IDOK, IDCANCEL, GPB_Variant, GPB_Board, IDC_Height, IDC_Width, IDC_Hand, IDC_Pieces, IDC_Def},
 {DLG_Fonts, IDOK, IDCANCEL, OPT_ChooseClockFont, OPT_ChooseMessageFont, OPT_ChooseCoordFont, OPT_ChooseTagFont,
  OPT_ChooseCommentsFont, OPT_ChooseConsoleFont, OPT_ChooseMoveHistoryFont, OPT_DefaultFonts, OPT_ClockFont, OPT_MessageFont,
  OPT_CoordFont, OPT_EditTagsFont, OPT_ChoosePieceFont, OPT_MessageFont8, OPT_SampleGameListFont, OPT_ChooseGameListFont,
  OPT_MessageFont7, OPT_CommentsFont, OPT_MessageFont5, GPB_Current, GPB_All, OPT_MessageFont6},
 {DLG_NewGameFRC, IDC_NFG_Label, IDC_NFG_Random, IDOK, IDCANCEL},
 {DLG_GameListOptions, IDC_GLT, IDC_GLT_Up, IDC_GLT_Down, IDC_GLT_Restore, IDC_GLT_Default, IDOK, IDCANCEL, IDC_GLT_RestoreTo},
 {DLG_MoveHistory},
 {DLG_EvalGraph},
 {DLG_EngineOutput, IDC_EngineLabel1, IDC_Engine1_NPS, IDC_EngineLabel2, IDC_Engine2_NPS},
 {
  DLG_Chat, IDC_Partner,
  IDC_Clear, IDC_Send,
  },
 {DLG_EnginePlayOptions, IDC_EpPonder, IDC_EpShowThinking, IDC_EpHideThinkingHuman, IDC_EpPeriodicUpdates, GPB_Adjudications,
  IDC_Draw, IDC_Moves, IDC_Threshold, IDC_Centi, IDC_TestClaims, IDC_DetectMates, IDC_MaterialDraws, IDC_TrivialDraws, GPB_Apply,
  IDC_Rule, IDC_Repeats, IDC_ScoreAbs1, IDC_ScoreAbs2, IDOK, IDCANCEL},
 {DLG_OptionsUCI, IDC_PolyDir, IDC_BrowseForPolyglotDir, IDC_Hash, IDC_Path, IDC_BrowseForEGTB, IDC_Cache, IDC_UseBook,
  IDC_BrowseForBook, IDC_CPU, IDC_OwnBook1, IDC_OwnBook2, IDC_Depth, IDC_Variation, IDC_DefGames, IDOK, IDCANCEL},
 {0}
};
...
void LoadLanguageFile(
 char * name) {  // load the file with translations, and make a list of the strings to be translated, and their translations
    FILE * f;
    int i = 0, j = 0, n = 0, k;
    char buf[MSG_SIZ];
    ...
    if (!strcmp(buf, oldLanguage)) {
        barbaric = 1;
        return;
    }  // this language already loaded; just switch on
    if ((f = fopen(buf, "r")) == NULL) {
        return;
    }
    while ((k = fgetc(f)) != EOF) {
        if (i >= sizeof(languageBuf)) {
            DisplayError("Language file too big", 0);
            return;
        }
        languageBuf[i] = k;
        if (k == '\n') {
            if (languageBuf[n] == '"' && languageBuf[i - 1] == '"') {
                ...
                if (p = strstr(languageBuf + n + 1, "\" === \"")) {
                    if (p > languageBuf + n + 1 && p + 8 < languageBuf + i) {
                        if (j >= sizeof(english)) {
                            DisplayError("Too many translated strings", 0);
                            return;
                        }
                        english[j] = languageBuf + n + 1;
                        *p = 0;
                        foreign[j++] = p + 7;
                        languageBuf[i - 1] = 0;
...
char * T_(char * s) {  // return the translation of the given string
...
void Translate(HWND hDlg, int dialogID) {  // translate all text items in the given dialog
    int i = 0, j, k;
    char buf[MSG_SIZ], *s;
    if (!barbaric) {
        return;
    }
    while (dialogItems[i][0] && dialogItems[i][0] != dialogID) {
        i++;  // find the dialog description
    }
    if (dialogItems[i][0] != dialogID) {
        return;  // unknown dialog, should not happen
    }
    GetWindowText(hDlg, buf, MSG_SIZ);
    s = T_(buf);
    if (strcmp(buf, s)) {
        SetWindowText(hDlg, s);  // replace by translated string (if different)
    }
    for (j = 1; k = dialogItems[i][j]; j++) {  // translate all listed dialog items
        ...
        s = T_(buf);
        if (strcmp(buf, s)) {
            SetDlgItemText(hDlg, k, s);  // replace by translated string (if different)
        }
    }
}
...
TranslateOneMenu(int i, HMENU subMenu) {
    ...
    for (j = GetMenuItemCount(subMenu) - 1; j >= 0; j--) {
        ...
        if (i < 10) {
            if (menuText[i][j]) {
                safeStrCpy(buf, menuText[i][j], sizeof(buf) / sizeof(buf[0]));
            } else {
...
void TranslateMenus(int addLanguage) {
    int i;
    char buf[MSG_SIZ];
...
#  define IDM_English 1970
    ...
    if ((hFind = FindFirstFile(buf, &fileData)) != INVALID_HANDLE_VALUE) {
        ...
        AppendMenu(
         subMenu, MF_ENABLED | MF_STRING | (barbaric ? MF_UNCHECKED : MF_CHECKED), (UINT_PTR)IDM_English, (LPCTSTR) "English");
        i = 0;
        lastChecked = IDM_English;
        do {
...
MyFont fontRec[NUM_SIZES][NUM_FONTS] = {
 {MF(CLOCK_FONT_TINY),     MF(MESSAGE_FONT_TINY),     MF(COORD_FONT_TINY),     MF(CONSOLE_FONT_TINY),     MF(COMMENT_FONT_TINY),
  MF(EDITTAGS_FONT_TINY),                                                                                                                                   MF(MOVEHISTORY_FONT_ALL), MF(GAMELIST_FONT_ALL)},
 {MF(CLOCK_FONT_TEENY),    MF(MESSAGE_FONT_TEENY),    MF(COORD_FONT_TEENY),    MF(CONSOLE_FONT_TEENY),    MF(COMMENT_FONT_TEENY),
  MF(EDITTAGS_FONT_TEENY),                                                                                                                                  MF(MOVEHISTORY_FONT_ALL), MF(GAMELIST_FONT_ALL)},
 {MF(CLOCK_FONT_DINKY),    MF(MESSAGE_FONT_DINKY),    MF(COORD_FONT_DINKY),    MF(CONSOLE_FONT_DINKY),    MF(COMMENT_FONT_DINKY),
  MF(EDITTAGS_FONT_DINKY),                                                                                                                                  MF(MOVEHISTORY_FONT_ALL), MF(GAMELIST_FONT_ALL)},
 {MF(CLOCK_FONT_PETITE),   MF(MESSAGE_FONT_PETITE),   MF(COORD_FONT_PETITE),   MF(CONSOLE_FONT_PETITE),   MF(COMMENT_FONT_PETITE),
  MF(EDITTAGS_FONT_PETITE),                                                                                                                                 MF(MOVEHISTORY_FONT_ALL), MF(GAMELIST_FONT_ALL)},
 {MF(CLOCK_FONT_SLIM),     MF(MESSAGE_FONT_SLIM),     MF(COORD_FONT_SLIM),     MF(CONSOLE_FONT_SLIM),     MF(COMMENT_FONT_SLIM),
  MF(EDITTAGS_FONT_SLIM),                                                                                                                                   MF(MOVEHISTORY_FONT_ALL), MF(GAMELIST_FONT_ALL)},
 {MF(CLOCK_FONT_SMALL),    MF(MESSAGE_FONT_SMALL),    MF(COORD_FONT_SMALL),    MF(CONSOLE_FONT_SMALL),    MF(COMMENT_FONT_SMALL),
  MF(EDITTAGS_FONT_SMALL),                                                                                                                                  MF(MOVEHISTORY_FONT_ALL), MF(GAMELIST_FONT_ALL)},
 {MF(CLOCK_FONT_MEDIOCRE), MF(MESSAGE_FONT_MEDIOCRE), MF(COORD_FONT_MEDIOCRE), MF(CONSOLE_FONT_MEDIOCRE), MF(COMMENT_FONT_MEDIOCRE),
  MF(EDITTAGS_FONT_MEDIOCRE),                                                                                                                               MF(MOVEHISTORY_FONT_ALL), MF(GAMELIST_FONT_ALL)},
 {MF(CLOCK_FONT_MIDDLING), MF(MESSAGE_FONT_MIDDLING), MF(COORD_FONT_MIDDLING), MF(CONSOLE_FONT_MIDDLING), MF(COMMENT_FONT_MIDDLING),
  MF(EDITTAGS_FONT_MIDDLING),                                                                                                                               MF(MOVEHISTORY_FONT_ALL), MF(GAMELIST_FONT_ALL)},
 {MF(CLOCK_FONT_AVERAGE),  MF(MESSAGE_FONT_AVERAGE),  MF(COORD_FONT_AVERAGE),  MF(CONSOLE_FONT_AVERAGE),  MF(COMMENT_FONT_AVERAGE),
  MF(EDITTAGS_FONT_AVERAGE),                                                                                                                                MF(MOVEHISTORY_FONT_ALL), MF(GAMELIST_FONT_ALL)},
 {MF(CLOCK_FONT_MODERATE), MF(MESSAGE_FONT_MODERATE), MF(COORD_FONT_MODERATE), MF(CONSOLE_FONT_MODERATE), MF(COMMENT_FONT_MODERATE),
  MF(EDITTAGS_FONT_MODERATE),                                                                                                                               MF(MOVEHISTORY_FONT_ALL), MF(GAMELIST_FONT_ALL)},
 {MF(CLOCK_FONT_MEDIUM),   MF(MESSAGE_FONT_MEDIUM),   MF(COORD_FONT_MEDIUM),   MF(CONSOLE_FONT_MEDIUM),   MF(COMMENT_FONT_MEDIUM),
  MF(EDITTAGS_FONT_MEDIUM),                                                                                                                                 MF(MOVEHISTORY_FONT_ALL), MF(GAMELIST_FONT_ALL)},
 {MF(CLOCK_FONT_BULKY),    MF(MESSAGE_FONT_BULKY),    MF(COORD_FONT_BULKY),    MF(CONSOLE_FONT_BULKY),    MF(COMMENT_FONT_BULKY),
  MF(EDITTAGS_FONT_BULKY),                                                                                                                                  MF(MOVEHISTORY_FONT_ALL), MF(GAMELIST_FONT_ALL)},
 {MF(CLOCK_FONT_LARGE),    MF(MESSAGE_FONT_LARGE),    MF(COORD_FONT_LARGE),    MF(CONSOLE_FONT_LARGE),    MF(COMMENT_FONT_LARGE),
  MF(EDITTAGS_FONT_LARGE),                                                                                                                                  MF(MOVEHISTORY_FONT_ALL), MF(GAMELIST_FONT_ALL)},
 {MF(CLOCK_FONT_BIG),      MF(MESSAGE_FONT_BIG),      MF(COORD_FONT_BIG),      MF(CONSOLE_FONT_BIG),      MF(COMMENT_FONT_BIG),      MF(EDITTAGS_FONT_BIG),
  MF(MOVEHISTORY_FONT_ALL),                                                                                                                                                           MF(GAMELIST_FONT_ALL)},
 {MF(CLOCK_FONT_HUGE),     MF(MESSAGE_FONT_HUGE),     MF(COORD_FONT_HUGE),     MF(CONSOLE_FONT_HUGE),     MF(COMMENT_FONT_HUGE),
  MF(EDITTAGS_FONT_HUGE),                                                                                                                                   MF(MOVEHISTORY_FONT_ALL), MF(GAMELIST_FONT_ALL)},
 {MF(CLOCK_FONT_GIANT),    MF(MESSAGE_FONT_GIANT),    MF(COORD_FONT_GIANT),    MF(CONSOLE_FONT_GIANT),    MF(COMMENT_FONT_GIANT),
  MF(EDITTAGS_FONT_GIANT),                                                                                                                                  MF(MOVEHISTORY_FONT_ALL), MF(GAMELIST_FONT_ALL)},
 {MF(CLOCK_FONT_COLOSSAL), MF(MESSAGE_FONT_COLOSSAL), MF(COORD_FONT_COLOSSAL), MF(CONSOLE_FONT_COLOSSAL), MF(COMMENT_FONT_COLOSSAL),
  MF(EDITTAGS_FONT_COLOSSAL),                                                                                                                               MF(MOVEHISTORY_FONT_ALL), MF(GAMELIST_FONT_ALL)},
 {MF(CLOCK_FONT_TITANIC),  MF(MESSAGE_FONT_TITANIC),  MF(COORD_FONT_TITANIC),  MF(CONSOLE_FONT_TITANIC),  MF(COMMENT_FONT_TITANIC),
  MF(EDITTAGS_FONT_TITANIC),                                                                                                                                MF(MOVEHISTORY_FONT_ALL), MF(GAMELIST_FONT_ALL)},
};
...
VOID ConsoleCreate();
LRESULT CALLBACK ConsoleWndProc(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam);
...
static void HandleMessage(MSG * message) {
    ...
    if (msg.message == WM_CHAR && msg.wParam == '\t') {
        ...
        if (msg.hwnd == hwndMain) {
            currentElement = 7;
        } else if (msg.hwnd == engineOutputDialog) {
            currentElement = 2;
        } else if (msg.hwnd == e1) {
            currentElement = 2;
        } else if (msg.hwnd == e2) {
            currentElement = 3;
        } else if (msg.hwnd == moveHistoryDialog) {
            currentElement = 4;
        } else if (msg.hwnd == mh) {
            currentElement = 4;
        } else if (msg.hwnd == evalGraphDialog) {
            currentElement = 6;
        } else if (msg.hwnd == hText) {
            currentElement = 5;
        } else if (msg.hwnd == hInput) {
            currentElement = 6;
        } else {
            for (i = 0; i < N_BUTTONS; i++) {
                if (buttonDesc[i].hwnd == msg.hwnd) {
                    currentElement = 1;
                    break;
                }
            }
        ...
        if (currentElement) {
            HWND h = NULL;
            int direction = GetKeyState(VK_SHIFT) < 0 ? -1 : 1;
            do {
                currentElement = (currentElement + direction) % 7;
                switch (currentElement) {
                case 0:
                    h = hwndMain;
                    break;  // passing this case always makes the loop exit
                case 1:
                    h = buttonDesc[0].hwnd;
                    break;  // could be NULL
                case 2:
                    if (!EngineOutputIsUp()) {
                        continue;  // skip closed auxiliary windows
                    }
                    h = e1;
                    break;
                case 3:
            ...
            if (currentElement > 4 && IsIconic(hwndConsole)) {
                ShowWindow(hwndConsole, SW_RESTORE);
            }
            if (currentElement < 5 && IsIconic(hwndMain)) {
                ShowWindow(hwndMain, SW_RESTORE);  // all open together
            }
            SetFocus(h);
    ...
    if (!(commentDialog && IsDialogMessage(commentDialog, &msg)) &&
     ...
     !(editTagsDialog && IsDialogMessage(editTagsDialog, &msg)) && !(gameListDialog && IsDialogMessage(gameListDialog, &msg)) &&
     !(errorDialog && IsDialogMessage(errorDialog, &msg)) && !(!frozen && TranslateAccelerator(hwndMain, hAccelMain, &msg)) &&
     JAWS_ACCEL !(!hwndConsole && TranslateAccelerator(hwndMain, hAccelNoICS, &msg)) &&
...
void DoEvents() { /* Dispatch pending messages */
    MSG msg;
...
void SetUserLogo() {  // update user logo if necessary
...
void EnsureOnScreen(int * x, int * y, int minX, int minY) {
    ...
    if (*x > screenGeometry.right - 32) {
        *x = screenGeometry.left;
    }
    if (*y > screenGeometry.bottom - 32) {
        *y = screenGeometry.top;
    }
    if (*x < screenGeometry.left + minX) {
        *x = screenGeometry.left + minX;
    }
    if (*y < screenGeometry.top + minY) {
        *y = screenGeometry.top + minY;
    }
}

VOID LoadLogo(ChessProgramState * cps, int n, Boolean ics) {
    ...
    if (appData.logo[n] && appData.logo[n][0] != NULLCHAR) {
        ...
        if (ics) {  // [HGM] logo: in ICS mode second can be used for ICS
            ...
            if (gameMode == IcsPlayingWhite) {
                opponent = gameInfo.black;
            }
            if (gameMode == IcsPlayingBlack) {
                opponent = gameInfo.white;
            }
...
BOOL InitInstance(HINSTANCE hInstance, int nCmdShow, LPSTR lpCmdLine) {
    ...
    hwnd = CreateWindow(
     szAppName, szTitle, (WS_OVERLAPPEDWINDOW & ~WS_MAXIMIZEBOX), CW_USEDEFAULT, 0, CW_USEDEFAULT, 0, NULL, NULL, hInstance, NULL);
    hwndMain = hwnd;
    ...
    for (ibs = (int)NUM_SIZES - 1; ibs >= 0; ibs--) {
        ...
        if (boardSize == (BoardSize)-1 &&
         winH <= screenHeight - GetSystemMetrics(SM_CYFRAME) - GetSystemMetrics(SM_CYCAPTION) - 10 && winW <= screenWidth) {
    ...
    if (!appData.noGUI) {
        SetWindowPos(hwndMain, alwaysOnTop ? HWND_TOPMOST : HWND_NOTOPMOST, 0, 0, 0, 0, SWP_NOMOVE | SWP_NOSIZE);
    }

    if (hwndConsole) {
# if AOT_CONSOLE
        SetWindowPos(hwndConsole, alwaysOnTop ? HWND_TOPMOST : HWND_NOTOPMOST, 0, 0, 0, 0, SWP_NOMOVE | SWP_NOSIZE);
# endif
    ...
    if (gameListDialog) {
        SetFocus(gameListDialog);  // [HGM] jaws: for if we clicked multi-game game file
    }
...
VOID InitMenuChecks() {
    ...
    (void)EnableMenuItem(
     hmenu, IDM_CommPort, MF_BYCOMMAND | ((appData.icsActive && *appData.icsCommPort != NULLCHAR) ? MF_ENABLED : MF_GRAYED));
    (void)CheckMenuItem(hmenu, IDM_SaveSettingsOnExit, MF_BYCOMMAND | (saveSettingsOnExit ? MF_CHECKED : MF_UNCHECKED));
...
VOID LFfromMFP(LOGFONT * lf, MyFontParams * mfp) {
...
VOID ParseFontName(char * name, MyFontParams * mfp) {
    ...
    if (q) {
        ...
        while (q > mfp->faceName && q[-1] == ' ') {
            q--;
        }
        *q = NULLCHAR;
    }
    if (!*p) {
        ExitArgError(_("Font point size missing:"), name, TRUE);
    }
...
void ParseFont(char * name, int number) {  // wrapper to shield back-end from 'font'
    ParseFontName(name, &font[boardSize][number]->mfp);
}

void SetFontDefaults() {  // in WB  we have a 2D array of fonts; this initializes their description
    int i, j;
...
void CreateFonts() {  // here we create the actual fonts from the selected descriptions
    int i, j;
    for (i = 0; i < NUM_FONTS; i++) {
        for (j = 0; j < NUM_SIZES; j++) {
...
ParseColorName(char * name) {
    ...
    if (count != 3) {
        count = sscanf(name, "%3d%*[^0-9]%3d%*[^0-9]%3d", &red, &green, &blue);
    }
    if (count != 3) {
...
void ParseColor(int n, char * name) {  // for WinBoard the color is an int, which needs to be derived from the string
    if (colorVariable[n]) {
        *(int *)colorVariable[n] = ParseColorName(name);
    }
}
...
void ParseBoardSize(void * addr, char * name) {  // [HGM] rewritten with return-value ptr to shield back-end from BoardSize
...
void LoadAllSounds() {  // [HGM] import name from appData first
    ColorClass cc;
    SoundClass sc;
...
void SaveFontArg(
 FILE * f, ArgDescriptor * ad) {  // in WinBoard every board size has its own font, and the "argLoc" identifies the table,
    ...
    for (bs = 0; bs < NUM_SIZES; bs++) {
        ...
        fprintf(f, "/%s=\"%s:%g%s%s%s%s%sc%d\"\n", ad->argName, mfp->faceName, mfp->pointSize,
         mfp->bold || mfp->italic || mfp->underline || mfp->strikeout ? " " : "", mfp->bold ? "b" : "", mfp->italic ? "i" : "",
         mfp->underline ? "u" : "", mfp->strikeout ? "s" : "", (int)mfp->charset);
    }
}

void ExportSounds() {  // [HGM] copy the names from the internal WB variables to appData
    ColorClass cc;
    SoundClass sc;
    for (cc = (ColorClass)0; cc < ColorNormal; cc++) {
...
void SaveAttribsArg(FILE * f,
 ArgDescriptor * ad) {  // here the "argLoc" defines a table index. It could have contained the 'ta' pointer itself, though
    ...
    fprintf(f, "/%s=\"%s%s%s%s%s#%02lx%02lx%02lx\"\n", ad->argName, (ta->effects & CFE_BOLD) ? "b" : "",
     (ta->effects & CFE_ITALIC) ? "i" : "", (ta->effects & CFE_UNDERLINE) ? "u" : "", (ta->effects & CFE_STRIKEOUT) ? "s" : "",
     (ta->effects) ? " " : "", ta->color & 0xff, (ta->color >> 8) & 0xff, (ta->color >> 16) & 0xff);
}
...
void SaveColor(FILE * f,
 ArgDescriptor * ad) {  // in WinBoard the color is an int and has to be converted to text. In X it would be a string already?
    ...
    fprintf(f, "/%s=#%02lx%02lx%02lx\n", ad->argName, color & 0xff, (color >> 8) & 0xff, (color >> 16) & 0xff);
}
...
void ParseCommPortSettings(char * s) {  // wrapper to keep dcb from back-end
    ParseCommSettings(s, &dcb);
}

void GetWindowCoords() {  // wrapper to shield use of window handles from back-end (make addressible by number?)
...
void PrintCommPortSettings(FILE * f, char * name) {  // wrapper to shield back-end from DCB
...
int MySearchPath(char * installDir, char * name, char * fullname) {
    ...
    if (name[0] == '%') {
        ...
        if (appData.debugMode) {
            fprintf(debugFP, "name = '%s', expanded name = '%s'\n", name, fullname);
        }
...
int MainWindowUp() {  // [HGM] args: allows testing if main window is realized from back-end
...
static char pieceToFontChar[(int)EmptySquare] = {'p', 'n', 'b', 'r', 'q', 'n', 'b', 'p', 'n', 'b', 'r', 'b', 'r', 'q', 'k', 'k',
 'o', 'm', 'v', 't', 'w', 'v', 't', 'o', 'm', 'v', 't', 'v', 't', 'w', 'l', 'l'};
...
void CreatePiecesFromFont() {
    ...
    if (!appData.useFont || appData.renderPiecesWithFont == NULL || appData.renderPiecesWithFont[0] == NULLCHAR ||
     appData.renderPiecesWithFont[0] == '*') {
...
DoLoadBitmap(HINSTANCE hinst, char * piece, int squareSize, char * suffix) {
    ...
    if (gameInfo.event && strcmp(gameInfo.event, "Easter Egg Hunt") == 0 && strcmp(name, "k80s") == 0) {
...
*/
VOID InsertInPalette(COLORREF color) {
...
VOID InitDrawingColors() {
    ...
    for (i = 0; i < 8; i++) {
        markerBrush[i] = CreateSolidBrush(markerColor[i]);  // [HGM] markers
    }
...
int BoardWidth(int boardSize, int n) { /* [HGM] argument n added to allow different width and height */
    ...
    return (n + 1) * lineGap + n * sizeInfo[boardSize].squareSize;
}

/* Respond to board resize by dragging edge */
VOID ResizeBoard(int newSizeX, int newSizeY, int flags) {
    ...
    while (newSize > 0) {
        ...
        if (newSizeX >= sizeInfo[newSize].cliWidth && newSizeY >= sizeInfo[newSize].cliHeight) {
            break;
        }
        newSize--;
    }
...
VOID InitDrawingSizes(BoardSize boardSize, int flags) {
    ...
    if ((int)boardSize >= 1000) {
        boardSize -= 1000;
        suppressVisibleEffects = 1;
    }
    ...
    if (boardSize == -1) {
        return;  // no size defined yet; abort (to allow early call of InitPosition)
    }
    oldBoardSize = boardSize;

    if (boardSize != SizeMiddling && boardSize != SizePetite && boardSize != SizeBulky &&
     !appData.useFont) {  // correct board size to one where built-in pieces exist
        int fairies = 0;
        for (i = WhiteQueen + 1; i < WhiteKing; i++) {
            fairies += (PieceToChar(i) != '.' || PieceToChar(WHITE_TO_BLACK i) != '.');
        }
        if ((v == VariantCapablanca || v == VariantGothic || v == VariantGrand || v == VariantCapaRandom || v == VariantJanus ||
             v == VariantSuper) &&
         ...
         || v == VariantKnightmate || v == VariantSChess || v == VariantXiangqi || v == VariantSpartan || v == VariantShatranj ||
         v == VariantMakruk || v == VariantGreat || v == VariantLion || v == VariantJanggi || v == VariantFairy && fairies) {
            if (boardSize < SizeMediocre) {
                boardSize = SizePetite;
            } else if (boardSize > SizeModerate) {
                boardSize = SizeBulky;
            } else {
    ...
    if (!appData.useFont && boardSize == SizePetite && (v == VariantKnightmate)) {
        boardSize = SizeMiddling;  // no Unicorn in Petite
    }
    ...
    if (tinyLayout != oldTinyLayout) {
        ...
        if (tinyLayout == 2) {
            style &= ~WS_SYSMENU;
            InsertMenu(hmenu, IDM_Exit, MF_BYCOMMAND, IDM_Minimize, "&Minimize\tCtrl+F4");
        } else {
            style |= WS_SYSMENU;
        ...
        for (i = 0; menuBarText[tinyLayout][i]; i++) {
            ModifyMenu(hmenu, i, MF_STRING | MF_BYPOSITION | MF_POPUP, (UINT)GetSubMenu(hmenu, i), T_(menuBarText[tinyLayout][i]));
        }
        DrawMenuBar(hwndMain);
    }
    ...
    winH = 2 * GetSystemMetrics(SM_CYFRAME) + GetSystemMetrics(SM_CYMENU) + GetSystemMetrics(SM_CYCAPTION) + boardRect.bottom +
     OUTER_MARGIN;
    winW *= 1 + twoBoards;
    if (suppressVisibleEffects) {
        return;  // [HGM] when called for filling sizeInfo only
    }
    ...
    SetWindowPos(hwndMain, NULL, 0, 0, wpMain.width, wpMain.height, SWP_NOCOPYBITS | SWP_NOZORDER | SWP_NOMOVE);
    ...
    switch (flags) {
    case WMSZ_TOPLEFT:
        SetWindowPos(hwndMain, NULL, wrect.right - wpMain.width, wrect.bottom - wpMain.height, wpMain.width, wpMain.height,
    ...
    case WMSZ_TOP:
        SetWindowPos(
         hwndMain, NULL, wrect.left, wrect.bottom - wpMain.height, wpMain.width, wpMain.height, SWP_NOCOPYBITS | SWP_NOZORDER);
        break;

    case WMSZ_BOTTOMLEFT:
    case WMSZ_LEFT:
        SetWindowPos(
         hwndMain, NULL, wrect.right - wpMain.width, wrect.top, wpMain.width, wpMain.height, SWP_NOCOPYBITS | SWP_NOZORDER);
        break;
    ...
    for (i = 0; i < N_BUTTONS; i++) {
        ...
        if (appData.showButtonBar) {
            buttonDesc[i].hwnd = CreateWindow("BUTTON", buttonDesc[i].label, WS_VISIBLE | WS_CHILD | BS_PUSHBUTTON,
             boardRect.right - BUTTON_WIDTH * (N_BUTTONS - i), messageRect.top, BUTTON_WIDTH, messageSize.cy, hwndMain,
             (HMENU)buttonDesc[i].id, (HINSTANCE)GetWindowLongPtr(hwndMain, GWLP_HINSTANCE), NULL);
            if (tinyLayout == 2) {
                SendMessage(buttonDesc[i].hwnd, WM_SETFONT, (WPARAM)font[boardSize][MESSAGE_FONT]->hf, MAKELPARAM(FALSE, 0));
            }
            if (buttonDesc[i].id == IDM_Pause) {
                hwndPause = buttonDesc[i].hwnd;
            }
            buttonDesc[i].wndproc = (WNDPROC)SetWindowLongPtr(buttonDesc[i].hwnd, GWLP_WNDPROC, (LONG_PTR)ButtonProc);
        }
    }
    if (gridPen != NULL) {
    ...
    if (premovePen != NULL) {
        DeleteObject(premovePen);
    }
    if (lineGap != 0) {
        logbrush.lbStyle = BS_SOLID;
        logbrush.lbColor = RGB(0, 0, 0); /* grid pen color = black */
        gridPen = ExtCreatePen(PS_GEOMETRIC | PS_SOLID | PS_ENDCAP_FLAT | PS_JOIN_MITER, lineGap, &logbrush, 0, NULL);
        logbrush.lbColor = highlightSquareColor;
        highlightPen = ExtCreatePen(PS_GEOMETRIC | PS_SOLID | PS_ENDCAP_FLAT | PS_JOIN_MITER, lineGap, &logbrush, 0, NULL);

        logbrush.lbColor = premoveHighlightColor;
        premovePen = ExtCreatePen(PS_GEOMETRIC | PS_SOLID | PS_ENDCAP_FLAT | PS_JOIN_MITER, lineGap, &logbrush, 0, NULL);
        ...
        for (i = 0; i < BOARD_HEIGHT + 1; i++) {
            ...
            gridEndpoints[i * 2 + 1].x = boardRect.left + lineGap / 2 + BOARD_WIDTH * (squareSize + lineGap) + border;
            gridVertexCounts[i * 2] = gridVertexCounts[i * 2 + 1] = 2;
        }
        for (i = 0; i < BOARD_WIDTH + 1; i++) {
            gridEndpoints[i * 2 + BOARD_HEIGHT * 2 + 2].y = boardRect.top + lineGap / 2 + border;
            gridEndpoints[i * 2 + BOARD_HEIGHT * 2 + 2].x = gridEndpoints[i * 2 + 1 + BOARD_HEIGHT * 2 + 2].x =
             boardRect.left + lineGap / 2 + (i * (squareSize + lineGap)) + border;
            gridEndpoints[i * 2 + 1 + BOARD_HEIGHT * 2 + 2].y = boardRect.top + BOARD_HEIGHT * (squareSize + lineGap) + border;
            gridVertexCounts[i * 2] = gridVertexCounts[i * 2 + 1] = 2;
        }
    }
...
# ifdef GOTHIC
    if (gameInfo.variant == VariantGothic) {
        GothicPopUp(GOTHIC, VariantGothic);
    } else
# endif
# ifdef FALCON
     if (gameInfo.variant == VariantFalcon)
        GothicPopUp(FALCON, VariantFalcon);
    else
# endif
    ...
    for (i = 0; i <= 2; i++) {
        for (piece = WhitePawn; (int)piece < (int)BlackPawn; piece = (ChessSquare)((int)piece + 1)) {
    ...
    if (squareSize == 72 || squareSize == 49 || squareSize == 33) { /* experiment with some home-made bitmaps */
        ...
        if (gameInfo.variant == VariantDuck) {
            char name[20];
            sprintf(name, "ducky%d", squareSize);
            ducky = LoadBitmap(hInst, name);
        }
        ...
        if ((int)boardSize < 2) {
            minorSize = sizeInfo[0].squareSize;
        } else {
            minorSize = sizeInfo[(int)boardSize - 2].squareSize;
        }
    ...
    if (appData.pieceDirectory[0]) {
        for (i = WhitePawn; i < BlackPawn; i++) {  // try for all missing pieces with new naming convention
            char buf[MSG_SIZ];
            if (pieceBitmap[0][i]) {
                continue;
            }
...
VOID SquareToPos(int row, int column, int * x, int * y) {
    if (flipView) {
...
VOID DrawHighlightOnDC(HDC hdc, BOOLEAN on, int x, int y, int pen) {
    int x1, y1;
    HPEN oldPen, hPen;
    if (lineGap == 0) {
        return;
    ...
    if (flipView) {
        x1 = boardRect.left + lineGap / 2 + ((BOARD_WIDTH - 1) - x) * (squareSize + lineGap) + border;
        y1 = boardRect.top + lineGap / 2 + y * (squareSize + lineGap) + border;
    } else {
        x1 = boardRect.left + lineGap / 2 + x * (squareSize + lineGap) + border;
        y1 = boardRect.top + lineGap / 2 + ((BOARD_HEIGHT - 1) - y) * (squareSize + lineGap) + border;
    }
...
VOID DrawHighlightsOnDC(HDC hdc, HighlightInfo * h, int pen) {
    int i;
    for (i = 0; i < 2; i++) {
        if (h->sq[i].x >= 0 && h->sq[i].y >= 0) {
            DrawHighlightOnDC(hdc, TRUE, h->sq[i].x, h->sq[i].y, pen);
        }
    }
}
...
VOID DrawPieceOnDC(HDC hdc, ChessSquare piece, int color, int sqcolor, int x, int y, HDC tmphdc) {
    HBITMAP oldBitmap;
    ...
    if (fontBitmapSquareSize >= 0 && (squareSize > 32 || gameInfo.variant >= VariantShogi)) {
        ...
        if (fontBitmapSquareSize == squareSize) {
            ...
            if (appData.upsideDown ? color == flipView : (flipView && gameInfo.variant == VariantShogi)) {
                StretchBlt(
                 hdc, x + squareSize, y + squareSize, -squareSize, -squareSize, tmphdc, 0, 0, squareSize, squareSize, SRCAND);
            } else {
            ...
            if (appData.upsideDown ? color == flipView : (flipView && gameInfo.variant == VariantShogi)) {
                StretchBlt(
                 hdc, x + squareSize, y + squareSize, -squareSize, -squareSize, tmphdc, 0, 0, squareSize, squareSize, SRCPAINT);
            } else {
    ...
    if (appData.monoMode) {
        SelectObject(tmphdc, PieceBitmap(piece, color == sqcolor ? OUTLINE_PIECE : SOLID_PIECE));
        BitBlt(hdc, x, y, squareSize, squareSize, tmphdc, 0, 0, sqcolor ? SRCCOPY : NOTSRCCOPY);
    } else {
        HBRUSH xBrush = whitePieceBrush;
        tmpSize = squareSize;
        if (appData.pieceDirectory[0]) {
            xBrush = GetStockObject(WHITE_BRUSH);
        }
        if (minorSize &&
            ...
            if ((col + row) & 1) {
                ...
                if (lite_w >= squareSize && lite_h >= squareSize) {
                    if (lite_w >= squareSize * BOARD_WIDTH) {
                        backTextureSquareInfo[row][col].x = (2 * col + 1) * lite_w / (2 * BOARD_WIDTH) -
                         squareSize / 2; /* [HGM] cut out of center of virtual square */
                    } else {
                        backTextureSquareInfo[row][col].x =
                         col * (lite_w - squareSize) / (BOARD_WIDTH - 1); /* [HGM] divide by size-1 in stead of size! */
                    }
                    if (lite_h >= squareSize * BOARD_HEIGHT) {
                        backTextureSquareInfo[row][col].y =
                         (2 * (BOARD_HEIGHT - row) - 1) * lite_h / (2 * BOARD_HEIGHT) - squareSize / 2;
                    } else {
                ...
                if (dark_w >= squareSize && dark_h >= squareSize) {
                    ...
                    if (dark_h >= squareSize * BOARD_HEIGHT) {
                        backTextureSquareInfo[row][col].y =
                         (2 * (BOARD_HEIGHT - row) - 1) * dark_h / (2 * BOARD_HEIGHT) - squareSize / 2;
                    } else {
...
BOOL HasHighlightInfo() {
    ...
    if (highlightInfo.sq[0].x >= 0 && highlightInfo.sq[0].y >= 0 && highlightInfo.sq[1].x >= 0 && highlightInfo.sq[1].y >= 0) {
        result = TRUE;
    }
...
VOID DrawArrowHighlight(HDC hdc) {
    if (IsDrawArrowEnabled() && HasHighlightInfo()) {
        DrawArrowBetweenSquares(hdc, highlightInfo.sq[0].x, highlightInfo.sq[0].y, highlightInfo.sq[1].x, highlightInfo.sq[1].y);
    }
}
...
VOID DrawBackgroundOnDC(HDC hdc) {
    BITMAP bi;
    HDC tmphdc;
    HBITMAP hbm;
    ...
    StretchBlt(hdc, boardRect.left, boardRect.top, boardRect.right - boardRect.left, boardRect.bottom - boardRect.top, tmphdc, 0, 0,
     w, h, SRCCOPY);
...
VOID DrawBoardOnDC(HDC hdc, Board board, HDC tmphdc) {
    ...
    if (liteBackTexture != NULL || darkBackTexture != NULL) {
        ...
        if (backTextureSquareSize != squareSize || backTextureBoardSize != BOARD_WIDTH + BOARD_FILES * BOARD_HEIGHT) {
    ...
    for (row = 0; row < BOARD_HEIGHT; row++) {
        for (column = 0; column < BOARD_WIDTH; column++) {
            ...
            if (column == BOARD_LEFT - 1) { /* left align */
                ...
                squareSize, DisplayHoldingsCount(hdc, x, y, !flipView, (int)board[row][column]);
                ...
                if (piece == EmptySquare) {
                    BitBlt(hdc, x, y, squareSize, squareSize, 0, 0, 0, square_color ? WHITENESS : BLACKNESS);
                } else {
                ...
                DrawTile(x, y, squareSize, squareSize, hdc, texture_hdc, backTextureSquareInfo[r][c].mode,
                 backTextureSquareInfo[r][c].x, backTextureSquareInfo[r][c].y);
...
VOID DrawLogoOnDC(HDC hdc, RECT logoRect, HBITMAP logo) {
    ...
    StretchBlt(
     hdc, logoRect.left, logoRect.top, logoRect.right - logoRect.left, logoRect.bottom - logoRect.top, tmphdc, 0, 0, w, h, SRCCOPY);
    SelectObject(tmphdc, hbm);
    DeleteDC(tmphdc);
}
...
void UpdateLogos(int display) {  // called after loading new engine(s), in tourney or from menu
    LoadLogo(&first, 0, FALSE);
    LoadLogo(&second, 1, appData.icsActive);
...
void DrawSeekBackground(int left, int top, int right, int bottom) {
    ...
    rc.top = boardRect.top + top;
    rc.left = boardRect.left + left;
    rc.bottom = boardRect.top + bottom;
    rc.right = boardRect.left + right;
...
void DrawSeekDot(int x, int y, int color) {
    ...
    HBRUSH oldBrush = SelectObject(hdcSeek, color == 0 ? markerBrush[1] : color == 1 ? darkSquareBrush : explodeBrush);
    color &= 0x7f;
    if (square) {
...
VOID HDCDrawPosition(HDC hdc, BOOLEAN repaint, Board board) {
    ...
    if (DrawSeekGraph()) {
        return;  // [HG} seekgraph: suppress printing board if seek graph up
    }
    ...
    if (dragInfo.from.x >= 0 && dragInfo.pos.x >= 0) {
        ...
        } else if (dragInfo.from.x == BOARD_RGHT + 1) {
            if (--board[dragInfo.from.y][dragInfo.from.x - 1] == 0) {
                board[dragInfo.from.y][dragInfo.from.x] = EmptySquare;
            }
        } else {
    ...
    if (!fullrepaint && lastDrawnValid[nr] && (nr == 1 || lastDrawnFlipView == flipView)) {
        ...
        if (nr == 0) {  // [HGM] dual: no highlights on second board
            for (i = 0; i < 2; i++) {
                if (lastDrawnHighlight.sq[i].x != highlightInfo.sq[i].x || lastDrawnHighlight.sq[i].y != highlightInfo.sq[i].y) {
                    if (lastDrawnHighlight.sq[i].x >= 0 && lastDrawnHighlight.sq[i].y >= 0) {
                        SquareToPos(lastDrawnHighlight.sq[i].y, lastDrawnHighlight.sq[i].x, &x, &y);
                        clips[num_clips++] =
                         CreateRectRgn(x - lineGap, y - lineGap, x + squareSize + lineGap, y + squareSize + lineGap);
                    }
            ...
            for (i = 0; i < 2; i++) {
                if (lastDrawnPremove.sq[i].x != premoveHighlightInfo.sq[i].x ||
                 lastDrawnPremove.sq[i].y != premoveHighlightInfo.sq[i].y) {
                    if (lastDrawnPremove.sq[i].x >= 0 && lastDrawnPremove.sq[i].y >= 0) {
                        SquareToPos(lastDrawnPremove.sq[i].y, lastDrawnPremove.sq[i].x, &x, &y);
                        clips[num_clips++] =
                         CreateRectRgn(x - lineGap, y - lineGap, x + squareSize + lineGap, y + squareSize + lineGap);
                    }
                    if (premoveHighlightInfo.sq[i].x >= 0 && premoveHighlightInfo.sq[i].y >= 0) {
                        SquareToPos(premoveHighlightInfo.sq[i].y, premoveHighlightInfo.sq[i].x, &x, &y);
                        clips[num_clips++] =
                         CreateRectRgn(x - lineGap, y - lineGap, x + squareSize + lineGap, y + squareSize + lineGap);
                    }
                }
            ...
            for (i = 0; i < 2; i++) {
                if (partnerHighlightInfo.sq[i].x >= 0 && partnerHighlightInfo.sq[i].y >= 0) {
                    SquareToPos(partnerHighlightInfo.sq[i].y, partnerHighlightInfo.sq[i].x, &x, &y);
                    clips[num_clips++] =
                     CreateRectRgn(x - lineGap, y - lineGap, x + squareSize + lineGap, y + squareSize + lineGap);
                }
                if (oldPartnerHighlight.sq[i].x >= 0 && oldPartnerHighlight.sq[i].y >= 0) {
                    SquareToPos(oldPartnerHighlight.sq[i].y, oldPartnerHighlight.sq[i].x, &x, &y);
                    clips[num_clips++] =
                     CreateRectRgn(x - lineGap, y - lineGap, x + squareSize + lineGap, y + squareSize + lineGap);
                }
            }
     ...
     */
    GetClientRect(hwndMain, &Rect);
    bufferBitmap = CreateCompatibleBitmap(hdc, Rect.right - Rect.left + 1, Rect.bottom - Rect.top + 1);
        ...
        for (row = 0; row < BOARD_HEIGHT; row++) {
            for (column = 0; column < BOARD_WIDTH; column++) {
                if (marker[row][column]) {  // marker changes only occur with full repaint!
                    ...
                    SquareToPos(row, column, &x, &y);
                    Ellipse(hdcmem, x + squareSize / 4, y + squareSize / 4, x + 3 * squareSize / 4, y + 3 * squareSize / 4);
                    SelectObject(hdcmem, oldBrush);
                }
            }
        }
    ...
    if (dragged_piece != EmptySquare) {
        ...
        y = dragInfo.pos.y - squareSize / 2;
        DrawPieceOnDC(
         hdcmem, dragInfo.piece, ((int)dragInfo.piece < (int)BlackPawn), (dragInfo.from.y + dragInfo.from.x) % 2, x, y, tmphdc);
    }
    ...
    if (animInfo.piece != EmptySquare) {
        ...
        DrawPieceOnDC(
         hdcmem, animInfo.piece, ((int)animInfo.piece < (int)BlackPawn), (animInfo.from.y + animInfo.from.x) % 2, x, y, tmphdc);
    }
    ...
    if (!fullrepaint) {
        if (nr == 1) {
            for (x = 0; x < num_clips; x++) {  // [HGM] dual: translate clips
                RECT rect;
                GetRgnBox(clips[x], &rect);
                DeleteObject(clips[x]);
                clips[x] = CreateRectRgn(rect.left + wpMain.width / 2, rect.top, rect.right + wpMain.width / 2, rect.bottom);
            }
        }
    ...
    BitBlt(hdc, boardRect.left + twoBoards * partnerUp * wpMain.width / 2, boardRect.top,  // [HGM] dual
     boardRect.right - boardRect.left, boardRect.bottom - boardRect.top, tmphdc, boardRect.left, boardRect.top, SRCCOPY);
    if (saveDiagFlag) {
        BITMAP b;
        int i, j = 0, m, w, wb, fac = 0;
        char * pData;
        BITMAPINFOHEADER bih;
        int color[16], nrColors = 0;
        HBITMAP src = bufferBitmap, obmp;
        ...
        BitBlt(tmp, 0, 0, boardRect.right - boardRect.left, Rect.bottom - Rect.top - 2 * OUTER_MARGIN, tmphdc, boardRect.left,
         OUTER_MARGIN, SRCCOPY);
        ...
        if (pData = malloc(b.bmWidthBytes * b.bmHeight + 10000)) {
            ...
            if (fac) {
                fputDW(diagFile, 0x040001);  // planes and bits/pixel
            } else {
                fputDW(diagFile, 0x200001);  // planes and bits/pixel
            }
            fputDW(diagFile, 0);
            fputDW(diagFile, 0);
            fputDW(diagFile, 0);
...
VOID PaintProc(HWND hwnd) {
    ...
    if ((hdc = BeginPaint(hwnd, &ps))) {
        if (IsIconic(hwnd)) {
            ...
            ExtTextOut(hdc, messageRect.left, messageRect.top, ETO_CLIPPED | ETO_OPAQUE, &messageRect, messageText,
             strlen(messageText), NULL);
            SelectObject(hdc, oldFont);
            DisplayBothClocks();
            DisplayLogos();
        }
...
VOID SetupDropMenu(HMENU hmenu) {
    int i, count, enable;
    char * p;
    ...
    for (i = 0; i < sizeof(dropEnables) / sizeof(DropEnable); i++) {
        p = strchr(gameMode == IcsPlayingWhite ? white_holding : black_holding, dropEnables[i].piece);
        count = 0;
        while (p && *p++ == dropEnables[i].piece) {
            count++;
        }
        ...
        ModifyMenu(
         hmenu, dropEnables[i].command, MF_BYCOMMAND | (enable ? MF_ENABLED : MF_GRAYED) | MF_STRING, dropEnables[i].command, item);
    }
}
...
void ChangeDragPiece(ChessSquare piece) { dragInfo.piece = piece; }
...
VOID MouseEvent(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam) {
    ...
    case WM_MOUSEMOVE:
        if (SeekGraphClick(Press, pt.x - boardRect.left, pt.y - boardRect.top, 1)) {
            break;
        }
        if (PromoScroll(pt.x - boardRect.left, pt.y - boardRect.top)) {
            break;
        }
        MovePV(pt.x - boardRect.left, pt.y - boardRect.top, boardRect.bottom - boardRect.top);
        if ((appData.animateDragging || appData.highlightDragging) && (wParam & MK_LBUTTON || dragging == 2) &&
         dragInfo.from.x >= 0) {
    ...
    case WM_MOUSEWHEEL:  // [DM]
    {
        ...
        }  // [HGM] suppress first event in direction
...
LRESULT CALLBACK Promotion(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam) {
    ...
    case WM_INITDIALOG: /* message: initialize dialog box */
        ...
        ShowWindow(GetDlgItem(hDlg, PB_Archbishop),
         ((PieceToChar(WhiteAngel) >= 'A' && WhiteOnMove(currentMove) && PieceToChar(WhiteAngel) != '~') ||
          (PieceToChar(BlackAngel) >= 'A' && !WhiteOnMove(currentMove) && PieceToChar(BlackAngel) != '~'))
          ? SW_SHOW
          : SW_HIDE);
        ShowWindow(GetDlgItem(hDlg, PB_Chancellor),
         ((PieceToChar(WhiteMarshall) >= 'A' && WhiteOnMove(currentMove) && PieceToChar(WhiteMarshall) != '~') ||
          (PieceToChar(BlackMarshall) >= 'A' && !WhiteOnMove(currentMove) && PieceToChar(BlackMarshall) != '~'))
          ? SW_SHOW
          : SW_HIDE);
        ...
        ShowWindow(GetDlgItem(hDlg, IDC_Centaur), gameInfo.variant == VariantSuper ? SW_SHOW : SW_HIDE);
        return TRUE;
    ...
    case WM_COMMAND: /* message: received a command */
        ...
        case PB_Archbishop:
            ...
            break;
        case PB_Knight:
            promoChar = gameInfo.variant == VariantShogi ? '='
             : promoStyle                                ? NULLCHAR
        ...
        if (promoChar == '.') {
            return FALSE;  // invalid piece chosen
        }
        EndDialog(hDlg, TRUE); /* Exit the dialog */
...
VOID PromotionPopup(HWND hwnd) {
    ...
    DialogBox(hInst, MAKEINTRESOURCE(DLG_PromotionKing), hwnd, (DLGPROC)lpProc);
    FreeProcInstance(lpProc);
}
...
VOID LoadGameDialog(HWND hwnd, char * title) {
    UINT number = 0;
    FILE * f;
    char fileTitle[MSG_SIZ], dir[MSG_SIZ];
    GetCurrentDirectory(MSG_SIZ, dir);
    f = OpenFileDialog(hwnd, "rb", "", appData.oldSaveStyle ? "gam" : "pgn", GAME_FILT, title, &number, fileTitle, NULL);
...
VOID ChangedConsoleFont() {
    ...
    safeStrCpy(cfmt.szFaceName, font[boardSize][CONSOLE_FONT]->mfp.faceName, sizeof(cfmt.szFaceName) / sizeof(cfmt.szFaceName[0]));
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam) {
    ...
    case WM_CHAR:
        ...
        if (appData.icsActive && ((char)wParam == '\r' || (char)wParam > ' ' && !((char)wParam >= '1' && (char)wParam <= '9'))) {
            ...
            if (isalpha((char)wParam) || isdigit((char)wParam)) {
                TypeInEvent((char)wParam);
            } else if ((char)wParam == 003) {
                CopyGameToClipboard();
            } else if ((char)wParam == 026) {
    ...
    case WM_COMMAND: /* message: command from application menu */
        ...
        case IDM_LoadPosition:
            ...
            number = 1;
            GetCurrentDirectory(MSG_SIZ, dir);
            f = OpenFileDialog(hwnd, "rb", "", appData.oldSaveStyle ? "pos" : "fen", POSITION_FILT, _("Load Position from File"),
             &number, fileTitle, NULL);
            SetCurrentDirectory(dir);
            if (f != NULL) {
        ...
        case IDM_SaveGame:
            ...
            GetCurrentDirectory(MSG_SIZ, dir);
            f = OpenFileDialog(
             hwnd, "a", defName, appData.oldSaveStyle ? "gam" : "pgn", GAME_FILT, _("Save Game to File"), NULL, fileTitle, NULL);
            SetCurrentDirectory(dir);
            if (f != NULL) {
                SaveGame(f, 0, "");
            }
        ...
        case IDM_SavePosition:
            ...
            GetCurrentDirectory(MSG_SIZ, dir);
            f = OpenFileDialog(hwnd, "a", defName, appData.oldSaveStyle ? "pos" : "fen", POSITION_FILT, _("Save Position to File"),
             NULL, fileTitle, NULL);
            SetCurrentDirectory(dir);
            if (f != NULL) {
                SavePosition(f, 0, "");
        ...
        case IDM_SaveDiagram:
            defName = "diagram";
            GetCurrentDirectory(MSG_SIZ, dir);
            f = OpenFileDialog(hwnd, "wb", defName, "bmp", DIAGRAM_FILT, _("Save Diagram to File"), NULL, fileTitle, NULL);
            SetCurrentDirectory(dir);
            if (f != NULL) {
                SaveDiagram(f);
            }
        ...
        case IDM_SaveSelected:
            GetCurrentDirectory(MSG_SIZ, dir);
            f = OpenFileDialog(hwnd, "a", "", "pgn", GAME_FILT, _("Save Game to File"), NULL, fileTitle, NULL);
            SetCurrentDirectory(dir);
            if (f != NULL) {
                SaveSelected(f, 0, "");
            }
        ...
        case IDM_Match:  // [HGM] match: flows into next case, after setting Match Mode and nr of Games
            if (matchMode) {
                EnableMenuItem(GetMenu(hwndMain), IDM_Match, MF_BYCOMMAND | MF_GRAYED);
            }
        ...
        case OPT_GameListPrev:
            if (gameListDialog) {
                SendMessage(gameListDialog, WM_COMMAND, wmId, 0);
            }
            break;
        ...
        case IDM_MuteSounds:
            ...
            CheckMenuItem(GetMenu(hwndMain), IDM_MuteSounds, MF_BYCOMMAND | (mute ? MF_CHECKED : MF_UNCHECKED));
            break;
        ...
        case IDM_SaveSettingsOnExit:
            ...
            (void)CheckMenuItem(
             GetMenu(hwndMain), IDM_SaveSettingsOnExit, MF_BYCOMMAND | (saveSettingsOnExit ? MF_CHECKED : MF_UNCHECKED));
            break;
        ...
        case IDM_HELPCONTENTS:
            if (!MyHelp(hwnd, "winboard.hlp", HELP_KEY, (DWORD)(LPSTR) "CONTENTS") && !HtmlHelp(hwnd, "winboard.chm", 0, 0)) {
                MessageBox(GetFocus(), _("Unable to activate help"), szAppName, MB_SYSTEMMODAL | MB_OK | MB_ICONHAND);
            }
            break;

        case IDM_HELPSEARCH:
            if (!MyHelp(hwnd, "winboard.hlp", HELP_PARTIALKEY, (DWORD)(LPSTR) "") && !HtmlHelp(hwnd, "winboard.chm", 0, 0)) {
                MessageBox(GetFocus(), _("Unable to activate help"), szAppName, MB_SYSTEMMODAL | MB_OK | MB_ICONHAND);
            }
            break;
        ...
        case IDM_ABOUT:
            ...
            DialogBox(hInst, (gameInfo.event && strcmp(gameInfo.event, "Easter Egg Hunt") == 0) ? "AboutBox2" : "AboutBox", hwnd,
             (DLGPROC)lpProc);
            FreeProcInstance(lpProc);
            break;

        case IDM_DirectCommand1:
            AskQuestionEvent(_("Direct Command"), _("Send to chess program:"), "", "1");
            break;
        case IDM_DirectCommand2:
            AskQuestionEvent(_("Direct Command"), _("Send to second chess program:"), "", "2");
            break;
        ...
        default:
            ...
            } else if (wmId > IDM_English && wmId < IDM_English + 20) {
    ...
    case WM_TIMER:
        ...
        case ANALYSIS_TIMER_ID:
            if ((gameMode == AnalyzeMode || gameMode == AnalyzeFile || appData.icsEngineAnalyze) && appData.periodicUpdates) {
    ...
    case WM_WINDOWPOSCHANGING:
        ...
        if (hwnd == hwndMain && appData.useStickyWindows) {
            ...
            if (((lpwp->flags & SWP_NOMOVE) ==
                 0) /*&& ((lpwp->flags & SWP_NOSIZE) != 0)*/) {  // [HGM] in Win8 size always accompanies move?
                /* Window is moving */
                RECT rcMain;
    ...
    case WM_MOVING:
        if (appData.debugMode) {
            fprintf(debugFP, "moving\n");
        }
        return OnMoving(&sd, hwnd, wParam, lParam);

    case WM_EXITSIZEMOVE:
        if (appData.debugMode) {
            fprintf(debugFP, "exit size-move, size = %d\n", squareSize);
        }
        if (hwnd == hwndMain) {
            RECT client;
            doingSizing = FALSE;
...
void mysrandom(unsigned int seed) { randstate = seed; }
...
MyPlaySound(MySound * ms) {
    ...
    if (mute) {
        return TRUE;  // [HGM] mute: suppress all sound play when muted
    }
    switch (ms->name[0]) {
    case NULLCHAR:
        if (appData.debugMode) {
            fprintf(debugFP, "silence\n");
        }
        /* Silence */
        ok = TRUE;
        break;
...
LRESULT CALLBACK OldOpenFileHook(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam) {
    BOOL ok;
    OPENFILENAME * ofn;
...
UINT APIENTRY OpenFileHook(HWND hdlg, UINT uiMsg, WPARAM wParam, LPARAM lParam) {
    static UINT * number;
    OPENFILENAME * ofname;
    OFNOTIFY * ofnot;
    switch (uiMsg) {
...
FILE * OpenFileDialog(HWND hwnd, char * write, char * defName, char * defExt,  // [HGM] diag: type of 'write' now string
 char * nameFilt, char * dlgTitle, UINT * number, char fileTitle[MSG_SIZ], char fileName[MSG_SIZ]) {
    ...
    openFileName.Flags = OFN_PATHMUSTEXIST | OFN_HIDEREADONLY | (write[0] != 'r' ? 0 : OFN_FILEMUSTEXIST) |
     (number ? OFN_ENABLETEMPLATE | OFN_ENABLEHOOK : 0) | (oldDialog ? 0 : OFN_EXPLORER);
    ...
    if (write[0] != 'r' ? GetSaveFileName(&openFileName) : GetOpenFileName(&openFileName)) {
        /* open the file */
        f = fopen(openFileName.lpstrFile, write);
        if (f == NULL) {
            MessageBox(hwnd, _("File open failed"), NULL, MB_OK | MB_ICONEXCLAMATION);
            return NULL;
        }
    } else {
        int err = CommDlgExtendedError();
        if (err != 0) {
            DisplayError(_("Internal error in file dialog box"), err);
        }
        return FALSE;
    }
    return f;
...
BOOL CALLBACK ResizeEditPlusButtonsCallback(HWND hChild, LPARAM lparam) {
    ...
    cl->hdwp = DeferWindowPos(cl->hdwp, hChild, NULL, pt.x, pt.y, rect.right - rect.left, rect.bottom - rect.top, SWP_NOZORDER);
    return TRUE;
}
...
VOID ResizeEditPlusButtons(HWND hDlg, HWND hText, int sizeX, int sizeY, int newSizeX, int newSizeY) {
    ...
    cl.hdwp = DeferWindowPos(cl.hdwp, hText, NULL, 0, 0, newTextWidth, newTextHeight, SWP_NOZORDER | SWP_NOMOVE);
...
BOOL CenterWindow(HWND hwndChild, HWND hwndParent) { return CenterWindowEx(hwndChild, hwndParent, 0); }
...
void InitComboStringsFromOption(HANDLE hwndCombo, char * str) {
...
void SetStartupDialogEnables(HWND hDlg) {
    ...
    EnableWindow(GetDlgItem(hDlg, IDOK),
     IsDlgButtonChecked(hDlg, OPT_ChessEngine) || IsDlgButtonChecked(hDlg, OPT_ChessServer) || IsDlgButtonChecked(hDlg, OPT_View));
}

char * QuoteForFilename(char * filename) {
    int dquote, space;
...
VOID InitEngineBox(HWND hDlg, HWND hwndCombo, char * nthcp, char * nthd, char * nthdir, char * nthnames) {
    char buf[MSG_SIZ];
    char * q;
...
LRESULT CALLBACK StartupDialog(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam) {
    ...
    case WM_INITDIALOG:
        ...
        InitEngineBox(hDlg, GetDlgItem(hDlg, OPT_ChessEngineName), appData.firstChessProgram, "fd", appData.firstDirectory,
         firstChessProgramNames);
        InitEngineBox(hDlg, GetDlgItem(hDlg, OPT_SecondChessEngineName), appData.secondChessProgram, singleList ? "fd" : "sd",
         appData.secondDirectory,
...
LRESULT CALLBACK CommentDialog(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam) {
    ...
    case WM_INITDIALOG: /* message: initialize dialog box */
        ...
        SendMessage(GetDlgItem(hDlg, OPT_CommentText), WM_SETFONT, (WPARAM)font[boardSize][COMMENT_FONT]->hf, MAKELPARAM(FALSE, 0));
        ...
        if (!commentDialog) {
            ...
            sizeX = rect.right;
            sizeY = rect.bottom;
            if (wpComment.x != CW_USEDEFAULT && wpComment.y != CW_USEDEFAULT && wpComment.width != CW_USEDEFAULT &&
        ...
        if (wParam == OPT_CommentText) {
            ...
            if (lpMF->msg == WM_RBUTTONDOWN && (lpMF->wParam & (MK_CONTROL | MK_SHIFT)) == 0 ||
             lpMF->msg == WM_CHAR && lpMF->wParam == '\022') {
                ...
                GetWindowText(hwndText, str, len + 1);
                ReplaceComment(commentIndex, str);
                if (commentIndex != currentMove) {
                    ToNrEvent(commentIndex);
                }
...
VOID EitherCommentPopUp(int index, char * title, char * str, BOOLEAN edit) {
    FARPROC lpProc;
    char *p, *q;
    ...
    *q = NULLCHAR;
    ...
    if (commentDialog) {
        ...
        if (!commentUp) {
            ShowWindow(commentDialog, SW_SHOW);
        }
    } else {
        lpProc = MakeProcInstance((FARPROC)CommentDialog, hInst);
        CreateDialog(hInst, MAKEINTRESOURCE(DLG_EditComment), hwndMain, (DLGPROC)lpProc);
        FreeProcInstance(lpProc);
    }
    commentUp = TRUE;
}
...
LRESULT CALLBACK TypeInMoveDialog(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam) {
    char move[MSG_SIZ];
    HWND hInput;
...
VOID PopUpMoveDialog(char firstchar) {
    ...
    DialogBoxParam(hInst, MAKEINTRESOURCE(DLG_TypeInMove), hwndMain, (DLGPROC)lpProc, (LPARAM)firstchar);
    FreeProcInstance(lpProc);
}
...
LRESULT CALLBACK TypeInNameDialog(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam) {
    char move[MSG_SIZ];
    HWND hInput;
...
VOID PopUpNameDialog(char firstchar) {
    ...
    DialogBoxParam(hInst, MAKEINTRESOURCE(DLG_TypeInName), hwndMain, (DLGPROC)lpProc, (LPARAM)firstchar);
    FreeProcInstance(lpProc);
}
...
VOID ErrorPopUp(char * title, char * content) {
    FARPROC lpProc;
    char *p, *q;
    ...
    *q = NULLCHAR;
    ...
    if (modal) {
        ...
        CreateDialog(hInst, MAKEINTRESOURCE(n > 4 ? DLG_Big : DLG_Error), hwndMain, (DLGPROC)lpProc);
        FreeProcInstance(lpProc);
    }
}

VOID ErrorPopDown() {
    if (!appData.popupMoveErrors && moveErrorMessageUp) {
        DisplayMessage("", "");
    }
    if (errorDialog == NULL) {
        return;
...
LRESULT CALLBACK GothicDialog(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam) {
    RECT rChild;
    ...
    case WM_INITDIALOG:
        ...
        SetWindowPos(hDlg, NULL, wpMain.x, wpMain.y - height, wpMain.width, height, SWP_NOZORDER);
...
VOID GothicPopUp(char * title, VariantClass variant) {
    ...
    if (variant != VariantNormal && gothicDialog == NULL) {
        ...
        CreateDialog(hInst, MAKEINTRESOURCE(DLG_Error), hwndMain, (DLGPROC)lpProc);
        FreeProcInstance(lpProc);
    }
}
# endif
...
LoadIcsTextMenu(IcsTextMenuEntry * e) {
    ...
    while (e->item) {
        if (strcmp(e->item, "-") == 0) {
            ...
            if (!strcmp(e->command, "none")) {
                flags |= MF_GRAYED;  // [HGM] chatclick: provide inactive dummy
            }
...
void CommandX(HWND hwnd, char * command, BOOLEAN getname, BOOLEAN immediate) {
    ...
    if (sel.cpMin == sel.cpMax) {
        ...
        do {
            ...
            tr.lpstrText = name;
            if (SendMessage(hwnd, EM_GETTEXTRANGE, 0, (LPARAM)&tr) < 1) {
                break;
            }
        } while (isalpha(name[0]) || isdigit(name[0]) || name[0] == '-');
        sel.cpMax--;
    ...
    if (immediate) {
        if (strstr(command, "%s")) {
            snprintf(buf, MSG_SIZ, command, name);
        } else {
...
LRESULT CALLBACK ConsoleTextSubclass(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam) {
    HWND hInput;
    CHARRANGE sel;
    ...
    case WM_RBUTTONUP:
        if (GetKeyState(VK_SHIFT) & ~1) {
            SendDlgItemMessage(hwndConsole, OPT_ConsoleText, WM_COMMAND, MAKEWPARAM(IDM_QuickPaste, 0), 0);
        }
        return 0;
    case WM_PASTE:
        hInput = GetDlgItem(hwndConsole, OPT_ConsoleInput);
    ...
    case WM_COMMAND:
        ...
        default: {
            ...
            if (i >= 0 && i < ICS_TEXT_MENU_SIZE && icsTextMenuEntry[i].command != NULL) {
                CommandX(hwnd, icsTextMenuEntry[i].command, icsTextMenuEntry[i].getname, icsTextMenuEntry[i].immediate);
                return 0;
            }
        } break;
        }
...
LRESULT CALLBACK ConsoleInputSubclass(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam) {
    char buf[MSG_SIZ];
    char * p;
...
LRESULT CALLBACK ConsoleWndProc(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam) {
    ...
    case WM_INITDIALOG: /* message: initialize dialog box */
        hwndConsole = hDlg;
        SetFocus(hInput);
        consoleTextWindowProc = (WNDPROC)SetWindowLongPtr(hText, GWLP_WNDPROC, (LONG_PTR)ConsoleTextSubclass);
        SendMessage(hText, EM_SETBKGNDCOLOR, FALSE, consoleBackgroundColor);
        consoleInputWindowProc = (WNDPROC)SetWindowLongPtr(hInput, GWLP_WNDPROC, (LONG_PTR)ConsoleInputSubclass);
        ...
        sizeX = rect.right;
        sizeY = rect.bottom;
        if (wpConsole.x != CW_USEDEFAULT && wpConsole.y != CW_USEDEFAULT && wpConsole.width != CW_USEDEFAULT &&
         wpConsole.height != CW_USEDEFAULT) {
            WINDOWPLACEMENT wp;
    ...
    case WM_SIZE:
        ...
        if (sizeX != newSizeX || sizeY != newSizeY) {
            ...
            SetWindowPos(hText, NULL, 0, 0, newTextWidth, newTextHeight, SWP_NOZORDER | SWP_NOMOVE);
            GetWindowRect(hInput, &rectInput); /* gives screen coords */
            pt.x = rectInput.left;
            pt.y = rectInput.top + newSizeY - sizeY;
            ScreenToClient(hDlg, &pt);
            SetWindowPos(hInput, NULL, pt.x, pt.y, /* needs client coords */
             rectInput.right - rectInput.left + newSizeX - sizeX, rectInput.bottom - rectInput.top, SWP_NOZORDER);
        }
        sizeX = newSizeX;
        sizeY = newSizeY;
        break;
...
VOID ConsoleOutput(char * data, int length, int forceVisible) {
    ...
    if (hwndConsole == NULL || length > CO_MAX - 100 || length == 0) {
        return;
    }
    p = data;
    q = buf;
    ...
    *q = NULLCHAR;
    ...
    if (forceVisible || exlen == 0 || (rect.left <= pEnd.x && pEnd.x < rect.right && rect.top <= pEnd.y && pEnd.y < rect.bottom)) {
...
void DisplayHoldingsCount(HDC hdc, int x, int y, int rightAlign, int copyNumber) {
    char buf[100];
    char * str;
    ...
    if (copyNumber > 1) {
        snprintf(buf, sizeof(buf) / sizeof(buf[0]), "%d", copyNumber);
    } else {
        buf[0] = 0;
    }
    ...
    ExtTextOut(hdc, x + MESSAGE_LINE_LEFTMARGIN + (rightAlign ? (squareSize * 2) / 3 : 0), y, ETO_CLIPPED | ETO_OPAQUE, &rect, str,
     strlen(str), NULL);
...
void DisplayAClock(HDC hdc, int timeRemaining, int highlight, RECT * rect, char * color, char * flagFell) {
    char buf[100];
    char *str = buf, *p;
    COLORREF oldFg, oldBg;
    HFONT oldFont;
    ...
    ExtTextOut(hdc, rect->left + MESSAGE_LINE_LEFTMARGIN, rect->top, ETO_CLIPPED | ETO_OPAQUE, rect, str, strlen(str), NULL);
    if (logoHeight > 0 && p) {
        RECT r;
        str = p + 1;
        ...
        ExtTextOut(hdc, rect->left + MESSAGE_LINE_LEFTMARGIN, r.top, ETO_CLIPPED | ETO_OPAQUE, &r, str, strlen(str), NULL);
    }
    (void)SetTextColor(hdc, oldFg);
    (void)SetBkColor(hdc, oldBg);
    (void)SelectObject(hdc, oldFont);
...
int DoReadFile(HANDLE hFile, char * buf, int count, DWORD * outCount, OVERLAPPED * ovl) {
    int ok, err;
...
int DoWriteFile(HANDLE hFile, char * buf, int count, DWORD * outCount, OVERLAPPED * ovl) {
    int ok, err;
...
InputThread(LPVOID arg) {
    ...
    while (is->hThread != NULL) {
        is->error = DoReadFile(is->hFile, is->next, INPUT_SOURCE_BUF_SIZE - (is->next - is->buf), &is->count, &ovl);
...
NonOvlInputThread(LPVOID arg) {
    ...
    while (is->hThread != NULL) {
        is->error =
         ReadFile(is->hFile, is->next, INPUT_SOURCE_BUF_SIZE - (is->next - is->buf), &is->count, NULL) ? NO_ERROR : GetLastError();
        if (is->error == NO_ERROR) {
            /* Change CRLF to LF */
            if (is->next > is->buf) {
                p = is->next - 1;
...
VOID GreyRevert(Boolean grey) {  // [HGM] vari: for retracting variations in local mode
    HMENU hmenu = GetMenu(hwndMain);
...
VOID SetMenuEnables(HMENU hmenu, Enables * enab) {
    while (enab->item > 0) {
...
# else
 {IDM_TimeControl, MF_BYCOMMAND | MF_GRAYED}, {IDM_Sounds, MF_BYCOMMAND | MF_GRAYED},
# endif
 {IDM_Hint,           MF_BYCOMMAND | MF_GRAYED },
 {IDM_Book,           MF_BYCOMMAND | MF_GRAYED },
 {IDM_MachineBoth,    MF_BYCOMMAND | MF_GRAYED },
 ...
 *
\*---------------------------------------------------------------------------*/
VOID CheckMark(UINT item, int state) {
    if (item) {
        CheckMenuItem(GetMenu(hwndMain), item, MF_BYCOMMAND | state);
    }
}
...
VOID ModeHighlight() {
    ...
    if (pausing != prevPausing) {
        prevPausing = pausing;
        (void)CheckMenuItem(GetMenu(hwndMain), IDM_Pause, MF_BYCOMMAND | (pausing ? MF_CHECKED : MF_UNCHECKED));
        if (hwndPause) {
            SetWindowText(hwndPause, pausing ? "C" : "P");
        }
    }
    ...
    if (nowChecked == IDM_LoadGame || nowChecked == IDM_Training) {
        (void)EnableMenuItem(GetMenu(hwndMain), IDM_Training, MF_BYCOMMAND | MF_ENABLED);
    } else {
        (void)EnableMenuItem(GetMenu(hwndMain), IDM_Training, MF_BYCOMMAND | MF_GRAYED);
    }
...
VOID SetICSMode() {
    ...
    EnableMenuItem(GetSubMenu(hmenu, OPTIONS_POS), IDM_IcsOptions, MF_BYCOMMAND | MF_ENABLED);
# if ZIPPY
    if (appData.zippyPlay) {
        SetMenuEnables(hmenu, zippyEnables);
        if (!appData.noChessProgram) { /* [DM] icsEngineAnalyze */
            (void)EnableMenuItem(GetMenu(hwndMain), IDM_AnalysisMode, MF_BYCOMMAND | MF_ENABLED);
        }
    }
# endif
}

VOID SetGNUMode() { SetMenuEnables(GetMenu(hwndMain), gnuEnables); }
...
VOID SetCmailMode() { SetMenuEnables(GetMenu(hwndMain), cmailEnables); }

VOID SetTrainingModeOn() {
    int i;
...
VOID SetUserThinkingEnables() { SetMenuEnables(GetMenu(hwndMain), userThinkingEnables); }

VOID SetMachineThinkingEnables() {
    HMENU hMenu = GetMenu(hwndMain);
...
VOID DisplayMessage(char * str1, char * str2) {
    HDC hdc;
    HFONT oldFont;
    ...
    if (hwndMain == NULL || IsIconic(hwndMain)) {
        return;
    }
...
VOID DisplayError(char * str, int error) {
    ...
    if (error == 0) {
        ...
        len = FormatMessage(FORMAT_MESSAGE_FROM_SYSTEM, NULL, error, LANG_NEUTRAL, (LPSTR)buf2, MSG_SIZ, NULL);
        if (len > 0) {
            snprintf(buf, 2 * MSG_SIZ, "%s:\n%s", str, buf2);
        } else {
            ErrorMap * em = errmap;
            while (em->err != 0 && em->err != error) {
                em++;
            }
            if (em->err != 0) {
                snprintf(buf, 2 * MSG_SIZ, "%s:\n%s", str, em->msg);
...
VOID DisplayMoveError(char * str) {
    fromX = fromY = -1;
    ClearHighlights();
    DrawPosition(FALSE, NULL);
...
VOID DisplayFatalError(char * str, int error, int exitStatus) {
    ...
    if (appData.popupExitMessage) {
        ...
        (void)MessageBox(hwndMain, str, label, MB_OK | (exitStatus ? MB_ICONSTOP : MB_ICONINFORMATION));
    }
    ExitEvent(exitStatus);
}


VOID DisplayInformation(char * str) { (void)MessageBox(hwndMain, str, _("Information"), MB_OK | MB_ICONINFORMATION); }

char * Shorten(char * s) { return s; }

VOID DisplayNote(char * str) { ErrorPopUp(_("Note"), str); }
...
LRESULT CALLBACK QuestionDialog(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam) {
    ...
    case WM_COMMAND:
        ...
        case IDOK:
            ...
            EndDialog(hDlg, TRUE);
            if (err) {
                DisplayFatalError(_("Error writing to chess program"), err, 1);
            }
            return TRUE;
        case IDCANCEL:
            EndDialog(hDlg, FALSE);
...
VOID AskQuestion(char * title, char * question, char * replyPrefix, ProcRef pr) {
    ...
    DialogBoxParam(hInst, MAKEINTRESOURCE(DLG_Question), hwndMain, (DLGPROC)lpProc, (LPARAM)&qp);
    FreeProcInstance(lpProc);
}
...
// low-level front-end: clear text edit / list widget
void GLT_DeSelectList() { SendDlgItemMessage(gameListOptionsDialog, IDC_GameListTags, LB_SETCURSEL, 0, 0); }
...
VOID DisplayIcsInteractionTitle(char * str) {
...
void DrawPositionX(int fullRedraw, Board board) { HDCDrawPosition(NULL, (BOOLEAN)fullRedraw, board); }
...
VOID CommentPopUp(char * title, char * str) {
...
VOID EditCommentPopUp(int index, char * title, char * str) { EitherCommentPopUp(index, title, str, TRUE); }
...
VOID RingBell() { MyPlaySound(&sounds[(int)SoundMove]); }

VOID PlayIcsWinSound() { MyPlaySound(&sounds[(int)SoundIcsWin]); }

VOID PlayIcsLossSound() { MyPlaySound(&sounds[(int)SoundIcsLoss]); }

VOID PlayIcsDrawSound() { MyPlaySound(&sounds[(int)SoundIcsDraw]); }

VOID PlayIcsUnfinishedSound() { MyPlaySound(&sounds[(int)SoundIcsUnfinished]); }

VOID PlayAlarmSound() { MyPlaySound(&sounds[(int)SoundAlarm]); }

VOID PlayTellSound() { MyPlaySound(&textAttribs[ColorTell].sound); }
...
void Colorize(ColorClass cc, int continuation) {
    ...
    if (!continuation) {
        MyPlaySound(&textAttribs[cc].sound);
    }
}
...
int ClockTimerRunning() { return clockTimerEvent != 0; }

int StopClockTimer() {
    if (clockTimerEvent == 0) {
        return FALSE;
    }
...
void StartClockTimer(long millisec) { clockTimerEvent = SetTimer(hwndMain, (UINT)CLOCK_TIMER_ID, (UINT)millisec, NULL); }
...
void DisplayWhiteClock(long timeRemaining, int highlight) {
    ...
    if (!IsIconic(hwndMain)) {
        ...
        DisplayAClock(
         hdc, timeRemaining, highlight + 2 * m, flipClock ? &blackRect : &whiteRect, m ? clockMsg[0] : _("White"), flag);
    }
...
void DisplayBlackClock(long timeRemaining, int highlight) {
    ...
    if (!IsIconic(hwndMain)) {
        ...
        DisplayAClock(
         hdc, timeRemaining, highlight + 2 * m, flipClock ? &whiteRect : &blackRect, m ? clockMsg[1] : _("Black"), flag);
    }
...
void SetClockMessage(int color, char * msg) { safeStrCpy(clockMsg[color], !msg ? "" : *msg ? msg : clockMsg[!color], MSG_SIZ); }

int LoadGameTimerRunning() { return loadGameTimerEvent != 0; }

int StopLoadGameTimer() {
    if (loadGameTimerEvent == 0) {
        return FALSE;
    }
...
void StartLoadGameTimer(long millisec) { loadGameTimerEvent = SetTimer(hwndMain, (UINT)LOAD_GAME_TIMER_ID, (UINT)millisec, NULL); }
...
void AutoSaveGame() {
    ...
    GetCurrentDirectory(MSG_SIZ, dir);
    f = OpenFileDialog(
     hwndMain, "a", defName, appData.oldSaveStyle ? "gam" : "pgn", GAME_FILT, _("Save Game to File"), NULL, fileTitle, NULL);
    GetCurrentDirectory(MSG_SIZ, dir);
    if (f != NULL) {
        SaveGame(f, 0, "");
        fclose(f);
...
void ScheduleDelayedEvent(DelayedEventCallback cb, long millisec) {
    ...
    delayedTimerEvent = SetTimer(hwndMain, (UINT)DELAYED_TIMER_ID, (UINT)millisec, NULL);
}

DelayedEventCallback GetDelayedEvent() {
...
DWORD GetWin32Priority(
 int nice) {  // [HGM] nice: translate Unix nice() value to indows priority class. (Code stolen from Polyglot 1.4w11)
    /*
...
void RunCommand(char * cmdLine) {
    ...
    CreateProcess(NULL, cmdLine, /* command line */
...
# define BUFSIZE 4096
    ...
    *pr = NoProc;
    ...
    fSuccess =
     DuplicateHandle(GetCurrentProcess(), hChildStdoutRd, GetCurrentProcess(), &hChildStdoutRdDup, 0, FALSE, /* not inherited */
    ...
    fSuccess =
     DuplicateHandle(GetCurrentProcess(), hChildStdinWr, GetCurrentProcess(), &hChildStdinWrDup, 0, FALSE, /* not inherited */
      DUPLICATE_SAME_ACCESS);
    if (!fSuccess) {
        return GetLastError();
    }
    ...
    fSuccess = CreateProcess(NULL, cmdLine, /* command line */
     NULL, /* process security attributes */
     NULL, /* primary thread security attrs */
     TRUE, /* handles are inherited */
     DETACHED_PROCESS | CREATE_NEW_PROCESS_GROUP, NULL, /* use parent's environment */
    ...
    if (appData.niceEngines) {  // [HGM] nice: adjust engine proc priority
        if (appData.debugMode) {
            fprintf(debugFP, "nice engine proc to %d\n", appData.niceEngines);
        }
...
void DestroyChildProcess(ProcRef pr, int /*boolean*/ signal) {
    ...
    case CPReal:
        ...
        if (cp->hFrom) {
            CloseHandle(cp->hFrom); /* if NULL, InputThread will close it */
        }
    ...
    case CPRcmd:
        if (signal) {
            send(cp->sock2, "\017", 1, 0); /* 017 = 15 = SIGTERM */
        }
        closesocket(cp->sock);
...
int OpenTCP(char * host, char * port, ProcRef * pr) {
    ...
    if (bind(s, (struct sockaddr *)&mysa, sizeof(struct sockaddr_in)) == SOCKET_ERROR) {
        err = WSAGetLastError();
        WSACleanup();
        return err;
    }
...
int OpenCommPort(char * name, ProcRef * pr) {
    ...
    if (!SetCommTimeouts(h, (LPCOMMTIMEOUTS)&ct)) {
        return GetLastError();
    }
...
int OpenRcmd(char * host, char * user, char * cmd, ProcRef * pr) {
    ...
    mysa.sin_addr.s_addr = INADDR_ANY;
    for (fromPort = 1023;; fromPort--) {
        if (fromPort == prevStderrPort) {
            continue;  // don't reuse port
        }
        if (fromPort < 0) {
            (void)closesocket(s);
            WSACleanup();
        ...
        uport = (unsigned short)fromPort;
        mysa.sin_port = htons(uport);
        if (bind(s2, (struct sockaddr *)&mysa, sizeof(struct sockaddr_in)) == SOCKET_ERROR) {
            err = WSAGetLastError();
            if (err == WSAEADDRINUSE) {
                continue;
            }
    ...
    if (pr == NoProc) {
        ...
        */
        ...
        case CPReal:
            ...
            is->hThread = CreateThread(NULL, 0, (LPTHREAD_START_ROUTINE)NonOvlInputThread, (LPVOID)is, CREATE_SUSPENDED, &is->id);
            break;

        case CPComm:
            is->hFile = cp->hFrom;
            cp->hFrom = NULL; /* now owned by InputThread */
            is->hThread = CreateThread(NULL, 0, (LPTHREAD_START_ROUTINE)InputThread, (LPVOID)is, CREATE_SUSPENDED, &is->id);
            break;

        case CPSock:
            is->sock = cp->sock;
            is->hThread = CreateThread(NULL, 0, (LPTHREAD_START_ROUTINE)SocketInputThread, (LPVOID)is, CREATE_SUSPENDED, &is->id);
            break;
        ...
        case CPRcmd:
            ...
            *is2 = *is;
            ...
            is->hThread = CreateThread(NULL, 0, (LPTHREAD_START_ROUTINE)SocketInputThread, (LPVOID)is, CREATE_SUSPENDED, &is->id);
            is2->hThread =
             CreateThread(NULL, 0, (LPTHREAD_START_ROUTINE)SocketInputThread, (LPVOID)is2, CREATE_SUSPENDED, &is2->id);
            break;
        }
...
int OutputToProcess(ProcRef pr, char * message, int count, int * outError) {
    ...
    case CPReal:
        if (WriteFile(((ChildProc *)pr)->hTo, message, count, &dOutCount, NULL)) {
            *outError = NO_ERROR;
    ...
    case CPComm:
        *outError = DoWriteFile(((ChildProc *)pr)->hTo, message, count, &dOutCount, &ovl);
...
VOID StartAnalysisClock() {
    if (analysisTimerEvent) {
        return;
    }
    analysisTimerEvent = SetTimer(hwndMain, (UINT)ANALYSIS_TIMER_ID, (UINT)2000, NULL);
}
...
VOID ClearHighlights() { highlightInfo.sq[0].x = highlightInfo.sq[0].y = highlightInfo.sq[1].x = highlightInfo.sq[1].y = -1; }
...
VOID ClearPremoveHighlights() {
    premoveHighlightInfo.sq[0].x = premoveHighlightInfo.sq[0].y = premoveHighlightInfo.sq[1].x = premoveHighlightInfo.sq[1].y = -1;
}
...
static void Tween(POINT * start, POINT * mid, POINT * finish, int factor, POINT frames[], int * nFrames);


# define kFactor 4

void AnimateAtomicCapture(Board board, int fromX, int fromY, int toX, int toY) {  // [HGM] atomic: animate blast wave
    int i;
...
void AnimateMove(Board board, int fromX, int fromY, int toX, int toY) {
    ...
    if (killX >= 0 && IS_LION(board[fromY][fromX])) {
        Roar();
    }
    ...
    if (x2 >= 0) {
        toX = kill2X, toY = kill2Y, victim = board[killY][killX], victim2 = board[kill2Y][kill2X];
    } else if (killX >= 0) {
        toX = killX, toY = killY, victim = board[killY][killX];  // [HGM] lion: first to kill square
    }
...
again:
    ...
    if (victim2 != EmptySquare) {
        board[kill2Y][kill2X] = victim2;
    }
    if (victim != EmptySquare) {
        board[killY][killX] = victim;
    }
...
void SettingsPopUp(ChessProgramState * cps) {  // [HGM] wrapper needed because handles must not be passed through back-end
...
int flock(int fid, int code) {
    ...
    switch (code) {
    case 1:
        res = LockFileEx(hFile, LOCKFILE_EXCLUSIVE_LOCK, 0, 1024, 0, &ov);
        break;  // LOCK_SH
    case 2:
        res = LockFileEx(hFile, LOCKFILE_EXCLUSIVE_LOCK, 0, 1024, 0, &ov);
        break;  // LOCK_EX
    case 3:
        res = UnlockFileEx(hFile, 0, 1024, 0, &ov);
        break;  // LOCK_UN
...
void ActivateTheme(int new) {  // Redo initialization of features depending on options that can occur in themes
    InitTextures();
    if (new) {
        InitDrawingColors();
    }
