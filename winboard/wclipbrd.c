...
VOID CopyFENToClipboard() {
    ...
    if (gameMode == EditPosition) {
        EditPositionDone(TRUE);  // [HGM] mak sure castling rights are set consistently
    }
...
int CopyTextToClipboard(char * text) {
    ...
    if (!locked) {
        ...
        if (appData.debugMode) {
            fprintf(debugFP, "CopyTextToClipboard(): err %d locked %d\n", (int)err, locked);
        }
    }
    if (locked) {
...
int PasteTextFromClipboard(char ** text) {
    ...
    *text = (char *)malloc(GlobalSize(hClipMem) + 1);
    ...
    if (!locked) {
        locked = !((err == NO_ERROR) || (err == ERROR_NOT_LOCKED));
        if (appData.debugMode) {
            fprintf(debugFP, "PasteTextFromClipboard(): err %d locked %d\n", (int)err, locked);
        }
    }
    if (locked) {
