...
static void ResizeWindowControls(HWND hDlg) {
    ...
    SetControlPos(
     hDlg, IDC_ChatMemo, H_MARGIN, 2 * V_MARGIN + buttonHeight, maxControlWidth, clientHeight - 3 * V_MARGIN - 2 * buttonHeight);
...
LRESULT CALLBACK InterceptArrowKeys(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam) {
    char buf[MSG_SIZ];
    char * p;
    CHARRANGE sel;
    ...
    case WM_INITDIALOG:
        ...
        for (i = 0; i < MAX_CHAT; i++) {
            if (chatHandle[i]) {
                ...
                EnableWindow(GetDlgItem(hDlg, IDC_Focus1 + i - (i > partner)), 0);
            }
        }
        for (i = 0; i < MAX_CHAT - 1; i++) {
            Button_SetStyle(GetDlgItem(hDlg, IDC_Focus1 + i), BS_PUSHBUTTON | BS_LEFT, TRUE);
        }
        x = wpConsole.x;
        y = wpConsole.y;
        EnsureOnScreen(&x, &y, 0, 0);
...
void ChatPopUp(char * icsHandle) {
    ...
    if (partner == -1) {
        DisplayError("You first have to close a Chat Box\nbefore you can open a new one", 0);
        return;
    }
    if (icsHandle) {  // [HGM] clickbox set handle in advance
        safeStrCpy(chatPartner[partner], icsHandle, sizeof(chatPartner[partner]) / sizeof(chatPartner[partner][0]));
        if (sscanf(icsHandle, "%d", &i) == 1) {  // make sure channel is on
            snprintf(buf, MSG_SIZ, "addlist ch %d\n", i);
            SendToICS(buf);
            if (first) {
                first = 0,
                SendToICS(buf);  // work-around for weirdness: On public FICS code first attempt on login is completely ignored
            }
        }
    } else {
        chatPartner[partner][0] = NULLCHAR;
    }
    chatCount++;
