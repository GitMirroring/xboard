...
LRESULT CALLBACK EditTagsDialog(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam) {
    ...
    case WM_INITDIALOG: /* message: initialize dialog box */
        ...
        SendMessage(hwndText, WM_SETFONT, (WPARAM)font[boardSize][EDITTAGS_FONT]->hf, MAKELPARAM(FALSE, 0));
        ...
        if (!editTagsDialog) {
            ...
            if (wpTags.x != CW_USEDEFAULT && wpTags.y != CW_USEDEFAULT && wpTags.width != CW_USEDEFAULT &&
                ...
                *p = NULLCHAR;
                ...
                if (resPtr) {
                    *resPtr = strdup(str);
                    ...
                    SaveToBook(str), DisplayBook(currentMove);
                } else {
                    err = ReplaceTags(str, &gameInfo);
                }
                if (err) {
                    DisplayError(_("Error replacing tags."), err);
                }
...
VOID TagsPopDown(void) {
    if (editTagsDialog) {
        ShowWindow(editTagsDialog, SW_HIDE);
    }
...
VOID EitherTagsPopUp(char * tags, char * msg, BOOLEAN edit) {
    ...
    *q = NULLCHAR;
    ...
    if (editTagsDialog) {
        ...
        CreateDialog(hInst, MAKEINTRESOURCE(DLG_EditTags), hwndMain, (DLGPROC)lpProc);
...
VOID EditTagsPopUp(char * tags, char ** dest) {
    if (resPtr == &firstChessProgramNames && *engineListFile) {
