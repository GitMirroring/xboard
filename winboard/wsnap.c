...
static int Adjust(LONG * data, int new, int old, int vertical) {
    ...
    if (!vertical && (old == mainRect.left || old == mainRect.right)) {
        return 0;
    }
    if (vertical && (old == mainRect.top || old == mainRect.bottom)) {
        return 0;
    }
...
static void KeepTouching(
 int side, int new, int old, HWND hWnd) {  // if the mentioned window was touching on the moved edge, move its touching edge too
    if (IsWindowVisible(hWnd)) {
        ...
        case 1:
            i = Adjust(&rc.right, new, old, 0);
            break;
        case 2:
            i = Adjust(&rc.left, new, old, 0);
            break;
        case 3:
            i = Adjust(&rc.bottom, new, old, 1);
            break;
        case 4:
            i = Adjust(&rc.top, new, old, 1);
            break;
        }
...
LRESULT OnExitSizeMove(SnapData * snapData, HWND hWnd, WPARAM wParam, LPARAM lParam) {
    if (side && hWnd != hwndMain) {  // [HGM] glue: we have been sizing, by dragging an edge
        ...
        case 1:
            ...
            break;
        case 2:
            pos = activeRect.right;
            break;
        case 3:
            pos = activeRect.top;
            break;
        case 4:
            pos = activeRect.bottom;
            break;
        }

        for (i = 0; i < len; i++) {
            if (grid[i] == pos) {
                break;  // the dragged side originally touched another auxiliary window
            }
        }
