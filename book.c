...
int
int_from_file (FILE *f, int l, uint64_t *r)
{
    int i,c;
    for(i=0;i<l;i++){
        c=fsgetc(f);
...
int
find_key (FILE *f, uint64_t key, entry_t *entry)
{
...
void
move_to_string (char move_s[20], uint16_t move)
{
    int f,fr,ff,t,tr,tf,p;
...
void
int_to_file (FILE *f, int l, uint64_t r)
{
    int i;
...
void
NewEntry (entry_t *e, uint64_t key, int move, int result)
{
    e->key = key;
    e->move = move;
    e->learnPoints = 0;
