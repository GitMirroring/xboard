...
static void drawHighlight(int file, int rank, int type) {
    ...
    if (flipView) {
        x = lineGap / 2 + ((BOARD_WIDTH - 1) - file) * (squareSize + lineGap);
...
void SetHighlights(
 int fromX, int fromY, int toX, int toY) {  // [HGM] schedule old for erasure, and leave drawing new to DrawPosition
    ...
    if (change > 12 && IsDrawArrowEnabled()) {
        ArrowDamage(hi1X, hi1Y, hi2X, hi2Y);
    }
...
void ClearHighlights(void) { SetHighlights(-1, -1, -1, -1); }
...
void DoSleep(int n) { FrameDelay(n); }

static void SetRect(MyRectangle * rect, int x, int y, int width, int height) {
    rect->x = x;
    rect->y = y;
    rect->width = width;
    rect->height = height;
...
static void CalcUpdateRects(Pnt * old, Pnt * new, int size, MyRectangle update[], int * nUpdates) {
    ...
    if (new->y > old->y) {
        ...
        SetRect(&(update[count]), old->x, old->y + size - (old->y - new->y), size, old->y - new->y);
        count++;
    }
    /* Left or right edge - don't overlap any update calculated above. */
    if (new->x > old->x) {
        SetRect(&(update[count]), old->x, Max(new->y, old->y), new->x - old->x, size - abs(new->y - old->y));
        count++;
    } else if (old->x > new->x) {
        SetRect(&(update[count]), new->x + size, Max(new->y, old->y), old->x - new->x, size - abs(new->y - old->y));
        count++;
    }
    /* Done */
    *nUpdates = count;
...
static void BeginAnimation(AnimNr anr, ChessSquare piece, ChessSquare bgPiece, int startColor, Pnt * start) {
    AnimState * anim = &anims[anr];

    if (appData.upsideDown && flipView) {
        piece += piece < BlackPawn ? BlackPawn : -BlackPawn;
    }
...
static void AnimationFrame(AnimNr anr, Pnt * frame, ChessSquare piece) {
    ...
    if (Intersect(&anim->prevFrame, frame, squareSize, &overlap, &pt)) {
        ...
        for (i = 0; i < count; i++) {
            CopyRectangle(anr, 2, DISP, updates[i].x - anim->prevFrame.x, updates[i].y - anim->prevFrame.y, updates[i].width,
             updates[i].height, updates[i].x, updates[i].y);
        }
        /* [HGM] correct expose rectangle to encompass both overlapping squares */
        if (x > anim->prevFrame.x) {
            w += x - anim->prevFrame.x, x = anim->prevFrame.x;
        } else {
            w += anim->prevFrame.x - x;
        }
        if (y > anim->prevFrame.y) {
            h += y - anim->prevFrame.y, y = anim->prevFrame.y;
        } else {
            h += anim->prevFrame.y - y;
        }
    } else {
        /* Easy when no overlap */
        CopyRectangle(anr, 2, DISP, 0, 0, squareSize, squareSize, anim->prevFrame.x, anim->prevFrame.y);
...
static void EndAnimation(AnimNr anr, Pnt * finish) {
    MyRectangle updates[4];
    MyRectangle overlap;
    Pnt pt;
    int count, i;
...
static void FrameSequence(AnimNr anr, ChessSquare piece, int startColor, Pnt * start, Pnt * finish, Pnt frames[], int nFrames) {
    int n;
...
static void DrawDragPiece(void) {
    ...
    DrawOneSquare(
     anims[Player].startSquare.x, anims[Player].startSquare.y, EmptySquare, anims[Player].startColor, 0, NULL, NULL, 0);
...
static void DrawSquare(int row, int column, ChessSquare piece, int do_flash) {
    ...
    if (flipView) {
        x = lineGap + ((BOARD_WIDTH - 1) - column) * (squareSize + lineGap);
        y = lineGap + row * (squareSize + lineGap);
    } else {
        x = lineGap + column * (squareSize + lineGap);
        y = lineGap + ((BOARD_HEIGHT - 1) - row) * (squareSize + lineGap);
    }
    ...
    if (square_color == 2 || appData.blindfold) {
        piece = EmptySquare;
    }
...
void SquareExpose(int i, int j, int d) {
    int x, y;
    if (flipView) {
        x = lineGap + ((BOARD_WIDTH - 1) - j) * (squareSize + lineGap);
        y = lineGap + i * (squareSize + lineGap);
    } else {
        x = lineGap + j * (squareSize + lineGap);
        y = lineGap + ((BOARD_HEIGHT - 1) - i) * (squareSize + lineGap);
    }
...
void DrawPositionX(int repaint, Board board) {
    ...
    if (nr == 0) {  // [HGM] dual: no highlights on second board yet
        ...
        if (pm1X >= 0 && pm1Y >= 0) {
            ...
            if (lineGap) {
                damage[nr][pm1Y][pm1X] |= 2;
            }
        }
        if (pm2X >= 0 && pm2Y >= 0) {
            drawHighlight(pm2X, pm2Y, 2);
            if (lineGap) {
                damage[nr][pm2Y][pm2X] |= 2;
            }
        }
        if (hi1X >= 0 && hi1Y >= 0) {
            drawHighlight(hi1X, hi1Y, 1);
            if (lineGap) {
                damage[nr][hi1Y][hi1X] |= 2;
            }
        }
        if (hi2X >= 0 && hi2Y >= 0) {
            drawHighlight(hi2X, hi2Y, 1);
            if (lineGap) {
                damage[nr][hi2Y][hi2X] |= 2;
            }
        }
        DrawArrowHighlight(hi1X, hi1Y, hi2X, hi2Y);
    } else {
        DrawArrowHighlight(board[EP_STATUS - 3], board[EP_STATUS - 4], board[EP_STATUS - 1], board[EP_STATUS - 2]);
    }
        ...
        if (repaint && SubtractTimeMarks(&now, &programStartTime) < 1000) {
            ...
            while (*p) {
                ...
                if (!strstr(appData.suppress, p)) {
                    if (i == 0) {
                        DrawSeekBackground(2 * squareSize, 3 * squareSize, 6.5 * squareSize, 5 * squareSize);
                    }
...
static void DrawArrowBetweenPoints(int s_x, int s_y, int d_x, int d_y) {
    Pnt arrow[8];
...
static void ArrowDamage(int s_col, int s_row, int d_col, int d_row) {
    ...
    if (2 * (d_row - s_row) > abs(d_col - s_col)) {
        d_row = 4 * d_row + 1;
    } else if (2 * (s_row - d_row) > abs(d_col - s_col)) {
        d_row = 4 * d_row + 3;
    } else {
        d_row = 4 * d_row + 2;
    }
    if (2 * (d_col - s_col) > delta) {
        d_col = 4 * d_col + 1;
    } else if (2 * (s_col - d_col) > delta) {
        d_col = 4 * d_col + 3;
    } else {
        d_col = 4 * d_col + 2;
    }
...
static void DrawArrowBetweenSquares(int s_col, int s_row, int d_col, int d_row) {
...
static Boolean IsDrawArrowEnabled(void) { return (appData.highlightMoveWithArrow || twoBoards && partnerUp) && squareSize >= 32; }
