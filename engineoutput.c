...
void SetProgramStats(FrontEndProgramStats * stats)  // now directly called by back-end
{
    ...
    if (clearMemo) {
        if (!appData.headers) {
            columnHeader[0] = NULLCHAR;
        }
        DoClearMemo(which);
        nrVariations[which] = 0;
        ...
        if (gameMode == AnalyzeMode) {
            ...
            if (!which) {
                snprintf(header[which] + strlen(header[which]), MSG_SIZ - strlen(header[which]), "%s%s", exclu, columnHeader);
            }
...
static int IsEnginePondering(int which) {
    ...
    case TwoMachinesPlay:
        if (GetActiveEngineColor() != ENGINE_COLOR_UNKNOWN) {
            if (GetEngineColor(which) != GetActiveEngineColor()) {
                result = TRUE;
            }
        }
        break;
...
static void UpdateControls(EngineOutputData * ed) {
    ...
    if (pvStart != 0 && *pvStart != '\0') {
        ...
        if (ed->nodes < 1000000) {
            ...
            snprintf(s_nodes, sizeof(s_nodes), "%.*f%c%s\t", 1 + (x < 1e7), x / 1e6, unit, spaces + 10 + 2 * (ed->nodes >= 1e8));
        }
        ...
        } else if (h >= MATE_SCORE) {
            snprintf(s_score, 16, "  %s#%d%c\t", (h > MATE_SCORE + 9 ? "" : "  "), h - MATE_SCORE, fail);
        } else if (-h >= MATE_SCORE) {
            snprintf(s_score, 16, " %s#-%d%c\t", (-h > MATE_SCORE + 9 ? "" : "  "), -h - MATE_SCORE, fail);
        } else if (h > 0) {
        ...
        if (time_secs >= 3600) {
            snprintf(
             s_time, sizeof(s_time) / sizeof(s_time[0]), "%d:%02d:%02d\t", time_secs / 3600, (time_secs / 60) % 60, time_secs % 60);
        } else {
        ...
        if (ed->nodes == 0 && ed->score == 0 && ed->time == 0) {
            ...
            snprintf(
             buf, sizeof(buf) / sizeof(buf[0]), "%3d\t%s%s%s%s%s%s", ed->depth, s_score, s_nodes, s_time, s_hits, s_knps, s_seld);
        }
...
void Collapse(int n) {  // handle click on column headers, to hide / show them
    ...
    if (j != Ncol) {  // list hidden columns, so user ca click them
        ...
        for (i = 1; i < Ncol; i++) {
            if (m & 1 << i) {
                strcat(columnHeader, titles[i - 1]);
            }
        }
        strcat(columnHeader, ")");
    }
