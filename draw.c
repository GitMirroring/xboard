...
static void CreatePNGBoard(char * s, int kind) {
    ...
    if (strstr(s, ".png")) {
        ...
        if (cairo_surface_status(img) == CAIRO_STATUS_SUCCESS) {
            char c, *p = s, *q;
            int r, f;
            if (pngOriginalBoardBitmap[kind]) {
                cairo_surface_destroy(pngOriginalBoardBitmap[kind]);
            }
            if (n[kind] != 1.) {
                cairo_surface_destroy(pngBoardBitmap[kind]);
            }
            useTexture |= kind + 1;
            pngOriginalBoardBitmap[kind] = img;
            ...
            if (strlen(p) < 11 && sscanf(p, "-%dx%d.pn%c", &f, &r, &c) == 3 && c == 'g') {
                ...
                while (squareSize > n[kind] * w || squareSize > n[kind] * h) {
                    n[kind]++;
                }
            }
            if (n[kind] == 1.) {
...
char * pngPieceNames[] =  // must be in same order as internal piece encoding
 {"Pawn", "Knight", "Bishop", "Rook", "Queen", "Advisor", "Elephant", "Archbishop", "Marshall", "Gold", "Commoner", "Canon",
  ...
  "Iron", "Viking", "Flag", "Axe", "Dolphin", "Leopard", "Claw", "Left", "Butterfly", "PromoBishop", "PromoRook", "HCrown",
...
RsvgHandle * LoadSVG(char * dir, int color, int piece, int retry) {
    char buf[MSG_SIZ];
    ...
    if (svg) {
        ...
        if (cairo_surface_status(img) == CAIRO_STATUS_SUCCESS) {
            if (pngPieceImages[color][piece]) {
                cairo_surface_destroy(pngPieceImages[color][piece]);
            }
...
static void SaveWindowsBitmap(ChessSquare piece, int color, int * data, int stride, int w, int h, int bpp) {
    ...
    if (piece >= appData.bmpSave && piece != WhiteKing) {
        return;
    }
    f = fopen(buf, "wb");
    if (!f) {
    ...
    fclose(f);
    if (appData.bmpSave && piece == WhiteKing && color) {
        exit(0);
    }
}
...
static void ClearPieces(void) {
    ...
    for (i = 0; i < 2; i++) {
        for (p = 0; p < BlackPawn; p++) {
            if (pngPieceImages[i][p]) {
                cairo_surface_destroy(pngPieceImages[i][p]);
            }
            pngPieceImages[i][p] = NULL;
            if (svgPieces[i][p]) {
                rsvg_handle_close(svgPieces[i][p], NULL);
            }
            svgPieces[i][p] = NULL;
        }
    }
...
void InitDrawingParams(int reloadPieces) {
    if (reloadPieces) {
        ClearPieces();
    }
    CreateAnyPieces(1);
...
void SetPen(cairo_t * cr, float w, char * col, int dash) {
...
void DrawBorder(int x, int y, int type, int odd) {
    ...
    case 0:
        ...
        break;
    case 1:
        col = appData.highlightSquareColor;
        break;
    case 2:
...
static int CutOutSquare(int x, int y, int * x0, int * y0, int kind) {
    ...
    *x0 = 0;
    *y0 = 0;
    if (textureW[kind] < squareSize || textureH[kind] < squareSize) {
        return 0;
    }
    if (modV[kind] > 0) {
...
static void BlankSquare(cairo_surface_t * dest, int x, int y, int color, ChessSquare piece,
 int fac) {  // [HGM] extra param 'fac' for forcing destination to (0,0) for copying to animation buffer
    ...
    {  // evenly colored squares
        ...
        switch (color) {
        case 0:
            col = appData.darkSquareColor;
            break;
        case 1:
            col = appData.lightSquareColor;
            break;
        case 2:
            col = "#000000";
            break;
...
static void pngDrawPiece(cairo_surface_t * dest, ChessSquare piece, int square_color, int x, int y) {
    ...
    if (piece == WhiteKing && kind == appData.jewelled) {
        piece = WhiteZebra;
    }
...
void DoDrawDot(cairo_surface_t * cs, int marker, int x, int y, int r) {
    cairo_t * cr;
...
static void InitAnimState(AnimNr anr) {
    if (c_animBufs[anr]) {
        cairo_surface_destroy(c_animBufs[anr]);
    }
    if (c_animBufs[anr + 2]) {
        cairo_surface_destroy(c_animBufs[anr + 2]);
    }
...
static void CairoOverlayPiece(ChessSquare piece, cairo_surface_t * dest) {
    ...
    if (doubleClick) {
        cairo_paint_with_alpha(pieceSource, 0.6);
    } else {
        cairo_paint(pieceSource);
    }
    cairo_destroy(pieceSource);
    if (appData.inscriptions[0]) {
        InscribeKanji(dest, piece, 0, 0);
    }
}

void InsertPiece(AnimNr anr, ChessSquare piece) { CairoOverlayPiece(piece, c_animBufs[anr]); }

void DrawBlank(AnimNr anr, int x, int y, int startColor) { BlankSquare(c_animBufs[anr + 2], x, y, startColor, EmptySquare, 0); }
...
void DoDrawPolygon(cairo_surface_t * cs, Pnt arrow[], int nr) {
    cairo_t * cr;
    int i;
    cr = cairo_create(cs);
...
void DrawRectangle(int left, int top, int right, int bottom, int side, int style) {
    ...
    switch (side) {
    case 0:
        ChoosePen(cr, PEN_BOLDWHITE);
        break;
    case 1:
        ChoosePen(cr, PEN_BOLDBLACK);
        break;
    case 2:
        ChoosePen(cr, PEN_BACKGD);
        break;
    }
    cairo_fill(cr);
