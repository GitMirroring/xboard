...
ArgDescriptor argDescriptors[] = {
 ...
 {"firstComputerString",            ArgString,             (void *)&appData.firstComputerString,     FALSE,   (ArgIniType)COMPUTER_STRING},
 {"secondComputerString",           ArgString,             (void *)&appData.secondComputerString,    FALSE,   (ArgIniType)COMPUTER_STRING},
 {"firstChessProgram",              ArgFilename,           (void *)&appData.firstChessProgram,       FALSE,   (ArgIniType)FIRST_CHESS_PROGRAM},
 {"fcp",                            ArgFilename,           (void *)&appData.firstChessProgram,       FALSE,   INVALID},
 {"secondChessProgram",             ArgFilename,           (void *)&appData.secondChessProgram,      FALSE,   (ArgIniType)SECOND_CHESS_PROGRAM},
 ...
 {"alarm",                          ArgTrue,               (void *)&appData.icsAlarm,                FALSE,   INVALID},
 {"xalarm",                         ArgFalse,              (void *)&appData.icsAlarm,                FALSE,   INVALID},
 {"-alarm",                         ArgFalse,              (void *)&appData.icsAlarm,                FALSE,   INVALID},
 ...
 {"highlightLastMove",              ArgBoolean,            (void *)&appData.highlightLastMove,       TRUE,    (ArgIniType)TRUE},
 ...
 {"highlightDragging",              ArgBoolean,            (void *)&appData.highlightDragging,       !XBOARD, (ArgIniType)TRUE},
 ...
 {"firstChessProgramNames",         ArgString,             (void *)&firstChessProgramNames,          TRUE,    (ArgIniType)FCP_NAMES},
 {"secondChessProgramNames",        ArgString,             (void *)&secondChessProgramNames,         !XBOARD, (ArgIniType)SCP_NAMES},
 ...
 {"addMasterOption",                ArgMaster,             (void *)NULL,                             FALSE,   INVALID},
 ...
 {"zippyWrongPassword",             ArgString,             (void *)&appData.zippyWrongPassword,      FALSE,   (ArgIniType)ZIPPY_WRONG_PASSWORD},
 ...
 {"zippyNoplayCrafty",              ArgBoolean,            (void *)&appData.zippyNoplayCrafty,       FALSE,   (ArgIniType)ZIPPY_NOPLAY_CRAFTY},
 ...
 {NULL,                             ArgNone,               (void *) NULL,                            FALSE,   INVALID}
};
/* clang-format on */

/* Kludge for indirection files on command line */
char * lastIndirectionFilename;
ArgDescriptor argDescriptorIndirection = {"", ArgSettingsFilename, (void *)NULL, FALSE, INVALID};
...
void AppendToSettingsFile(char * line) {
    char buf[MSG_SIZ];
    FILE * f;
    int c;
...
int ValidateInt(char * s) {
    ...
    while (*p) {
        if (!isdigit(*p++)) {
            ExitArgError(_("Bad integer value %s"), s, TRUE);
        }
    }
    return atoi(s);
}
...
void ParseArgs(GetFunc get, void * cl) {
    ...
    for (;;) {
        ...
        if (ch == ';') {
            ...
            /* Switch */
            q = argName;
            while (ch != ' ' && ch != '=' && ch != ':' && ch != NULLCHAR && ch != '\n' && ch != '\t') {
                *q++ = ch;
                ch = get(cl);
            }
            *q = NULLCHAR;
            for (ad = argDescriptors; ad->argName != NULL; ad++) {
                if (strcmp(ad->argName, argName + 1) == 0) {
                    break;
                }
            }
            if (ad->argName == NULL) {
...
not_special:
                ...
                case '\\':
                    if (ad->argType == ArgFilename || ad->argType == ArgSettingsFilename) {
                        goto not_special;
                    }
                    ch = get(cl);
                    switch (ch) {
...
void ParseArgsFromString(char * p) { ParseArgs(StringGet, &p); }
...
void SetDefaultTextAttribs(void) {
    ColorClass cc;
...
void InitAppData(char * lpCmdLine) {
    int i;
    ...
    if (appData.viewer && appData.viewerOptions[0]) {
        ParseArgsFromString(appData.viewerOptions);
    }
    if (appData.tourney && appData.tourneyOptions[0]) {
        ParseArgsFromString(appData.tourneyOptions);
    }
    ...
    if (appData.matchMode || appData.matchGames) {
        chessProgram = TRUE;
    }
    ...
    if ((!appData.noChessProgram && !chessProgram && !appData.icsActive) || (appData.icsActive && *appData.icsHost == NULLCHAR) ||
     (chessProgram && (*appData.firstChessProgram == NULLCHAR || *appData.secondChessProgram == NULLCHAR))) {
        PopUpStartupDialog();
    }
...
void SaveSettings(char * name) {
    ...
    for (ad = argDescriptors; ad->argName != NULL; ad++) {
        ...
        case ArgFloat:
            ...
            break;
        case ArgBoolean:
            fprintf(f, OPTCHAR "%s" SEPCHAR "%s\n", ad->argName, (*(Boolean *)ad->argLoc) ? "true" : "false");
            break;
        case ArgTrue:
