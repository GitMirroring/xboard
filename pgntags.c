 ...
 */
int ParsePGNTag(char * tag, GameInfo * gameInfo) {
    ...
    *p = NULLCHAR;
    ...
    /* Fold together the various ways of denoting White/Black rating */
    else if ((StrCaseCmp(name, "WhiteElo") == 0) || (StrCaseCmp(name, "WhiteUSCF") == 0)) {
        success = TRUE;
        gameInfo->whiteRating = atoi(value);
    } else if ((StrCaseCmp(name, "BlackElo") == 0) || (StrCaseCmp(name, "BlackUSCF") == 0)) {
        success = TRUE;
        ...
        if (oldVariant != VariantNormal) {
            safeStrCpy(engineVariant, value, MSG_SIZ);
        }
...
/* Print game info */
void PrintPGNTags(FILE * fp, GameInfo * gameInfo) {
    char * p;
...
char * PGNTags(GameInfo * gameInfo) {
    ...
    if (gameInfo->site) {
        len += strlen(gameInfo->site);
    }
    if (gameInfo->date) {
        len += strlen(gameInfo->date);
    }
    if (gameInfo->round) {
        len += strlen(gameInfo->round);
    }
    if (gameInfo->white) {
        len += strlen(gameInfo->white);
    }
    if (gameInfo->black) {
        len += strlen(gameInfo->black);
    }
    if (gameInfo->whiteRating >= 0) {
        len += 40;
    }
    ...
    if (gameInfo->timeControl) {
        len += strlen(gameInfo->timeControl) + 20;
    }
    if (gameInfo->variant != VariantNormal) {
        len += strlen(engineVariant) + 150;
    }
    if (gameInfo->extraTags) {
        len += strlen(gameInfo->extraTags);
    }
...
int ReplaceTags(char * tags, GameInfo * gameInfo) {
    ChessMove moveType;
    int err;
